...
*           if f% = 8% then str(date$,7%) = " " else str(date$,9%) = " "
        ...
        REM *** Is the Date Format already Normalized? ***
            ...
            if str(date$,1%,2%) > "31" then ADD_CENTURY_2_STD
...
ADD_CENTURY_2_NON_STD:
            if str(date$,5%,2%) < "21" then cc$ = "20" else cc$ = "19"
            str(date$,5%,4%) = cc$ & str(date$,5%,2%)
            goto AMERICAN
ADD_CENTURY_2_STD:
            if str(date$,1%,2%) < "21" then cc$ = "00" else cc$ = "19"
            date$ = cc$ & date$
            goto TEST_DATE
NORMALIZECC:
        REM *** Is the Date Format already Normalized? ***
            if format$ = "S"           then TEST_DATE
            if str(date$,3%,2%) > "31" then TEST_DATE
...
TEST_DATE:
            ...
            if date% >= 19000101% and date% <= 22000101% then GOOD_DATE
