        REM CAELUSASSOCIATESSPOKANEWASHINGTONALLRIGHTSRESERVEDCAELUSASSOC~
            *                                                           *~
            *   AAA   RRRR   IIIII  PPPP   RRRR    SSS   U   U  3333    *~
            *  A   A  R   R    I    P   P  R   R  S      U   U      3   *~
            *  AAAAA  RRRR     I    PPPP   RRRR    SSS   U   U   333    *~
            *  A   A  R   R    I    P      R   R      S  U   U      3   *~
            *  A   A  R   R  IIIII  P      R   R   SSS    UUU   3333    *~
            *                                                           *~
            *-----------------------------------------------------------*~
            ...
            * 07/19/02 ! (EWD001) Mod to get desc from oracle     ! TBM *~
        ...
        sub "ARIPRSU3" (dup%, prt%, abend%, cust_code$, invoice$, rptid$, ~
                        inv_no$)
...
REM            gosub oracle_connect                  /* (EWD001)         */
...
L11100:         FMT POS(25), PD(14,4), POS(41), 2*PD(14,4)
            ...
            orapart_desc$ = str(fields$(), 32%,250%)               /*(EWD001)*/
...
L11130:     call "CONVERT" (qty_ordr, 0.0, qty_ordr$)
            call "CONVERT" (qty_ship, 0, oraqty_ship$)    /*  (EWD003)  */
            call "CONVERT" (stock_prc, 2.4, price$)
            call "CONVERT" (qty_ordr, 0, oraqty_ordr$)           
REM            call "CONVERT" (oraqty_ship, 0, oraqty_ship$)
REM            str(a$, 1%, 3%) = ora_seqnr$                                     /*(EWD001)*/
...
L12050:     nbr_lines% = 0% 
            str(a$,49%,13%) = invoice$
            str(a$,65%,8%)  = inv_date$
            convert page_nbr% to str(a$,75%,3%), pic(##0)
             gosub print_line        
             gosub fax_blank  
            str(a$,54%,24%) = crmemo$
             gosub print_line        
...
L64010:   return                                  /* (EWD002) - End    */
        ...
        set_file_name
            ...
            str(fax_key$,10%,15%) = "FAXINV01"
            read #17,hold,key = fax_key$, using L10210, desc$,           ~
                eod goto check_next_done
L10210:         FMT POS(25), CH(30)
            convert str(desc$,27%,4%) to cnt%, data goto L10220

L10220:     cnt% = cnt% + 1%
            if cnt% > 9999% then cnt% = 0%
            convert cnt% to str(desc$,27%,4), pic(0000)
            
            file$       = "ewdf" & str(desc$,27%,4)
            
            put #17, using L10210, desc$
            rewrite #17
...
/*(EWD001)*/
        ...
        exit_program
            ...
            counter%       = 0%   :     REM fx1% = 0%  fx2% = 0%
            if error%      = 1% then L65500
            close #21
                if error% <> 0% then delete_fax

           call "SHOSTAT" ("  F A X I N G  " & fax_phone$ )
