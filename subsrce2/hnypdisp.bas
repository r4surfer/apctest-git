        REM THISPROGRAMWASGENERATEDUSINGTHEGENPGMPROGRAMWHICHISAPROPRIETR~
            *                                                           *~
            *  H   H  N   N  Y   Y  PPPP   DDDD   IIIII   SSS   PPPP    *~
            *  H   H  NN  N  Y   Y  P   P  D   D    I    S      P   P   *~
            *  HHHHH  N N N   YYY   PPPP   D   D    I     SSS   PPPP    *~
            *  H   H  N  NN    Y    P      D   D    I        S  P       *~
            *  H   H  N   N    Y    P      DDDD   IIIII   SSS   P       *~
            *                                                           *~
            *-----------------------------------------------------------*~
            * HNYPDISP - Display HNYPOOL Info for Part Specified.       *~
            *-----------------------------------------------------------*~
        ...
        sub "HNYPDISP"   (part$,         /* Part Number to Display     */~
...
*        back to the caller.  Also the Caller must open the files.
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim cms2v$50
            cms2v$ = "R5.01.08 12/18/89 1990 Payroll Update             "
        REM *************************************************************
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            date$ = date
            call "DATEFMT" (date$)
            str(line2$,62) = "HNYPDISP: " & str(cms2v$,,8)
...
L09200:              FMT CH(25), CH(32)
                str(line2$,,61) = "Part: " & part$ & " (" & descr$ & ")"

            plowkey$ = str(part$,,25) & str(str$,,3) & str(lot$,,6) &    ~
                       hex(00000000)
            call "PLOWNEXT" (#2, plowkey$, 34%, f1%(2))
            if f1%(2) = 1% then L09380
                u3% = 2%
                call "ASKUSER" (u3%, "HNYPOOL DISPLAY",                  ~
...
L10000: REM *************************************************************~
        ...
        inputmode : first_screen
            init(" ") errormsg$, inpmessage$, dsply$()
            plowkey$ = str(part$,,25) & str(str$,,3) & str(lot$,,6) &    ~
                       hex(00000000)
            gosub load_screen
...
*        Main Screen: Get Display Options
        ...
        report_printing   /* Produce Report per Criteria specified     */
            ...
            select printer (134)
            call "TIME" (runtime$)
            plowkey1$ = str(part$,,25) & str(str$,,3) & str(lot$,,6) &   ~
                        hex(00000000)
        ...
        report_loop
            call "PLOWNEXT" (#2, plowkey1$, 34%, f1%(2))
            if f1%(2) = 0% then end_report

            get #2 using L10470, qtys(1), qtys(2), std, postdate$, text$
L10470:         FMT POS(39), 3*PD(14,4), POS(159), CH(6), POS(183), CH(40)
...
L30090:     call "PLOWNEXT" (#2, plowkey$, 34%, f1%(2))
...
L30140:     get #2 using L30160, qtyleft, qtyorig, std, stds(), postdate$,~
                                text$
L30160:         FMT POS(39), 15*PD(14,4), CH(6), POS(183), CH(40)
            l% = l% + 1%
            call "DATEFMT" (postdate$)
