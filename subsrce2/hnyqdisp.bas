        REM THISPROGRAMWASGENERATEDUSINGTHEGENPGMPROGRAMWHICHISAPROPRIETR~
            *                                                           *~
            *  H   H  N   N  Y   Y   QQQ   DDDD   IIIII   SSS   PPPP    *~
            *  H   H  NN  N  Y   Y  Q   Q  D   D    I    S      P   P   *~
            *  HHHHH  N N N   YYY   Q   Q  D   D    I     SSS   PPPP    *~
            *  H   H  N  NN    Y    Q   Q  D   D    I        S  P       *~
            *  H   H  N   N    Y     QQQ   DDDD   IIIII   SSS   P       *~
            *                           Q                               *~
            *-----------------------------------------------------------*~
            * HNYQDISP - Display HNYQUAN Info for Part Specified.       *~
            *-----------------------------------------------------------*~
        ...
        sub "HNYQDISP"   (part$,         /* Part Number to Display     */~
                          #1,            /* HNYMASTR Channel           */~
                          #2,            /* HNYQUAN  Channel           */~
                          #3,            /* HNYPOOL  Channel           */~
                          #4 )           /* SYSFILE2 Channel           */
...
*        back to the caller.  Also the Caller must open the files.
        ...
        REM *************************************************************~
            ...
            * #2  ! HNYQUAN  ! Inventory Quantity File                  *~
            * #3  ! HNYPOOL  ! Inventory Pool File                      *~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            date$ = date
            call "DATEFMT" (date$)
            str(line2$,62) = "HNYQDISP: " & str(cms2v$,,8)
...
L09190:              FMT CH(25), CH(32), POS(74), 2*CH(4), PD(14,7)
            ...
            if f1%(2) = 1% then L09310
                u3% = 2%
                call "ASKUSER" (u3%, "HNYQUAN DISPLAY",                  ~
...
*        RETURN/PF-9/PF-12 implies Show Pool Info, Locations, or Costs
            ...
            if file$ <> "HNYPOOL" then main_screen
                call "HNYPDISP" (part$, str(dsply$(c%),,3%),             ~
                              str(dsply$(c%),5%,6%), #1, #3, method$(c%))
                goto main_screen

            show_costs    /* Display Cost Breakdown for line indicated */
                readkey$ = str(part$,,25%) & str(dsply$(c%),,3%) &       ~
                                             str(dsply$(c%),5%,6%)
...
L10900:     gosub'101
        ...
        report_printing   /* Produce Report per Criteria specified     */
            ...
            select printer (134)
            call "TIME" (runtime$)
            plowkey1$ = str(part$,,25) & str(str$(2,1))
...
L30140:     get #2 using L30160, str$, lot$, oh, bo, oo, cm, qc,          ~
                                each, stds(), mthd$
L30160:         FMT POS(42), CH(3), CH(6), POS(69), 5*PD(14,4),          ~
                    POS(117), 13*PD(14,4), POS(403), CH(1)
...
L30490:     call "PLOWNEXT" (#2, plowkey1$, 25%, f1%(2))
            if f1%(2) = 0% then L30660
                get #2 using L30530, str$, lot$, oh, bo, oo, cm, qc,      ~
                                                             each, stds()
L30530:              FMT POS(42), CH(3), CH(6), POS(69), 5*PD(14,4),     ~
                         POS(117), 13*PD(14,4)
