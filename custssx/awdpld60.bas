        REM *************************************************************~
            *                                                           *~
            *  Program Name      - AWDPLD60                             *~
            *  Creation Date     - 09/09/2009                           *~
            *  Last Modified Date- 08/25/2014                           *~
            ...
            *03/11/2011! sash only option added.           AWD000 ! DES *~
            *08/25/2014!(AWD001) mod for additional data          ! CMG *~
            *04/22/2014!(IM7691) mod for litelift balances. Also  ! PWW *~
            *05/13/2015!         added additional weight checks.  !     *~
            *07/13/2015! SR66916  mod to change glass height com- ! PWW *~
            *          !          par from hgt to dec_height      !     *~
            *05/09/2016! SR74452  mod to change glass height com- ! PWW *~
            *          !          par from dec_height to hgt. Also!     *~
            *          !          fix bug on with_range%.         !     *~
            *01/23/2020! CR2273  balance summary qty for cont h/s ! MES *~
            *************************************************************

            sub "AWDPLD60" (#2, #3, #6, #4, #5, #9, #10, #1,             ~
                             #11, #63, minth%, maxth%)
        ...
        dim                                                              ~
            ...
            sav_tht2$9,                  /* Tube Height(s) for Compare */~
        ...
        dim seqn$5,                      /* Production Sequence No.    */~
            ...
            bal2_top$8,                  /* Top Balance                */~
            bal2_bot$8,                  /* Bot Balance                */~
            tht$9,                       /* Tube Heights               */~
            tht2$9,                      /* Tube Heights               */~
        ...
        dim prod$4,                      /* Prodcut Key                */~
            ...
            vendor2$1,                   /* Vendor for balance (IM7691)*/~
            bal_type$1,                  /* Type of balance for product*/~
            bal_type2$1,                 /* Type of bal for prd(IM7691)*/~
...
REM            call "SHOSTAT" ("WRITE DETAIL")  stop
            ...
            write #7, using L20300,                                      ~
                ...
                subPart$,                   /* Subpart  (AWD001)    */   ~
                dim1es,                     /* Dim1es   (AWD001)    */   ~
                dim2es,                     /* Dim2es   (AWD001)    */   ~
                dim3es,                     /* Dim3es   (AWD001)    */   ~
...
L20310:     return
        ...
        read_work_file
REM            call "SHOSTAT" ("READ WORK FILE")   stop

            if mode% = 2% then goto L20400
                mode% = 2%  :  f% = 7%  :  gosub open_work
L20400:     gosub clear_line
            read #7, key > wk_key$, using L20410, wk_key$,               ~
                cus$, part$, subPart$, dim1es, dim2es, dim3es, eod goto L20998
L20410:             fmt ch(51), xx(10), ch(9), ch(25), ch(20), pd(14,4), ~
                        pd(14,4), pd(14,4)
...
/*(CR2273)*/
            ...
            read #4,key = readkey$, using L22700, desc$, eod goto no_cont
            convert str(desc$,30%,1%) to cont_qty%

no_cont:
            convert str(wk_key$,47%,5%) to sc_mqty%, data goto L20412
L20412:            
            if cont_qty% = 0% then goto L20415
            sc_mqty% = sc_mqty% * cont_qty%
...
REM         +---------------------------------------------------------------+
REM         | part pos 11 = screen, 4, 5 or 6 only?                         |
REM         +---------------------------------------------------------------+
/*AWD000*/  if sc_sash_flg$ = "N" and scr$ = "4" then goto read_work_file
/*AWD000*/  if sc_sash_flg$ = "N" and scr$ = "5" then goto read_work_file
/*AWD000*/  if sc_sash_flg$ = "N" and scr$ = "6" then goto read_work_file
...
REM            if wcl$ = " " then wcl$ = "."
            ...
            call "STRING" addr("RJ", bal2_top$, 8%)  /* Both steps reqd.*/
            call "STRING" addr("CT", bal2_top$, 8%)  /* to center data. */ 
            call "STRING" addr("RJ", bal2_bot$, 8%)  /* Both steps reqd.*/
            call "STRING" addr("CT", bal2_bot$, 8%)  /* to center data. */ 
...
REM          call "SHOSTAT"( "BUILD SUMMARY " )  stop
         ...
         for i% = 1% to 2%
          for x% = 1% to 6% step 5%
            ...
            str(readkey$,9%,8%) = bal2_top$
            if i% = 2% then str(readkey$,1%,8%) = bal_bot$
            if i% = 2% then str(readkey$,9%,8%) = bal2_bot$

            if readkey$ = " " then L23300
            str(readkey$,17%,2%) = tdi_top$       
            if i% = 2% then str(readkey$,17%,2%) = tdi_bot$      

            if pos(bal_top$ = "H") <> 0% then str(readkey$,19%,1%) = "H"
            if pos(bal_bot$ = "H") <> 0% then str(readkey$,19%,1%) = "H"
...
L23100:             fmt CH(19), BI(4)        
            hit% = 1%                        
L23200:                                      

            
            if cont_qty% <> 0% then goto cont_hs
...
/*2273*/
cont_hs:
            if days% <= 21% then tot% = tot% + cont_qty% 
                            
L23250:   if tdi$ = "99" then tot% = tot% + sc_mqty% /* Line item Qty */       
...
/*AWD000*/  if fieldnr% = 4% and sc_sash_flg$ = " " then sc_sash_flg$="Y"
        return
...
*        Define the Input Message for the Screen/Field Indicated
        ...
        scrn1_msg  :  data                                               ~
         ...
         "Enter 'Y' to Include Sash Only Info else Enter 'N'.          "
        ...
        initialize_variables
            init(" ") errormsg$, inpmessage$, sc_dept$, sc_dept_d$,      ~
                      ...
                      prddate_fr$, prddate_to$, sc_sash_flg$
...
L40190:     accept                                                       ~
               ...
               at (05,25), fac(lfac$(3%)), sc_data_flg$         , ch(01),~
                                                                         ~
               at (06,02), "Include Sash Only?:",                        ~
               at (06,25), fac(lfac$(4%)), sc_sash_flg$         , ch(01),~
                                                                         ~
                                                                         ~
...
L50100: Rem Enter 'Y' to include sash only or 'N'   sc_sash_flg$  AWD000
            if sc_sash_flg$ = "Y" or sc_sash_flg$ = "N" then goto L50105
                errormsg$ = "Please Enter 'Y' for Yes or 'N' for No."
L50105:     return
...
REM            call "SHOSTAT" ("BAL BOT --> " & bal_bot$)  stop
            ...
            print using L64040,seqn$, mdl$, wdt$, hgt$, gls$, scr$, ~
                bal_top$, bal2_top$, tdi_top$, bal_bot$, bal2_bot$, ~
                tdi_bot$, tddiff$, cus$, inf$, loc_top$, loc_bot$
...
L63350:     ln% = 5%
        ...
        generate_summary_report                
            ...
            readkey$ = all(hex(00))
          summary_rpt_loop
            tht$, tht2$, tdi$, hd_flag$ = " "
            read #8, key > readkey$, using L63400, readkey$, tot%,       ~
                eod goto summary_rpt_done, data goto summary_rpt_loop
L63400:             fmt CH(19), BI(4)         
            tht$  = str(readkey$,,8%)         
            tht2$ = str(readkey$,9,8%)         
            tdi$  = str(readkey$,17%,2%)             
            hd_flag$ = str(readkey$,19%,1%)   
            if (tht$ = sav_tht$ and tht2$ = sav_tht2$)        ~
                           or ln% <= 5% then goto L63450             
                print  :  ln% = ln% + 1%
L63450:     if ln% > 58% then gosub print_headings
            print using L64090, tht$, tht2$, tdi$, hd_flag$, tot%
            ln% = ln% + 1%
            sav_tht$  = tht$
            sav_tht2$ = tht2$
...
L50200: return
        ...
        lookup_awdplnbl
             bal% = 0%
/*IM7691*/   init(" ") prod$, vendor$, bal_type$, vendor2$, bal_type2$
             max_weight = 0.00
             prod$ = str(part$,1%,3%) & "T"
...
/*IM7691 -      if within_range% = 1% then noWeightCheck    SR74452         */
/*IM7691 - */   if within_range% = 0% then noWeightCheck  /*SR74452         */
...
REM                   tdi$ = td$
                   ...
                   bal_bot_test$ = bal_bot$
                   bal3_bot_test$ = str(bal_rec$,12%,16%)
