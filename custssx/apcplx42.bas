...
*                  (PAR000)  01/15/2006 CR347        Dept (052)        *~
*                                                                      *~
...
*                                                                      *~
*                                                                      *~  
*                  New Flex Line Processing for 410 Line               *~
*                  ( As of 01/15/2006 - RHH  Dept 052 )                *~
*                                                                      *~
*        AWDPLB42 - Create File with Data for Saw Optimization         *~
...
*                                                                      *~
*                                                                      *~
*09/30/2010 ! (AWD001) modification to allow tso & bso                 *~
*12/16/2011 ! (AWD002) modification to turn off /S in file per   ! CMG *~
*           !          Chad Boardman                             !     *~
*12/20/2011 ! (AWD003) modification to correct cut pieces        ! CMG *~
*02/10/2012 ! (AWD004) modification for flex from dept 027 to 036! CMG *~
*05/19/2014 ! (CUT001) mod to add dim fields to CUTCC            ! CMG *~
*04/29/2022 ! CR3086   add foam flag to material like awdplf42   ! RDB *~
*01/17/2023 ! CR3224 Missing foam on some orders                 ! RDB *~
************************************************************************

        sub "APCPLX42" (size%,           /* Batch Size (No. Windows)  */ ~
...
/* (AWD004) */
            ...
            dtl_subpart$ = str(wrk_rec$,81%,20%) /*(CR3086)*/			
...
/* (\AWD001) */
        ...
        build_schedule                         /* SET-UP SCHEDULE REV. */
          ...
          write #ff%, using L02510, bat_rec$, eod goto L02070
...
L02070:   errormsg$ = "(Error) When Writing Revision Record??"
        ...
        build_schedule_no
          ...
          write #ff%, using L02510, bat_rec$, eod goto L02200
        return
L02200:   errormsg$ = "(Error) When writing Schedule Record??"
        ...
        build_schedule_title                        /* SCHEDULE TITLE  */
          ...
          write #ff%, using L02510, bat_rec$, eod goto L02310
        return
L02310:   errormsg$ = "(Error) When Writing Schedule Title??"
        ...
        build_schedule_quantity                     /* SCHEDULE Quantity*/
          ...
          write #ff%, using L02510, bat_rec$, eod goto L02320
        return
L02320:   errormsg$ = "(Error) When Writing Schedule Quantity"
        ...
        build_detail_saw
          ...
          str(bat_rec$,149%,1%) = sa_s$(sa%)            /* Saw Set-up  */
                                                        /*  Number     */
          write #ff%, using L02510, bat_rec$, eod goto L02400
        return
...
REM    	  str(bat_rec$,72,3) = "***"
                                                        /* Bin Location*/
          str(bat_rec$,75%,16%) = "STD - Window    "    /* Part Desc   */
REM       str(bat_rec$,75%,16%) = sa_d1$(1)             /* Part Desc   */
          str(bat_rec$,91%,30%) = sa_d2$                /* Label Desc  */
          ...
          str(bat_rec$,149%,1%) = welder_set$(j%)       /* Welder Setup*/
                                                        /*  Number     */
          des1$ = str(bat_rec$,1,75)
	        des2$ = str(bat_rec$,76,74) & " "
...
L02500:   write #ff%,using L02510, bat_rec$, eod goto L02640
L02510:   FMT CH(149)
L02600: return
...
CS4:
        ...
        build_end
          ...
          write #ff%, using L02510, bat_rec$, eod goto L02750
          bat_no% = bat_no% + 1%
...
L02750:   errormsg$ = "(Error) Building End Record??"
        ...
        build_saw_recs
          if save_part_s$ <> dtl_part$ then goto L02850
             for i% = 1% to sa_max%
                 str(sa_rack$(i%),4%,10%)  = "A" & ssq$ & "-A" & ssq$ & "/"
REM                 str(sa_rack$(i%),14%,2%) = cp$(i%) /* No. of Pieces    */
/* (AWD003) */
                 str(sa_rack$(i%),14%,2%) = str(sa_piece$(i%),3%,2%)
             next i%
REM             return           /* CR3224 */

L02850:   save_part_s$ = dtl_part$                 /* NEWFAMILY Info */
          init(" ") sa_type$(), sa_piece$(), sa_cut$(), sa_part$(),      ~
                    sa_d1$(), sa_d2$, sa_m$(), sa_s$(), saw_no$(), ~
                    sa_cut_type$() /* removed sa_rack$ */
...
no_sash_f: 
               ...
               if cut_type% <> 2% then goto L03260 
                                                   ...
                                                   /* Special Height Mod*/
REM                  if i% <= cw% then goto L03260    /* Height Eq        */
...
L03060:
               ...
               sa_cut_type$(sa%) = cut_type$       /* Frame or Sash    */
                                                   /* Frame Equations  */
               if cut_type% <> 2% then goto L03430
                                                   /* Special Height Mod*/
                  if i% <= cw% then goto L03430    /* Height Eq        */
                     if c_o$ = "CO"  then                 ~
                         str(sa_m$(sa%),3%,1%) = "8"
                     if c_o$ = "OR" then                 ~
                         str(sa_m$(sa%),3%,1%) = "9"
