...
*             New Subroutine for Florida Window Departments 014        *~
*                                                                      *~
...
*                                                                      *~
*                                                                      *~
*        AWDPLR42 - Create File with Data for Saw Optimization         *~
...
*                                                                      *~
*                                                                      *~
*                (8)Mod to change weld quantities to (2) for triples.  *~
*                   Models 773, 775, 777, 787, 789                     *~
*                                                                      *~
*03/16/2009 ! ORIGNAL                                            ! CMG *~
*03/29/2012 ! (AWD001) mod for 8900 dept 014                     ! CMG *~
*05/21/2012 ! (AWD002) replace apccutcc with awdcutcc            ! CMG *~
*07/10/2012 ! (AWD003) mods for cottage/oriel/tso/bso cuts       ! CMG *~
*03/19/2013 ! (AWD004) mod to awdcutcc addl data                 ! CMG *~
*04/07/2014 ! (AWD005) mod for foam change weld code digit to 5  ! MES *~
*05/15/2014 ! (AWD006) mod for Joseph saw Jambs                  ! MES *~
*05/22/2014 ! (CUT001) mod to add dim fields in CUTCC            ! MES *~
*08/21/2014 ! (SR68115) mod for egress model E34                 ! MES *~
*08/11/2017 ! (CR1002)  - new top bottom parameter               ! RDB *~
*01/15/2019 ! (CR1842) mod for sash foam                         ! MES *~
...
************************************************************************
        ...
        sub "AWDPLR42" (size%,           /* Batch Size (No. Windows)  */ ~
        ...
        dim readkey$24,                  /* GENCODES Primary Key      */ ~
            ...
            bat_rec$222,                 /* (AWD006)Batch Record      */ ~
            ...
            dtl_subpart$20,              /* (AWD002) subpart          */ ~
            save_sub$20,                 /* Save Subpart (AWD003)     */ ~
            save_sub1$20,                /* Save subpart1 (AWD003)    */ ~
            dtl_new_part$45,             /* (AWD002) entire part      */ ~
...
/*(AWD005)*/foam$1,                      /* Foam Codes                */ ~
            ...
            cr_addl$(500%)5,             /* (AWD002) addl material num*/ ~
...
/*AWD004*/  adj(500%),                   /* Adjustment amt             */~
            ...
            save_newpart_s$45,           /* (AWD006) New Part Number  */ ~
            save_newpart_w$45,           /* (AWD006) MFG Part Number  */ ~
            sa_jpart$(500%)20,           /* (AWD006) joseph part      */ ~
            jpart$20,                    /* (AWD006) joseph part      */ ~
            sa_jmodel$(500%)20,          /* (AWD006) joseph model     */ ~
...
REM            if err% <> 0% then goto exit_program      /*(AWD006)*/

            call "AWDCUTLD" (scr_prod$, cw%, ch%, csw%, csh%, tw$, ~
                            th$, tsw$, tsh$,#2, err% )
            if err% <> 0% then goto create_next         /*(AWD006)*/
...
REM            call "AWDCUTLD" (scr_prod$, cw%, ch%, csw%, csh%, tw$, ~
                            th$, tsw$, tsh$,#2, err% )
REM            if err% <> 0% then goto create_next
/*(AWD006)*/

            dt_samp$  = str(wrk_rec$,77%,1%)  /* 0=No, 1=Samp, 2=Disp */

            dtl_subpart$ = str(wrk_rec$,81%,20%)                  /*(AWD002)*/

            sub_part$ = str(wrk_rec$,81%,20%) /* Subpart (AWD003) Add CR2887   */
            
            str(dtl_new_part$,1%,25%)  = str(dtl_part$,1%,25%)    /*(AWD002)*/
            str(dtl_new_part$,26%,20%) = str(dtl_subpart$,1%,20%) /*(AWD002)*/
            ...
            foam$     = str(dtl_subpart$,5%,1%) /*Set foam codes*/ /*(AWD005)*/

            gosub lookup_color                /* Get Color Code       */

REM            p% = pos("456" = s$)                        /*(AWD003) */
REM            if p% <> 0% then goto create_next           /*(AWD003)*/
/*(AWD003)*/
...
L02320:   errormsg$ = "(Error) When Writing Schedule Quantity"
        ...
        if sa_cut_type$(sa%) <> type$ then return /* type$ = 2 Frame */
          ...
          str(bat_rec$,203%,20%) = sa_jpart$(sa%)       /* Joseph Part (AWD005)*/
...
/* (AWD003) */
          ...
          pass% = 0%
        second_weld
          if type% = 1% and (c_o$ = "OR" or c_o$ = "CO" ~
                    or model$ = "E34") then sash_coor_weld% = 1%  /*(SR68115)*/
/* (\AWD003) */
          init(" ") bat_rec$
...
/*(AWD005)*/
          if type% = 2% and (foam$ = "3" or foam$ = "4") ~
              and str(model$,1%,1%)="E" then ~
                str(bat_rec$,42%,1%) = "5"
/*(AWD005)*/
...
REM             if save_part_s$ <> dtl_part$ or save_sub$ <> sub_part$ ~
                                   then goto L02850
/*(AWD006)*/if save_newpart_s$ <> dtl_new_part$ then goto L02850
                       for i% = 1% to sa_max%
...
REM Slider Sash Foam Matl
                p% = 0%
                p%=pos(sa_part$(sa%) = " ")
/*(CR1842)*/    if (model$ <> "S28" and model$ <> "S38") then goto foam_frame
                if (foam$ <> "3" and foam$ <> "4") then goto foam_frame
                if str(eq$(i%),6,3) = "205" then str(sa_part$(sa%),p%,1%) = "F"
                      /*sash HT cut*/
                if (str(eq$(i%),6,3) = "905" or str(eq$(i%),6,3) = "712") ~
                    then str(sa_part$(sa%),12%,1%) = "F"           /*(CR1842)*/
                      /* TSO BSO HT cuts   */                    
                    
foam_frame:                    
REM Foam Material Frame
               if cut_type% <> 2% then goto no_sash_f
                 if str(dtl_part$,1%,1%) = "E"              ~
                     and str(eq$(i%),7%,2%) = "03" then goto no_sash_f
                 if str(dtl_part$,1%,1%) = "S"              ~
                     and str(eq$(i%),7%,2%) = "11" then goto no_sash_f
                 if str(dtl_subpart$,5%,1%) = "3" then  /* (AWD002) */    ~
                                              str(sa_part$(sa%),p%,1%) = "F"
                 if str(dtl_subpart$,5%,1%) = "4" then  /* (AWD002) */    ~
                                              str(sa_part$(sa%),p%,1%) = "F"
no_sash_f:
REM               str(sa_part$(sa%),11%,5%) = cr_addl$(i%)    /* (AWD002) */
/* (AWD002) */
REM                p% = 0%
REM               for k% = 1% to 5%
REM                if str(sa_part$(sa%),10%+k%,1%) <> " " then p%=k%
REM                if p% <> 0% then k% = 5%
REM               next k%
REM               str(sa_part$(sa%),p%+11,5%-len%) = str(cr_addl$(i%),1%,5%-len%)
/* (\AWD002) */
...
/*(AWD006)*/  jpart$ = str(desc$,10%,20%)           /*  Joseph Part Nbr*/
...
/* (AWD003) */
            ...
            read #2,key = readkey$, using L04180 , desc$, eod goto L04260
               ...
               set_up$   = str(desc$,5%,1%)          /* Set-Up Code    */
               saw_no$   = str(desc$,7%,2%)          /* Saw Number     */
/*(AWD006)     jpart$    = str(desc$,10%,20%)        /* jsph part AWD006*/
               check%    = 1%                        /* Valid Equation */
...
REM          IF DLT_PART$ = SAVE_PART$ THEN RETURN /* Frame 1st - Sash 2nd  */
/* (AWD003) */
          if dtl_part$ = save_part$ and save_sub1$ = sub_part$ then return
             save_part$ = dtl_part$
             save_sub1$ = sub_part$                /*sub_part$ not defined*/
/*(AWD006)*/
REM             if dtl_new_part$ = save_newpart_w$ then return
REM                save_newpart_w$ = dtl_new_part$
...
/* (AWD003) */
          ...
          w%, h%, weld% = 0%
/* (AWD003) record cottage / oriel first and second width cuts */
          co_h1%, co_h2% = 0%
...
L05000:     a  = ct(i%)                        /* Welder Sizes are in   */
                                               /* Decimal               */
* (AWD003) has to be cottage or oriel has to be equations 3 or 4 */
          add% = 0%
REM Do not add to Sliders
          if str(dtl_part$,1%,1%) = "S" then goto no_add
...
/* (AWD003) if the part is a top sash then 0.9375 width and Height      */
/* so the welder will open for the correct bottom size and the top sash */

/*SR68115*/ if str(dtl_part$,1,3) = "E34" then add% = 1%
            if str(dtl_part$,11,1) = "4" then a = a + 0.9375
            if add% = 1% and str(eq$(i%),7,2) = "05" then a = a + 0.9375
/* (AWD003\) */
