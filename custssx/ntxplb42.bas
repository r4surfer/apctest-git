...
*             (PAR000) 01/15/06 CR347            NTX Dept (058/057)    *~
*             Models (5W1)    copy of dept 053  AWDPLFU2               *~
*																	   *~	                    
...
************************************************************************
        ...
        sub "NTXPLB42" (size%,           /* Batch Size (No. Windows)  */ ~
...
ntx_dept:
            ...
            ff% = 3% : ff_nam$ = "@SAW058@"     /* DEPT 058 */
/*(AWD002)*/
	        if dept% <> 59% and dept% <> 57% then goto StartFile            
            
            ff% = 7% : ff_nam$ = "@SAW059@"   /* DEPT 059 */
            
            if dept% <> 57% then goto StartFile 
            
            ff% = 8% : ff_nam$ = "@SAW057@"   /* DEPT 057 */
StartFile:            
...
/*(AWD002) non std changes to frame jambs*/
  			if cut_type% <> 2% then goto NoFab  /* Only Frames (AWD002) */
                 	slider% = 0%
REM                	 if model$ = "5W1" then sh% = 1%         /*(AWD002*/
                 	if model$ = "5S2"  then slider% = 2%     /*(AWD002)*/
                 	if model$ = "8S2"  then slider% = 2%     /*(AWD002)*/
                 	if model$ = "5S3" then slider% = 3%    /*(AWD002)*/
                 	if model$ = "8S3" then slider% = 3%    /*(AWD002)*/
                                                       /* Skip Meeting Rail */
...
REM    Begin Slider H&S non std (AWD002)
		sliderhs
		 ...
		 if slider% = 2% and hng$ = "07" then jpart$ = "5S225" 
		 if slider% = 3% and hng$ = "09" then jpart$ = "5S206" 
...
L03940: return
        ...
        check_cut                                    /* 1st Frame      */
            ...
            if dept% = 58% then str(readkey$,1%,9%)  = "NEWFAMILY" /*(AWD002)*/  
            if dept% = 59% or dept% = 57%                        /*(SR73949)*/~
            	then str(readkey$,1%,9%)  = "NEWFAMIL2" /*(AWD002)*/
...
L04260: return
        ...
        check_welder                                 /* 1st Frame      */
            ...
            if dept% = 58% then str(readkey$,1%,9%)  = "NEWFLEXWD" /*(AWD002)*/  
            if dept% = 59% or dept% = 57%                      /*(SR73949)*/ ~
            	then str(readkey$,1%,9%)  = "NEWFLEXW2" /*(AWD002)*/
