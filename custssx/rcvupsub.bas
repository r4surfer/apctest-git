        REM CAELUSASSOCIATESSPOKANEWASHINGTONALLRIGHTSRESERVEDCAELUSASSOC~
            ...
            * 08/02/06 ! Changes for the voiding (RCVXREF)        ! DES *~
...
L10125:         FMT CH(25), CH(16), CH(9), CH(16), POS(116), CH(6),      ~
            ...
            gosub check_xref:
	    /* is voided, skip */
REM         if str(xref_rec$,98,1) = "V" then goto L10035
...
/*AWD002*/  if str(xref_rec$,98,1) <> "V" then goto L12940
            temp_gl_amt = glamtc
	    glamtc = glamtd
	    glamtd = temp_gl_amt

L12940:     call "GLPSTSUB" (gltype$, "   ", 0%, userid$, hnydate$,      ~
...
L16820:     if count% = 0% then return
            ...
            for i% = 1% to count%
                if newquan(i%) = 0 then L16930
                     ...
                     if str(xref_rec$,98,1) = "V" then quan = quan * -1
REM                  if str(xref_rec$,98,1) = "V" then quan = 0
...
check_xref:
            init(" ") xref_rec$
	    /* fix serial$ */
            str(xref_rec$,01,16) = str(record$(),51,16) /* PO       */
            str(xref_rec$,17,09) = str(record$(),42, 9) /* Ven Code */
            str(xref_rec$,26,03) = str(record$(),67, 3) /* Vbklseq# */
            read #13,key = str(xref_rec$,1,28), hold, using L19550,      ~     
                              xref_rec$, eod goto L19570   

            if str(xref_rec$,98,1) = "V" then return
REM         if str(xref_rec$,79,16) <> "                " then return
L19550:     FMT CH(256)
            /* key3 */
            str(xref_rec$,37,25) = str(record$(),01,25) /* Part#    */
            str(xref_rec$,62,03) = str(record$(),332,3) /* Store    */
            str(xref_rec$,65,06) = str(record$(),335,6) /* Lot      */
            str(xref_rec$,71,08) = str(record$(),70,08) /* DateTime */
            /* key4 */
            str(xref_rec$,79,16) = str(record$(),26,16) /* Receiver */
            /* data */
            str(xref_rec$,95,3)  = userid$

            rewrite #13, using L19550, xref_rec$, eod goto L19570
