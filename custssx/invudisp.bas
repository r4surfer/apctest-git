        REM *************************************************************~
            *                                                           *~
            *  IIIII  N   N  V   V  U   U  DDDD   IIIII   SSS   PPPP    *~
            *    I    NN  N  V   V  U   U  D   D    I    S      P   P   *~
            *    I    N N N  V   V  U   U  D   D    I     SSS   PPPP    *~
            *    I    N  NN   V V   U   U  D   D    I        S  P       *~
            *  IIIII  N   N    V     UUU   DDDD   IIIII   SSS   P       *~
            *                                                           *~
            *-----------------------------------------------------------*~
            * INVUDISP - Display HNYQUAN Info for Part Specified.       *~
            *-----------------------------------------------------------*~
        ...
        sub "INVUDISP"   (part$,         /* Part Number to Display     */~
                          #1,            /* INVMASTR Channel           */~
                          #2,            /* INVHSTRY Channel           */~
                          #3,            /* STORNAME Channel           */~
                          #4 )           /* INVDETAL Channel           */
...
*        back to the caller.  Also the Caller must open the files.
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim cms2v$50
            cms2v$ = "REV:01.00 03/03/06 New Part Number                "
        REM *************************************************************
        ...
        REM *************************************************************~
            ...
            * #2  ! INVHSTRY ! Inventory Usage History File     (PAR000)*~
            * #3  ! STRONAME !                                          *~
            * #4  ! INVDETAL !                                  (PAR000)*~ 
            *************************************************************~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            str(line2$,62) = "INVUDISP: " & str(cms2v$,,8)
...
L09330:              FMT CH(45), CH(32)
            ...
            if f1%(2%) = 1% then L09450
                u3% = 2%
                call "ASKUSER" (u3%, "INVHSTRY DISPLAY",                 ~
...
L09460:     str$, str1$ = str(plowkey$,46%,3%)
            convert (mod( val( str(plowkey$,49%,2%), 2%), 100%)) to yy$, pic(00)
            convert     ( val( str(plowkey$,49%,2%), 2%)) to oldyear$, pic(0000)
            call "PLOWNEXT" (#2, plowkey$, 48%, f1%(2%))
...
*        RETURN indicates to show detail for month where cursor is at
            ...
            if mm% < 1% or mm% > 12% then main_screen
                ...
                call "DATEFMT" (dat2$): call "DATUNFMT" (dat2$)
                                                     /* (PAR000)  */
                call "INVDDISP" (part$, "D", dat1$, dat2$, str$, str$,   ~
                                 "ALL", "   ", " ", #1, #4)
                goto main_screen

        first_year                                  /* (PAR000)  */ 
            plowkey$ = str(part$,,45%) & str(str$) & hex(00)
            call "PLOWNEXT" (#2, plowkey$, 48%, f1%(2%))
            if f1%(2%) = 0% then main_screen
                get #2 using L10380, yyyy%
L10380:              FMT XX(48), BI(2)             /* (PAR000)  */
        ...
        prev_year                                    /* (PAR000)  */ 
            plowkey$ = str(part$,,45%) & str(str$) & hex(00)
            first%   = 1%
L10460:     call "PLOWNEXT" (#2, plowkey$, 48%, f1%(2%))
            if f1%(2%) = 0% then L10530
                get #2 using L10380, yyyy%           /* (PAR000)  */ 
...
L10560:     year$ = yy$
        ...
        next_year                                   /* (PAR000)  */
            ...
            plowkey$ = str(part$,,45%) & str(str$) & bin( year%, 2% )
            call "PLOWNEXT" (#2, plowkey$, 48%, f1%(2%))
            if f1%(2%) = 1% then L10660
                errormsg$ = "Already displaying last year on file."
                goto main_screen
L10660:     get #2 using L10380, year%              /* (PAR000)  */
...
L10750:     gosub'101
        ...
        load_screen
            init (" ") dsply$()
                                                            /* (PAR000) */
            convert year$ to year%
            readkey$ = str(part$,,45%) & str(str$) & bin( year%, 2% )
