...
*                                                                      *~
*                                                                      *~
*                  New Flex Line Processing for 450 Brick Mold Line    *~
*                  ( As of 01/15/2006 - RHH  Dept 017 )                *~
*                                                                      *~
*        AWDPLJ42 - Create File with Data for Saw Optimization         *~
...
*03/09/2015! (AWD008) mod to remove sash size limit,REP in AWD007! MES *~
*11/17/2015!(SR69252) mod to add egress model G18 & B27          ! MES *~
*12/22/2015!(SR70993) mod to F for foam                          ! MES *~
*02/27/2017! (CR851)  mod 130 to Urban saws                      ! MES *~
*08/11/2017! (CR1002)  - new top bottom parameter                ! RDB *~
*06/15/2021! (CR2846) update for BMDH sliders                    ! CMN *~
*03/16/2022! CR2704  Change weld descrpt to show paint           ! RDB *~
*08/15/2022! CR3153  Add logic like dept 019 jpart for dept 017  ! RDB *~
************************************************************************

        sub "AWDPLJ42" (size%,           /* Batch Size (No. Windows)  */ ~
...
/* (\AWD006) */
            ...
            ff% = 3% : ff_nam$ = "@SAW017@"     /* DEPT 017 */
/* (AWD006) */
            if dept% <> 19% then goto NotDept019
              ff% = 7% : ff_nam$ = "@SAW019@"   /* DEPT 019 */
...
/* CR3153    if dept% = 19% then goto SawDept19            (CR851)     */
/* Per CR both dept 017 and 019 set position 183 only 2 depts calling  */
          goto SawDept19 
...
SawDept19:
          str(bat_rec$,183%,20%) = sa_jpart$(sa%)      /* Urban Cluster (CR851)*/
          str(bat_rec$,203%,20%) = "                    "
L02399:
...
REM          egress% = 0%
        second_weld
          if dept% = 19% then goto noSecondWeld
/*(SR69252)*/
REM       if model$ = "G18" or model$ = "B27" then egress% = 1%
REM          if type% = 1% and (c_o$ = "OR" or c_o$ = "CO" and ~
REM                     egress% = 1%) then sash_coor_weld% = 1%
          if type% = 1% and (c_o$ = "OR" or c_o$ = "CO" or ~
                    egress% = 1%) then sash_coor_weld% = 1%
...
REM             aa = 0.0                                   /* Special Test*/
                                                        /* Sash Limit  */
REM             gosub convert_height
      /*(AWD008)*/
...
REM        convert_height                              /* Of window       */
                                                      /*(AWD008)*/
REM            convert str(dtl_part$,17%,2%) to a1, data goto CS3
REM CS3:
REM            convert str(dtl_part$,19%,1%) to a2, data goto CS4
REM CS4:
REM            aa = a1 + (a2/8.0)                     /* Decimal Height   */
...
not_sash:
               ...
               str(sa_part$(sa%),11%,5%) = "     "
/*(SR70993)*/
                 if cut_type% <> 2% then goto not_sdl
                 p% = 0%                               /* (SR70993)        */
                 p% = pos(sa_part$(sa%) = " ")
...
/* (AWD001) - sdl bottom lock rail */
               goto not_sdl               /* TURN OFF SDL LOGIC */
...
not_sdl:
               ...
               if c_o$ <> "OR" and c_o$ <> "CO" and egress% = 0%   ~
                        then goto not_co_or
...
/* (AWD006) */
REM               if dept% <> 19% then goto Not130
REM               if cut_type% <> 2% then goto Not130  /* Only Frames       */
REM                slider%, sh% = 0%
REM               if model$ = "FH1" then sh% = 1%
REM               if model$ = "FX1" then sh% = 2%      /*(AWD007)*/
REM               if model$ = "FS2" then slider% = 2%
REM               if model$ = "FS3" then slider% = 3%
                                                       /* Skip Meeting Rail */
REM               if eqNum$ = "05" and slider% = 0%  then goto Not130
REM                  if eqNum$ = "17" and slider% <> 0% then goto Not130 /*(CR851)*/

REM                  if i% > cw% and eqNum$ = "02" and slider% <> 0%  ~
REM                         then goto Not130
REM                  p1% = 0%
REM                  p1% = pos(sa_part$(sa%) = " ")


REM                  p2% = 0%
REM                  p2% = pos(sa_part$(sa%) = " ")
REM                  if sillopt$ = "5" then gosub sillOpt5
REM                  if sillopt$ = "4" then gosub sillOpt4
REM                  if sillopt$ = "7" then gosub sillOpt7
REM                  if sillopt$ = "6" then gosub sillOpt6
REM Not130
...
REM        if eqNum$ = "01" /*and i% <= cw%*/ then goto JambPart
REM        if eqNum$ = "02" then goto HeadSillPart    /*(AWD007)*/
REM        return

REM        JambPart
REM          if i% <= cw% and slider% = 0%  then return /* SH width */
REM          if i% > cw%  and slider% <> 0% then return /* Slider height*/
/*(CR851)*/
REM          if sh% = 1% and c_o$ = "OR" then jpart$ = "130 SH JAMBS OR"
REM        return

REM        HeadSillPart
REM        if i% <= cw% and slider% = 0%  then return /* SH widths*/
/*(CR851)*/
REM        if slider% = 2% and hng$ = "07" then jpart$ = "130 SL H&S OX"
REM        if slider% = 3% and hng$ = "09" then jpart$ = "130 SL H&S 333"
REM        return
...
REM --------------------
REM        sillOpt5
REM        str(sa_part$(sa%),p2%,1%) = "F"

REM         if eqNum$ = "01" then goto JambPart
REM         if eqNum$ = "02" then goto HeadSillPart


/*        JambPart5 removed CR851 */
REM (AWD007)
REM        if i% <= cw% and slider% = 0%  then return
REM        if i% > cw%  and slider% <> 0% then return
REM          if slider% = 2% and hng$ = "07" then jpart$ = "HS HEAD&SILL OX"
REM          if slider% = 3% and hng$ = "09" then jpart$ = "HS H&S 333"
REM          if sh% = 1% and c_o$ = "OR" then jpart$ = "130 SH JAMBS OR"
REM        return
REM        HeadSillPart5
REM        if i% <= cw% and slider% = 0%  then return
...
REM --------------------
REM        sillOpt4
REM          str(sa_part$(sa%),p2%,2%) = "RF"

REM         if jpart$ <> " " then                     ~
REM /*(CR851)*/     jpart$ = str(jpart$,1%,3%) & "RF" & str(jpart$,4%,16%)

REM         if eqNum$ = "01" then goto JambPart      /*(AWD008)(CR851)*/
REM         if eqNum$ = "02" then goto HeadSillPart

 /*          JambPart4                                  (AWD008)(CR851) removed */
REM         if i% <= cw% then str(sa_part$(sa%),p%,3%) = "RFH"
REM            if i% <= cw% and slider% = 0%  then return
REM            if i% > cw%  and slider% <> 0% then return

REM          if sh% = 1% and c_o$ = "OR" then jpart$ = "SH REP JAMBS ORL"
REM           if sh% = 1% and c_o$ = "OR" then jpart$ = "130 SH JAMBS OR"
REM           return
REM        HeadSillPart4
REM         if i% <= cw% then str(sa_part$(sa%),p%,3%) = "RFS"
REM (CR851)
REM        if slider% = 2% and hng$ = "07" then jpart$ = "130 SL H&S OX"
REM        if slider% = 3% and hng$ = "09" then jpart$ = "130 SL H&S 333"

REM         goto setHeadSillJmodel (CR851)
REM        return 
...
REM --------------------
REM        sillOpt7
REM         str(sa_part$(sa%),p2%,1%) = "R"

REM         if jpart$ <> " " then                     ~
REM /*(CR851)*/     jpart$ = str(jpart$,1%,3%) & "R" & str(jpart$,4%,16%)

REM         if eqNum$ = "01" then goto JambPart         /*(AWD008)(CR851)*/
REM         if eqNum$ = "02" then goto HeadSillPart


REM        JambPart7                                    (AWD008)(CR851) removed
REM         if i% <= cw% then str(sa_part$(sa%),p%,2%) = "RH"
REM            if i% <= cw% and slider% = 0%  then return
REM            if i% > cw%  and slider% <> 0% then return
REM          if slider% = 2% and hng$ = "07" then jpart$ = "HS REP HEAD OX"
REM          if slider% = 3% and hng$ = "09" then jpart$ = "HS REP HEAD 333"
REM          if sh% = 1% and c_o$ = "OR" then jpart$ = "SH REP JAMBS ORL"
REM           if sh% = 1% and c_o$ = "OR" then jpart$ = "130 SH JAMBS OR"

REM          return
REM        HeadSillPart7
REM       if i% <= cw% then str(sa_part$(sa%),p%,2%) = "RS"      (AWD008)
REM (CR851)
REM        if slider% = 2% and hng$ = "07" then jpart$ = "130 SL H&S OX"
REM        if slider% = 3% and hng$ = "09" then jpart$ = "130 SL H&S 333"

REM          goto setHeadSillJmodel
REM        return 
...
/* (AWD002) */
            ...
            if s$ = "5" and str(readkey$,14%,1%) = "H" then ~
                         str(readkey$,14%,1%) = "9"
/* (AWD006) */
REM In APCCUTEQ, the Head formula will need to be turned on, Y for width equation 01,
REM but will only need to be included in the @saws019@.dat file when SILLOPT field 29 is 4 or 7.

REM            checkHead% = 0%
REM            headForm%  = 0%
REM            if dept% = 19% and eqNum$ = "01" and i% <= cw% then checkHead% = 1%
REM            if checkHead% = 1% then gosub checkHead
REM            if checkHead% = 1% and headForm% = 0% then goto L04240
/* (\AWD006) */
...
/*(CR2846) logic from APCPLR42 for sliders*/              
              if dept% <> 19% then goto notSliderMachine
                                                     /* Cottage        */
...
/* CR3153 */
             /* if dept% = 19% then                       (CR2846)       */ 
                 jpart$ = str(desc$,10%,20%)         /* Urban Cluster(CR851)*/
...
REM ------------------
          ...
          if dept% = 19% then goto no_add            /* (AWD006) */
...
REM if str(dtl_part$,1,3) = "G18" then add% = 1%
REM if str(dtl_part$,1,3) = "B27" then add% = 1%
/*(SR69252)*/
            if egress% = 1% then add% = 1%
            if dept% = 19% then no_add1    /* (CR2846) */
