...
*             (PAR000) 01/15/06 CR347            NTX Dept (045)        *~
*																	   *~	                    
*              NTX Linealmate file   DH line                           *~  
...
* 06/09/2015! (AWD002) mod changes for Dept045                   ! MES *~
* 02/16/2015! (AWD003) mod for nosashes for 8300 slider          ! MES *~
* 02/26/2015! (AWD004) mod for CHS sash qty                      ! MES *~
* 03/16/2015! (AWD005) mod to add sash welds back for 8300 slider! MES *~
* 04/06/2015! (IM63505) mod logc for PW and no sash weld line    ! MES *~
* 06/11/2015! (SR65694) mod CO/OR logic                          ! MES *~
* 08/11/2015! (SR67763) mod to take subtract 1/8 from 8300 DH    ! MES *~
* 01/12/2015! (SR71691) mod for egress G18/B27 models            ! MES *~
* 06/28/2016! (SR75762) mod to correct SR67763 and make fraction ! MES *~
* 07/27/2017! (CR1002) mod for AWDCUTCC field t_b$               ! MES *~
* 08/04/2017! (CR1003) mod for Cot/Oriel width welders           ! MES *~
* 10/09/2017! (CR1161) mod to remove 1/8 SR67763                 ! MES *~
* 02/23/2018! (CR1265) mod to add sash welder                    ! MES *~
* 08/20/2018! (CR1518) mod to add descr for slider               ! MES *~
************************************************************************

        sub "NTXPLE42" (size%,           /* Batch Size (No. Windows)  */ ~
        ...
        dim readkey$24,                  /* GENCODES Primary Key      */ ~
            ...
            sz$100,                      /* FRACTIONS FOR 16'THS (SR75672)  */~
            calc$9,                      /* Temp for Conversion  (SR75672)  */~
            t_b$(100%)1                  /* (CR1002) Top/Bot           */ 
...
REM            if dt_seq$ <> "00312" then goto NotSeq
REM               call "SHOSTAT" ("AT SEQ --> " & dt_seq$)  stop
REM NotSeq:
...
/* (SR65694) If tso or bso and it is cottage or oriel then skip*/
            ...
            if count% = 0% then gosub build_schedule   /* CREATE BATCH */
                                                       /* New Area     */
               egress%=0%							   /*(SR71691)     */
               if model$ = "G18" or model$ = "B27" then egress% = 1%                                         
...
/* (AWD002) */
REM         if type% = 2% and (s$ = "4" or s$ = "5") then return (CR1265)
...
/*(SR65694)*/          
          ...
          pass% = 0%
        second_weld
          if type% = 1% and (c_o$ = "OR" or c_o$ = "CO" or egress% = 1%)   ~ 
          		then sash_coor_weld% = 1%                 /*(SR71691)     */
/*(\SR65694)*/		
...
/*(AWD004)*/
 ...
 /*(CR1003)*/
          if sash_coor_weld% = 1% and pass% = 0% then ~
                str(bat_rec$,20%,9%)  = width$(co_w1%) /* Height Cut  */

          if sash_coor_weld% = 1% and pass% = 1% then ~
                str(bat_rec$,20%,9%)  = width$(co_w2%) /* Height Cut  */
/*(\CR1003)*/                                                          
/*(SR65694)*/
          if sash_coor_weld% = 1% and pass% = 0% then ~
...
/*(SR65694)*/ 
          if (sash_coor_weld% = 1% and pass% = 0%) or (s$= "4") then /*CR1265*/~
               str(bat_rec$,41%,2%) = "21"    
          if (sash_coor_weld% = 1% and pass% = 1%) or (s$= "5") then /*CR1265*/~
               str(bat_rec$,41%,2%) = "22"
/*(\SR65694)*/   
/*(SR71691)*/     
		  if egress% = 1% and pass% = 0% then ~
               str(bat_rec$,41%,2%) = "23"
          if egress% = 1% and pass% = 1% then ~
               str(bat_rec$,41%,2%) = "24"
/*(\SR71691)*/
...
/*SR67763 Remove logic CR1161*/
			
REM                if model$ <> "267" then goto Not8300HS	
					
REM                    if s$ = "4" or  s$ = "5" then goto Not8300HS  
REM                    if i% > cw% then goto Not8300HS		/*add SR75762*/	   
                                   
REM                    if i% < cw% and (eqNum$ > "02") 
REM			                       then goto Not8300HS/*changed to >02 SR75762*/
REM			         ct(i%) = ct(i%) - .125          /* SUBTRACT 1/8   */
...
/*(AWD002) non std changes to frame jambs*/
  			if cut_type% <> 2% then goto NoFab  /* Only Frames (AWD002) */
                 	slider% = 0%
                    if model$ = "267" or model$ ="B21" then dh% = 1% /*(AWD002*/
                	if model$ = "B14" or model$ ="B24" then dh% = 1% /*(AWD002*/
                	if model$ = "G17" or model$ ="G49" then dh% = 1% /*(AWD002*/
                	if model$ = "C15" or model$ ="C20" then dh% = 1% /*(AWD002*/
                	if model$ = "417" or model$ ="449" then dh% = 1% /*(AWD002*/
                	if model$ = "C12" or model$ ="C18" then dh% = 1% /*(AWD002*/
                 	if model$ = "G59" or model$ ="G79" then dh% = 1% /*(AWD002*/
                	if model$ = "B15" or model$ ="B22" then dh% = 1% /*(AWD002*/
                 	if model$ = "G69" or model$ ="G89" then dh% = 1% /*(AWD002*/
                	if model$ = "B16" or model$ ="B26" then dh% = 1% /*(AWD002*/
                 	if model$ = "128"  then slider% = 2%     /*(AWD002)*/
                 	if model$ = "B32"  then slider% = 2%     /*(AWD002)*/
                 	if model$ = "S24"  then slider% = 2%     /*(AWD002)*/
                 	if model$ = "138" then slider% = 3%    /*(AWD002)*/
                 	if model$ = "B33" then slider% = 3%    /*(AWD002)*/
                 	if model$ = "S34" then slider% = 3%    /*(AWD002)*/
                                                       /* Skip Meeting Rail */
REM                  	if eqNum$ = "33" and slider% = 0%  then goto NoFab   
                 	if i% > cw% and eqNum$ = "01" and slider% <> 0%  ~
                         then goto NoFab  /*slider jambs*/ /*(AWDD003)*/
                  	p% = 0%
                 	 p% = pos(sa_part$(sa%) = " ")
             
            if slider% = 2% or slider% = 3% then gosub sliderdh
            if dh% = 1% then gosub dhjamb  /*  (AWD002)  */ 	     	 
...
REM    Begin Slider H&S non std (AWD002)
		sliderdh
		 ...
		 if eqNum$ <> "01" then return  /*sill eqution needs to be 01 0r 02*/
		 if slider% = 2% and hng$ = "07" then str(jmodel$,1%,2%) = "25" 
		 if slider% = 3% and hng$ = "09" then str(jmodel$,1%,2%) = "06" 
		 
		 return
		 
REM Begin DH Jamb non std (AWD002) 
		dhjamb
		 if i% <= cw% then return
		 if eqNum$ <> "01" then return
		 if c_o$ = "OR" then str(jmodel$,1%,2%) = "04"
		 if c_o$ = "CO" then str(jmodel$,1%,2%) = "03"
REM		 if egress% = 1% then str(jmodel$,1%,2%) = "24"
            ...
            if str(desc$, p%+2%, 4%) <> "TWIN" and                       ~
               str(desc$, p%+2%, 4%) <> "TRPL" then goto L03900
                  ...
                  if str(desc$,p%+2%,4%) = "TRPL" then                   ~
                     str(co$, cnt%+1%,3%) = "/TR"
               cnt% = cnt% + 3%
L03900:     if str(dtl_part$,9%,2%) < "06" and str(dtl_part$,9%,2%) > "09"  ~
                then goto L03910                            /*( CR1518) */
            if str(dtl_part$,9%,2%) = "06"                             ~
                then str(co$,cnt%+1%,3%) = "/XO"                 /* XO  */
       	    if str(dtl_part$,9%,2%) = "07"                              ~
                then str(co$,cnt%+1%,3%) = "/OX"                 /* OX  */
            if str(dtl_part$,9%,2%) = "08"                              ~
                then str(co$,cnt%+1%,3%) = "/42"                 /* 1/4 1/2  */
            if str(dtl_part$,9%,2%) = "09"                              ~
                then str(co$,cnt%+1%,3%) = "/33"                 /* 1/3  */
             cnt% = cnt% +3%   
L03910:     if str(dtl_new_part$,41%,1%) <> "2" then goto L03940 /*(CR1518)*/
            if str(dtl_new_part$,41%,1%) = "2"                           ~
                then str(co$,cnt%+1%,3%) = "/CD"                  /* WOCD   */
           

REM if str(dtl_part$,9%,2%) <> "09" then goto L03940
REM               str(co$,cnt%+1%,3%) = "/33"                 /* 1/3,1/3  */
REM            cnt% = cnt% + 3%
...
L03940: return
        ...
        check_cut                                    /* 1st Frame      */
            ...
            if dept% = 45% then str(readkey$,1%,9%)  = "NEWFAMILY" /*(AWD002)*/  
...
L04260: return
        ...
        check_welder                                 /* 1st Frame      */
            ...
            if dept% = 45% then str(readkey$,1%,9%)  = "NEWFLEXWD" /*(AWD002)*/  
...
REM if dt_seq$ <> "00045" then goto NotSeq
REM call "SHOSTAT" ("SEQ ->" & seq$)stop
REM NotSeq:
...
/*CR1002*/
            if (s$ = "4" or s$ = "5") then goto sashOnly         /*(CR1265)*/

            if sh$(i%) = "H" and t_b$(i%) = "T" then h% = 3%   /*top*/  
            if sh$(i%) = "H" and t_b$(i%) = "B" then h% = 2%   /*bot*/
            if sh$(i%) = "W" and t_b$(i%) = "T" then w% = 3%   /*(CR1003) top*/
            if sh$(i%) = "W" and t_b$(i%) = "B" then w% = 2%   /*(CR1003) bot*/  
...
/*(SR65694)*/
			...
			if (c_o$ <> "OR" and c_o$ <> "CO" and egress% = 0%) then goto L05020          
/*              if str(eq$(i%),7,2) = "05" then co_h1% = h%     top  
              if str(eq$(i%),7,2) = "06" then co_h2% = h%     bot
              if str(eq$(i%),7,2) = "10" then co_w1% = w%     (CR1003) top
              if str(eq$(i%),7,2) = "08" then co_w2% = w%     (CR1003) bot*/
              
              if sh$(i%) = "H" and t_b$(i%) = "T" then co_h1% = h%     /*top*/  
              if sh$(i%) = "H" and t_b$(i%) = "B" then co_h2% = h%     /*bot*/
              if sh$(i%) = "W" and t_b$(i%) = "T" then co_w1% = w%     /*(CR1003) top*/
              if sh$(i%) = "W" and t_b$(i%) = "B" then co_w2% = w%     /*(CR1003) bot*/
...
L02270:       convert a% to str(calc$,1%,3%), pic(###)
              if b% <> 0% then str(calc$,5%,5%) = str(sz$,(b%*5%) - 4%,5%)
        return   
