...
*             (PAR000) 01/15/06 CR347            NTX Dept (015)        *~
*																	   *~	                    
*              NTX Linealmate file   150/160                           *~  
...
* 02/09/2015! (AWD002) mod changes for Dept015                   ! MES *~
...
* 01/11/2018! (CR1250) switch to Joseph Saw                      ! MES *~
************************************************************************

        sub "NTXPLC42" (size%,           /* Batch Size (No. Windows)  */ ~
        ...
        dim readkey$24,                  /* GENCODES Primary Key      */ ~
            ...
            mtg_rail$3,                  /* (AWD002) 5'2 sash location*/~
...
/*(AWD002) non std changes to frame jambs*/
  			if cut_type% <> 2% then goto NoFab  /* Only Frames (AWD002) */
                 	slider% = 0%
                    if model$ = "511" then sh% = 1%         /*(AWD002*/
                	if model$ = "513" then sh% = 1%         /*(AWD002*/
                	if model$ = "512" then sh% = 1%         /*(AWD002*/
                	if model$ = "514" then sh% = 1%         /*(AWD002*/
                 	if model$ = "126"  then slider% = 2%     /*(AWD002)*/
                 	if model$ = "127"  then slider% = 2%     /*(AWD002)*/
                 	if model$ = "S23"  then slider% = 2%     /*(AWD002)*/
                 	if model$ = "136" then slider% = 3%    /*(AWD002)*/
                 	if model$ = "137" then slider% = 3%    /*(AWD002)*/
                 	if model$ = "S33" then slider% = 3%    /*(AWD002)*/
                                                       /* Skip Meeting Rail */
...
REM     if slider% = 2% and hng$ = "07" then str(jpart$,4%,2%) = "25" 
REM		 if slider% = 3% and hng$ = "09" then str(jpart$,4%,2%) = "06" 
/*(CR1250)*/
...
REM		 if c_o$ = "OR" and mtg_rail$ = "000" then str(jpart$,4%,2%) = "04"
REM		 if c_o$ = "OR" and mtg_rail$ <> "000" then str(jpart$,4%,2%) = "24"
REM		 if c_o$ = "CO" then str(jpart$,4%,2%) = "03"
/*(CR1250)*/
         if c_o$ = "OR" and mtg_rail$ = "000" then str(jmodel$,1%,2%) = "04"
         if c_o$ = "OR" and mtg_rail$ <> "000" then str(jmodel$,1%,2%) = "24"
...
L03940: return
        ...
        check_cut                                    /* 1st Frame      */
            ...
            if dept% = 15% then str(readkey$,1%,9%)  = "NEWFAMILY" /*(AWD002)*/  
...
L04260: return
        ...
        check_welder                                 /* 1st Frame      */
            ...
            if dept% = 15% then str(readkey$,1%,9%)  = "NEWFLEXWD" /*(AWD002)*/  
