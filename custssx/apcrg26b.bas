        REM *************************************************************~
            *                                                           *~
            *  Program Name      - APCRG26B  (Like APCRGA6B)            *~
        ...
        sub "APCRG26B"  (#1,             /* GENCODES File Channel      */~
                         #2,             /* CUSTOMER File Channel      */~
                         #6,             /* SLMMASTR File Channel      */~
                         rpt_value$,     /* Salesman No. or 'ALL'      */~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim cms2v$50
            cms2v$ = "01.00.00 11/13/95 RGA by Salesman Report        "
        REM *************************************************************
            mat f2% = con
...
/*PAR000*/              varc,     indexed,  recsize =  512,              ~
        ...
        REM *************************************************************~
            ...
            *************************************************************
        begin_process
            call "SHOSTAT" ("Creating RGA's by Salesman Report")
        ...
        select_one
            str(workkey$,1%,4%) = str(rpt_value$,1%,4%)

        select_all
            read #12,key > workkey$, using L35190, rga_salesman$,         ~
                rga_number$, rga_item$, eod goto L19360

            if  proc$         = "1"           then L19250
                proc$         = "1"
                sav_salesman$ = rga_salesman$
                gosub lookup_salesman

L19250:     if sav_salesman$  = rga_salesman$ then L19290
...
L19290:     if rpt_sum$ = "D" then gosub process_apcrgadt                ~
            ...
            str(workkey$,1%,4%) = rga_salesman$
            str(workkey$,5%,4%) = rga_number$
            str(workkey$,9%,2%) = rga_item$
            goto select_all
L19360: return
...
L19710: return
        ...
        select_report
            ...
            title$  = "APC Building Products  -  RGA's by Salesman  (Deta~
        ~il)"
            goto L19800
L19780:     title$  = "APC Building Products  -  RGA's by Salesman  (Summ~
        ~ary)"
L19800:     pageno% = 0%
            lcnt%   = 99%
            call "TIME"    (xtime$)
            call "SETPRNT" (" ", "RGA6", 0%, 0%)
        ...
        initialize_variables
            init(" ") rga_number$, rga_item$, rga_cuscode$, rga_auth_id$,~
                ...
                rga_so$, rga_line$, rga_piece$, rga_qty$, sav_salesman$, ~
                rga_hd_status$, rga_dt_status$, sav_number$, proc$,      ~
                salesman_name$, rga_salesman$, xtime$, sc_txt$
            dtlkey$ = all(hex(20)) : workkey$ = all(hex(20))
            rga_total% = 0%        : salesman_total% = 0%
        return
...
L31130:     get  #4, using L31140, rga_salesman$, rga_number$, rga_item$
L31140:         FMT POS(224), CH(04), POS(12), CH(04), CH(02)

            if str(rpt_value$,1%,3%) = "ALL"          then L31180
            if str(rpt_value$,1%,4%) <> rga_salesman$ then L31200
...
L35190:     FMT CH(04),                  /* RGA Salesman No.           */~
                CH(04),                  /* RGA Number                 */~
                CH(02)                   /* RGA Item                   */
        ...
        REM *************************************************************~
            ...
            *************************************************************
                                    /* RGA's by Salesman No. Columns   */
                                    /* Header Format                   */
L55060: %  ######## @ ########            ###############################~
        ~#######################            Page: ###

                                    /* APCRGAHD Format                 */
L55100: %  Salesman No.: ####   Name: ##############################
...
L55170: %  Total RGA's for Salesman No. #### : ####
L55180: %  Total RGA Items for Salesman No. #### : ####
...
L55280: %         ***** End Of RGA's by Salesman Report  (Summary) *****
L55300: %         ***** End Of RGA's by Salesman Report  (Detail) *****
        ...
        print_apcrgadt
            ...
            lcnt%           = lcnt%           + 1%
            salesman_total% = salesman_total% + 1%
        return
        ...
        print_header
            ...
            print
            print using L55100, rga_salesman$, salesman_name$
            print
            lcnt% = 6%
...
L60410: return
        ...
        end_selection
            ...
            print
            print using L55170, sav_salesman$, rga_total$
            goto L60540
L60500:     convert salesman_total% to salesman_total$, pic(####)

            print
            print using L55180, sav_salesman$, salesman_total$
L60540:     gosub lookup_salesman
        ...
        end_report
            ...
            print using L55170, sav_salesman$, rga_total$
            goto L60760
L60720:     convert salesman_total% to salesman_total$, pic(####)
...
L60940:     init(" ") status_desc$
        ...
        lookup_salesman
            read #6,key = rga_salesman$, using L61000, salesman_name$,    ~
                eod goto L61030
L61000:         FMT POS(05), CH(30)

        return
L61030:     salesman_name$ = "No Salesman Name"
        return

        lookup_part                           /* Check HNYMASTR        */
            init(" ") part_desc$, apc_prt$, apc_sze$, apc_scr$, dt_flag$
            read #9,key = rga_part$,using L61100, part_desc$, apc_prt$,   ~
                apc_sze$, eod goto L61140
L61100:         FMT XX(25), CH(32), POS(606), CH(60), CH(20)

            goto L61280

L61140:     err% = 0%
            if len(rga_part$) > 18% then goto L61230
...
L61230:     call "APCDESCR" (rga_part$, apc_scr$, apc_prt$, apc_sze$,    ~
                             #11, err%)
...
L61280: return
        ...
        lookup_apcplnsc
            ...
            convert rga_line$ to xx%, data goto L61350
L61350:
            convert xx%       to str(sc_key$,9%,2%), pic(##)

            read #8,key = sc_key$, using L61390, sc_txt$, eod goto L61410
L61390:         FMT POS(100), CH(04)
...
L61410: return
        ...
        lookup_text                           /* Look Up Text Id       */
            ...
            str(text_key$,9%,1%) = "1"
            sav_key1$            = text_key$
            read #5,key > text_key$, eod goto L61650

            get  #5, using L61590, text_key$, atext$()
L61590:         FMT CH(11), POS(64), 2*CH(70)

            if str(sav_key1$,1%,9%) <> str(text_key$,1%,9%) then L61650
...
L61720: return
        ...
        return
L61830:     call "SHOSTAT" ("ERROR - CANNOT OPEN (APCRGAWK)") : stop
