...
*             (PAR000) 01/15/06 CR347               Dept (053)         *~
*             Models (5W1)    copy of dept 027  AWDPLF42               *~
*                                                                      *~  
...
*           09/15/2014! (AWD002) mod to add Urban new            ! MES *~
*           09/16/2016!(SR68922) mod to add repl Models 8S2,8S3  ! MES *~
*           08/11/2017!(CR1002)  new top bottom parameter        ! RDB *~ 
************************************************************************

        sub "AWDPLU42" (size%,           /* Batch Size (No. Windows)  */ ~
        ...
        dim readkey$24,                  /* GENCODES Primary Key      */ ~
            ...
            sa_jpart$(100%)20,           /* (AWD002) Urban cluster    */ ~
            sa_jmodel$(100%)20,          /* (AWD002) joseph model     */ ~
            jpart$20,                    /* (AWD002) urban cluster    */ ~          
...
L03040:
        ...
        REM          convert ct(i%) to str(sa_cut$(sa%),1%,7%), pic(###.###)
               ...
               str(sa_part$(sa%),1%,7%) = cr$(i%) /* Raw Material Part*/
               str(sa_part$(sa%),8%,2%) = cr_addl$(i%)
               str(sa_part$(sa%),10%,6%) = "    "
...
/*(AWD002) non std changes to frame jambs*/
  			if cut_type% <> 2% then goto NoFab  /* Only Frames (AWD002) */
                 	slider%, sh% = 0%
                	 if model$ = "5W1" or model$ = "8W1"  ~ 
                	 		then sh% = 1%         /*(AWD002*//*(SR68922)*/
                 	if model$ = "5S2" or model$ = "8S2"   ~  
                 			then slider% = 2%     /*(AWD002)*//*(SR68922)*/
                 	if model$ = "5S3" or model$ = "8S3"   ~ 
                 	  		then slider% = 3%    /*(AWD002)*//*(SR68922)*/
                                                       /* Skip Meeting Rail */
                  	if eqNum$ = "33" and slider% = 0%  then goto NoFab   
                 	if i% > cw% and eqNum$ = "39" and slider% <> 0%  ~
...
REM    Begin Slider H&S non std (AWD002)
		sliderhs
		 if i% > cw% then return
		 if eqNum$ <> "41" then return
		 if slider% = 2% and hng$ = "07" then jpart$ = "5700 SL H&S OX" 
		 if slider% = 3% and hng$ = "09" then jpart$ = "5700 SL H&S 333" 
		 
		 return
		 
REM Begin SH Jamb non std (AWD002)
		shjamb
		 if i% <= cw% then return
		 if eqNum$ <> "31" then return
		 if c_o$ = "OR" then jpart$ = "5700 SH JAMBS ORL"
		 if c_o$ = "CO" then jpart$ = "5700 SH JAMBS COT"
