        REM *************************************************************~
            ...
            *                                                           *~
            *  Program Name      - APCPLE40 - Cloned from APCPLC40      *~
            ...
            * 10/21/05 ! SR69112  Mod to fix PF5 calculateing     ! PWW *~
            *          !          scanned units.                  !     *~
            *************************************************************

            sub "APCPLE40" ( dt_dept$,   /* Specified Department Code  */~
...
/*SR69122 temp for testing * * * * *  !    ~
            sc_dte_test$ = date~
            call "DATE" addr("G+",sc_dte_test$, -(4%),sc_dte_test2$,err%)~
            sc_dte$ = sc_dte_test2$~
   temp for testing * * * * *   */
...
/*         bg_dte$ = hex(00201510060F)      temp for testing  */
/*         ed_dte$ = hex(00201510070F)      temp for testing  */
...
/*         if ad_st$ <> "14" then goto calc_scanned_nxt       Scanned */
                  goto L02150
...
/*SR69112  str(dt_key$,19%,3%) = str(ad_rec$,25%,3%)      Department   */
/*SR69112*/str(dt_key$,19%,3%) = "   "                 /* Department   */
/*SR69112  str(dt_key$,22%,2%) = str(ad_rec$,28%,2%)      Process Code */
           read #2,key 0% > dt_key$, using L02210 , dt_rec$,                  ~
                                                eod goto calc_scanned_nxt
L02210:       FMT CH(256)                           /* Check Load No.  */
/*SR69112*/if str(dt_rec$,24%, 18%) <> str(ad_rec$,34%,18%) then              ~
                                       goto calc_scanned_nxt
...
/*SR69112*/if str(ad_rec$,30%,2%) <> p_shft$ then goto calc_scanned_nxt
/*SR69112*/goto CALC_RECEIVED
