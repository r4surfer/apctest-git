...
*       ****************************************************************~
*        APCCUTCC - Calculate ( Width and Height Cuts )       CT$()    *~
...
*                                                                      *~
*                                                                      *~
* 03/17/04 ! (EWD001) Modification for TSO/BSO cottage and oriel ! CMG *~
* 05/12/04 ! (EWD002) Modification Specified Cottage and Oriel   ! CMG *~
* 01/29/08 ! (AWD003) mod for 1/3 sliders                        ! CMG *~
* 04/03/08 ! (AWD004) mod for SDL glazing bead                   ! CMG *~
*07/22/2013! (AWD005) mod for new dimensions                     ! CMG *~
*       ****************************************************************

        sub "APCCUTCC" (partno$,         /* MFG Part Number           */ ~
                        dim1es,          /* (AWD005) dim1es           */ ~
                        dim2es,          /* (AWD005) dim2es           */ ~
                        dim3es,          /* (AWD005) dim3es           */ ~
...
/*EWD001*/  cot_or_phantom$25,           /* Phantom Designator        */ ~
        ...
        ~ 3/4 13/16 7/8 15/16     "
           ...
           mat ct = zer
           cot_or_tso_bso% = 0%                        /*  (EWD001)  */
           sdl% = 0%                              /* (AWD004) */
           if mfg% = 3% then sdl% = 1%            /* (AWD004) */
           if mfg% = 3% then mfg% = 0%            /* (AWD004) */
...
L01480:    gosub lookup_fin                /* (AWD003) */
           gosub lookup_slider             /* (AWD003) */
...
REM           gosub lookup_hinge             /* Check For Cottage/Oriel   */
           if c_o$ = "0" then goto L01630
              typ$ = c_o$                 /* Calc Cot/Or - Model, Color*/
              j% = cw% : cal% = 3%
              gosub calculate

L01630:    if triple% = 0% then goto exit_program
              typ$ = "5"                              /* 1/3, 1/3, 1/3 */
              j% = 0% : cal% = 2% : c_o$ = "X"
              gosub calculate
...
L01920:        
/* (AWD003) */
               if slider% = 0% then goto L01925
               if triple% = 0% then goto L01925
                  if typ$ <> "1" then goto L01925
                     if str(sa_rec$,32,2) <> "W" then goto calculate_next

L01925:
/* (AWD003) */
               e$ = str(sa_rec$,7%,2%)         /* Convert Equation No. */
...
L02270:       convert a% to str(calc$,1%,3%), pic(###)
        ...
        calc_cut
                  ...
                  call "APCCALSB" (cal%,           /* CALC TYPE 1%,2%,3*/~
                                   partno$,        /* PART NUMBER      */~
                                   dim1es,         /* Dim1es (AWD005)  */~
                                   dim2es,         /* Dim2es (AWD005)  */~
                                   dim3es,         /* Dim3es (AWD005)  */~
...
L02580: return
        ...
        lookup_slider
           ...
           read #3, key = readkey$, eod goto not_slider
                  slider% = 1%
