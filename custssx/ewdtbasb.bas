        REM THISPROGRAMWASGENERATEDUSINGTHEGENPGMPROGRAMWHICHISAPROPRIETR~
            *                                                           *~
            *  EEEEE  W   W  DDDD   TTTTT  BBBB    AAA    SSS   BBBB    *~
            *  E      W   W  D   D    T    B   B  A   A  S      B   B   *~
            *  EEEE   W W W  D   D    T    BBBB   AAAAA   SSS   BBBB    *~
            *  E      WW WW  D   D    T    B   B  A   A      S  B   B   *~
            *  EEEEE  W   W  DDDD     T    BBBB   A   A   SSS   BBBB    *~
            *                                                           *~
            *-----------------------------------------------------------*~
            * EWDTBASB - Prints Detail Trial Balance Report (aged).     *~
            *-----------------------------------------------------------*~
            ...
            *---WHEN---+----------------WHAT----------------------+-WHO-*~
            * 05/24/01 ! (EWD001) - Mod to allow 6 buckets for    ! CMG *~
            *          !            aging for Kathy P.  - ORIGINAL!     *~
            * 07/19/01 ! (EWD002) - Mod to add Atrium Division and! CMG *~
            *          !            terms on Report.      (EWD002)!     *~
            * 07/25/01 ! (EWD003)  Mod to add text from Atrium to ! CMG *~
            *          !           end of rpt.                    !     *~
            PRODUCTOFCAELUSASSOCIATESSPOKANEWASHINGTONALLRIGHTSRESERVED**

        sub "EWDTBASB"   (caller$,       /* Calling Program ID         */~
                          rptid$,        /* Report ID                  */~
...
*          account will be listed.
        ...
        dim                                                              ~
            ...
            bals(10), bals$(6)13,        /* Transaction Balance        */~
            ...
            division$4,                  /* Atrium Division Code EWD002*/~
            docdate$8,                   /* DOCUMENT DATE              */~
            duedate$8,                   /* Due Date for Report        */~
            errormsg$79,                 /* Error message              */~
            hdrs$(6)13,                  /* Report Column Headings     */~
            ...
            terms$20,                    /* Customer Terms       EWD002*/~
...
L09320:     call "COMPNAME" (12%, compname$, u3%)
            ...
            if types$ <> " " then str(types$,13%,6%) = "CPCACD"
            ...
            for b% = 1% to 6%                /* (EWD001) - Change from 5 to 6 */
...
L09670:                   % ###### -####
            if aging%(9) =  99999% then                                  ~
                     put hdrs$(6) using L09670, "  OVER", aging%(5)
                                                         /* (EWD001) */
REM                  put hdrs$(5) using L09670, "  OVER", aging%(4)
            amthdr$ = "TRANSACTION"
...
*                   EWDAGING now handles multicurrency. But the 2nd to
*                   last arg is 0% and this causes the sub to work in
*                   a NON-Multi-Currency mode. See EWDAGING for futher
...
*                   separately.
                     ...
                     call "EWDAGING" (str(plowkey$,,9), str(stlmnt$,,10),~
                                    asof$, ageper$, aging%(), #1, bals(),~
                                    #44, #9, 0%, currency$, cage$(),     ~
                                    acctie$, acctu$(1), acctu$(2), types$)
...
L10830:         billto$ = str(plowkey$,,9)
                ...
                if f1%(2) = 0% then billtoname$ = "(** NOT ON FILE **)"  ~
                                         ...
                                         terms$, division$      /*  (EWD002)  */
L10870:                                  FMT POS(433), CH(20), CH(10),   ~
                                             POS(543), CH(20), POS(1069),~
                                             CH(4)
                if phone$ <> " " then                                    ~
...
*                   separately.
            ...
            if min_bal$ = " " then                                       ~
                call "EWDAGING" (billto$, str(stlmnt$,,10),              ~
                                 asof$, ageper$, aging%(), #1, bals(),   ~
                                 #44, #9, 0%, currency$, cage$(),        ~
                                 acctie$, acctu$(1), acctu$(2), types$)

            aged = 0
            for b% = 1% to 6%         /* (EWD001) - Changed 5 to 6 */
...
*       * Handle Detail Option
          ...
          trans_loop
            ...
            if f1%(1) = 0% then report_loop
                ...
                if types$ = " " then L11190
                     search types$ = type$ to p%() step 2
                     if p%(1) = 0% then trans_loop
...
*       ****  SUBROUTINES  *********************************************
        ...
        print_billto                                    /*  (EWD002)  */
            print using L20450, billto$, billtoname$, division$, terms$
            line% = line% + 1%
...
L12170:         FMT XX(9), CH(12), XX(8), CH(6), XX(1), CH(6), CH(16),   ~
        ...
        print_trans       /* Print the transaction detail line         */
            ...
            print using L20470, stlmnt_prnt$, type_descrs$(1,type%),     ~
                               docdate$, duedate$, amt$,                 ~
                               bals$(1), bals$(2), bals$(3),             ~
                               bals$(4), bals$(5), bals$(6)
                                          /* (EWD001) - Add bals$(6)  */
...
L20360: %    SETTLEMENT D DOCUMENT            ########### -- - - - - - - ~
        ~- - - - - - - A G E D   O P E N   A / R  - - - - - - - - - - - - - -
L20390: %       NUMBER  T   DATE   DUE DATE     AMOUNT    ############# #~
        ~############ ############# ############# ############# #############
L20420: %      -------- - -------- -------- ------------- ------------- -~
        ~------------ ------------- ------------- ------------- -------------
                                                           /*  (EWD002)  */
...
L20620: %        PERCENT OF TOTAL AMOUNT DUE              ############# #~
        ~############ ############# ############# ############# #############
                                    /* (EWD001) - Changed last  10 image */
                                    /* statements so that will allow you */
                                    /* sixth column */
                                                        /*  (EWD003)    */
L20630: %################################################################~
        ~###################################################################
