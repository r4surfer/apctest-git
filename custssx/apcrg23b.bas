        REM *************************************************************~
            *                                                           *~
            *  Program Name      - APCRG23B  (Like APCRGA3B)            *~
            *  Creation Date     - 01/21/97                             *~
            *  Last Modified Date- 01/11/06 CMG                         *~
            *  Description       - This Program provides RGA Line Item  *~
            *                      Distribution for APC RGAII.          *~
            *                                                           *~
            *  Special Comments  - Subroutine of APCRGA23               *~
...
L30470:     return
        ...
        REM *************************************************************~
            ...
            *************************************************************
        dataput
            read   #2,hold,key = dtlkey$, eod goto L31540
            delete #2
...
/*PAR000*/           rga_dt_subp$,       /* RGA Item Subpart           */~
            ...
            write  #2, eod goto L31540
            gosub check_dt_status

        return
L31540:     errormsg$ = "(In Use) RGA Record In Use.  Try again later?"
        return
...
L40540:         close ws
        ...
        check_rga
            errormsg$ = " "
            convert dtlkey$ to dtlkey%, data goto L50140
...
L50370:     errormsg$ = "(Error) - Invalid STATUS Lookup?"
        ...
        next_dt_status
            read #2,key > dtlkey1$, using L50480, rga_dt1$, eod goto L50540
L50480:         FMT POS(10), CH(8)

            if str(rga_dt1$,3%,4%) <> str(dtlkey$,1%,4%) then L50540
            if str(rga_dt1$,1%,2%) < "13"                then L50560
            dtlkey1$               =  rga_dt1$
            goto next_dt_status
L50540:     gosub update_header

L50560: return

        update_header
            hdrkey$       = str(dtlkey$,1%,4%)
            read   #1,key = hdrkey$, using L50620, rga_hd_rec$,           ~
                eod goto L50740
L50620:         FMT CH(80)

            read   #1,hold,key = hdrkey$, using L50620, rga_hd_rec$,      ~
                eod goto L50740
            delete #1
            ...
            put    #1, using L50620, rga_hd_rec$
            write  #1, eod goto L50740
