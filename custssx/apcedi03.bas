        REM *************************************************************~
            *                                                           *~
            * AAAAAA  PPPP   CCCC  EEEE  DDDD  IIIIII   0000   333      *~
            * AA  AA  PP  P CC     EE    DD  D   II    0    0    33     *~
            * AAAAAA  PPPP  CC     EEEE  DD  D   II    0    0  333      *~
            * AA  AA  PP    CC     EE    DD  D   II    0    0    33     *~
            * AA  AA  PP     CCCC  EEEE  DDDD  IIIIII   0000   333      *~
            *                                                           *~
            *-----------------------------------------------------------*~
            * APCEDI03 - Subroutine To Create Invoice Edi Transmit File *~
            ...
            * 03/20/95 ! Mod To Seperate Freight Charges from the ! RHH *~
            *          !   Invoice. Put Charge in FRT$. Also      !     *~
            *          !   Deduct from Invoice Amount.            !     *~
            * 05/30/95 ! Mod To Line (2100) Leave the Freight Chg ! RHH *~
            *          !   in the Total Invoice Amount.           !     *~
            * 07/12/95 ! Mod to Send Sales Order Number with      ! RHH *~
            *          !   Invoice Header for Home Quarters Only. !     *~
            *          !   Using HOWSHIP$                         !     *~
            * 09/19/96 ! Mod To Add Disc Amt to Net Amt for HQ    ! JBF *~
            * 12/21/96 ! Mod To Convert Disc Amt from Whole Number! JBF *~
            * 06/25/97 ! Mod to allow for other trading Partners  ! RHH *~
            *          !   Create Invoice Data for EDI Transmit   !     *~
            *          !   other than Kleinsmidth.                !     *~
            *          !                                          !     *~
            * 11/18/97 ! Revision For 60403                       ! DJD *~
            * 05/29/98 ! Fixed problem with due dates for Lowes   ! ERN *~
            * 07/27/98 ! (EWD001) - Calc the line item Disc %     ! RHH *~
            *          !   mod made at L03390. linedisc           !     *~
            * 08/27/98 ! (EWD002) Fix Ship Date                   ! RHH *~  
            * 09/08/99 ! (EWD003) - Fix Printing of Sku Number for! RHH *~
            *          !   Buyout products. Two different Products!     *~
            *          !   have the same SKU Number. Need to      !     *~
            *          !   by UPC code. Cross-ref table PLANLABSP !     *~
            *          !   lookup in table with UPC Code and get  !     *~
            *          !   Color Code and Assoc. SKU Number       !     *~ 
            * 06/18/01 ! (EWD004) - Mod to put '99999' in buy_in$ ! CMG *~
            *          !   if no valid sku number found.  Lowes   !     *~
            *          !   requested it to be that way.           !     *~
            * 07/10/01 ! (EWD005) - Mod so that routine will not  ! CMG *~
            *          !   calculate due dates, it will take what !     *~
            *          !   is passed to subroutine.               !     *~
            * 07/30/01 ! (EWD006) - Mod for LO0020 and LO0378 so  ! CMG *~
            *          !   that EDI will bill LO0020 to LO0378.   !     *~
            *-----------------------------------------------------------*

        sub "APCEDI03" (#1,                /* ARIBUFFR File - Headers */ ~
...
/* <<<<<<<<<< Y2K >>>>>>>>>> */
        ...
        validate_edi_id                /* Get EDI Id and Customer Data */
           ...
           if edi_cust1$ = "LO0020" then edi_cust1$ = "LO0378"
...
L01750:            FMT POS(3), CH(4), POS(18), CH(16), CH(16), POS(413), ~
                       ...
                       POS(908), CH(20)
                                                  ...
                                                  /* (EWD006)    */
              if str_id$ = "0020" then str_id$ = "0378"
              init(" ") xx$
              xx$ = ship_date$
...
*       JBF - Special Mod to EDI Invoice Amount for HQ - 9/19/96
        ...
        REM - Mod 06/25/97. Otherwise,Howship from Sales Order(PLAN HOWS)
              if edi_id$ = "001" then howship$ = str(so_no$,1%,9%)
...
get_lowes_stock_sku
        init(" ") low_key$
	str(low_key$,1,25) = ven_in$ 
        read #10,key 2 > low_key$, using awdskuxr, low_sku$, low_key$, ~
                                        eod goto not_stock
	if str(low_key$,1,25) <> ven_in$ then goto not_stock 
awdskuxr: FMT POS(05), CH(12), POS(37), CH(45) 
        buy_in$ = low_sku$
