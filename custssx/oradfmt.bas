        ...
        dim jobid$10,                    /* Job ID                     */~
            ...
            hrs$2,                       /* Hours                      */~
            min$2,                       /* Minutes                    */~            
            dfhrs$2,                     /* DF Total Hrs               */~
            dfmin$2,                     /* DF Total Min               */~
            tothrs$2,                    /* Total Hrs                  */~
            totmin$2,                    /* Total Min                  */~
...
REM          init(" ") shftDiff$
REM          colval, shifDiff = 0.00
REM          table$ = "EMPSHFDIF"
REM          genkey$ = division$ & "0" & str(fields$(10%),1%,1%)
REM          gosub genRead
...
writeUpdtRec:        
        ...
        str(rec$,28%,03%)  = "DAF"          /* User ID       */
        str(rec$,31%,10%)  = mt_pay_code$   /* DF Pay Code   */
        str(rec$,41%,10%)  = fields$(07%)   /* DF Paid       */
        str(rec$,42%,10%)  = jobid$         /* DF JOBID      */
        str(rec$,52%,20%)  = " "            /* DF EMPID      */
        str(rec$,72%,10%)  = str(fields$(02%),15%,10%)   /*DF Dept*/
        str(rec$,82%,06%)  = appliedDte$    /* DF Appl Dte   */
...
REM ===================================
        ...
        read #3, hold, key = key$, using DFEMPMT_FMT, rec$,        ~
                                                  eod goto addDFRec
            delete #3
...
addDFRec:
        ...
        str(rec$,33%,02%)   = dfhrs$         /* HRS           */
        str(rec$,35%,02%)   = dfmin$         /* MINS          */
        str(rec$,37%,03%)   = "DAF"          /* User ID       */
        str(rec$,40%,10%)   = fields$(07%)   /* DF Paid       */
        str(rec$,50%,10%)   = jobid$         /* DF JOB ID     */
        str(rec$,60%,20%)   = " "            /* DF EMPID      */
        str(rec$,80%,10%)   = str(fields$(02%),15%,10%) /*DF Dept*/
        str(rec$,90%,06%)   = appliedDte$    /* DF Applied Dt */
        str(rec$,96%,08%)   = fields$(09%)   /* DF Pay Rate   */
        str(rec$,104%,02%)   = fields$(10%)   /* DF Shft Diff */
        str(rec$,106%,08%)   = shftDiff$      /* DF SftDiffR  */
...
REM ===================================
        ...
        put #3, using DFEMPMT_FMT, rec$
        write #3
            
DFEMPMT_FMT:  FMT CH(128)
