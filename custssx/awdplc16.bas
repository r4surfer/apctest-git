        ...
        REM *************************************************************~
            *                                                           *~
            *  Subroutine Name   - AWDPLC16                             *~
            *  Creation Date     - 06/09/05                             *~
            *  Last Modified Date- 08/26/05                             *~
            ...
            *---WHEN---+----------------WHAT----------------------+-WHO-*~
            * 06/09/05 ! Original -                               ! CMG *~
            * 08/26/05 ! (AWD001) - Mod for Comment Record        ! CMG *~
            *11/17/2009! (AWD002) - ultra batches                 ! CMG *~
            *03/17/2011! (AWD004) mod for multiple intercepts     ! CMG *~
            *************************************************************

        sub "AWDPLC16" (#1, #2, pre%, schema%)  
                                         /* Return Code                */
        ...
        dim gls_read$(2%)192,            /* GLS GED Read key           */~
            ...
            strength$2,                  /* Glass Strength             */~
            sav_strength$2               /* Save Glass Strength        */
...
L00100:          FMT CH(24), CH(30)
                 ...
                 if str(readkey$,1%,9%) <> "GED SCHED"                ~
                                        ...
                                        /* Only Get DS Files */
                 if str(readkey$,10%,2%) <> "DS" then goto sort_nxt
                 cnt% = cnt% + 1%
            ...
            read #2, key > readkey$, using L00100, readkey$, desc$,   ~
                 ...
                 if str(readkey$,1%,9%) <> str(savekey$,1%,9%)        ~
                                             then goto sort_files_done
                                                      ...
                                                      /*  Only Use 3 Files */
                 if str(readkey$,10%,1%) <> "3" then goto file_nxt
...
REM               gosub get_tt_max
        ...
        read_glsged
              ...
              init(" ") gls_read$(), sav_read$(), prv_read$(), tb_sort1$(),  ~
                        tb_sort2$(), sort$, sav_sort$, strength$, sav_strength$,~
                        gls_hdr$(), intercept$
              dept%, sav_dept%, tb_cnt%, rec% = 0%
...
REM                      if str(gls_read$(),250%,3%) = "SUN" then goto gls_nxt
                      ...
                      init(" ") sort$               /* End of File  */
                      init(" ") strength$
REM this is used for when V, H and COMMENT records are being written
                      if str(gls_read$(),2%,3%) = "COM" and                  ~
                          sav_strength$ = " " then                           ~
                                      sav_strength$ = str(gls_read$(),21%,2%)
REM                      if str(gls_read$(),242%,2%) = "SS" then             ~
                                                  init(" ") sav_strength$
                                                             /* (AWD001)  */
                      if str(gls_read$(),2%,3%) = "COM" and                  ~
                             str(gls_read$(),21%,2%) <> "DS" then            ~
                                                  init(" ") sav_strength$
...
L00510:
                      if str(gls_read$(),2%,1%) <> "H" and                   ~
                         str(gls_read$(),2%,1%) <> "V" then goto L00520
                              if sav_strength$ <> "DS" then goto gls_nxt

                              dept% = sav_dept%
                              sort$ = sav_sort$
                              strength$ = sav_strength$
                              goto L00540
                                                          /* (AWD001) - BEG */
L00520:                if str(gls_read$(),2%,3%) <> "COM" then goto L00550
                              if sav_strength$ <> "DS" then goto gls_nxt
...
L00550:
                                                          /* (AWD001) - END */
                      REM Only get double strength                               
REM                      if str(gls_read$(),242%,2%) <> "DS" then goto gls_nxt
                      if str(gls_read$(),21%,2%) <> "DS" then goto gls_nxt
L00540:
REM                      if dept% =  0% then                                    ~
...
REM                      if strength$ = " " then strength$ = str(gls_read$(),242%,2%)
                                                               /* (AWD001) */
                      if strength$ = " " then strength$ = str(gls_read$(),21%,2%)
                      sav_strength$ =  strength$

REM                      if sort$ = "TB" then gosub tb_sort
                      gosub tb_sort
        ...
        tb_sort
                             /* Loads file into array for memory processing */
...
tb_cont:
            ...
            tb_cnt% = tb_cnt% + 1%   
            if tb_cnt% >= 32000% then call "SHOSTAT" (" Max Counter!! ")
...
LOAD_ITEM:
        ...
        write_tb
                                  /* tb_cnt% is number of records read in to process */
             if tb_cnt% = 0% then return 
                for tb% = 1% to tb_cnt%
                    if tb_sort1$(tb%) = " " then goto L10000   /* Beginning of Batch */
...
REM                         if tb_counter% = 0% then gosub open_new_file
                       ...
                       if str(tb_sort1$(tb%+1%),2%,1%) = "H" or              ~
                             str(tb_sort1$(tb%+1%),2%,1%) = "V" then         ~
                             num_to_add% = 4%
...
REM            str(gls_sort$,1%,6%) = str(tb_sort2$(tb%+1%),106%,6%)

                                                /* (AWD001)  */
            str(gls_sort$,1%,11%) = str(tb_sort1$(tb%+2%),21%,6%)

                           /* sort will be DSSPXX for all depts */
                           /* except 047; but 047 is within SP11*/
REM            if str(tb_sort2$(tb%+1%),103%,3%) = "047" then          ~
                 str(gls_sort$,7%,3%) = str(tb_sort2$(tb%+1%),103%,3%)
REM            if str(tb_sort2$(tb%+1%),108%,4%) = "SP11" then         ~
                            str(gls_sort$,7%,3%) = " "
                                                /* (AWD001)  */
            if str(tb_sort1$(tb%+2%),18%,3%) = "047" then          ~
                 str(gls_sort$,7%,3%) = str(tb_sort1$(tb%+2%),18%,3%)
            if str(tb_sort1$(tb%+2%),23%,4%) = "SP11" then         ~
                            str(gls_sort$,7%,3%) = " "
                            
            if str(tb_sort1$(tb%+2%),18%,3%) = "007" then          ~
                 str(gls_sort$,7%,3%) = "007"
            if str(tb_sort1$(tb%+2%),18%,3%) = "005" then          ~
                 str(gls_sort$,7%,3%) = "007"
            if str(tb_sort1$(tb%+2%),18%,3%) = "049" then          ~
                 str(gls_sort$,7%,3%) = "007"                 
            if str(tb_sort1$(tb%+2%),18%,3%) = "002" then          ~
                 str(gls_sort$,7%,3%) = "007"                       


	          if str(tb_sort1$(tb%+2%),18%,3%) = "014" then          ~
                 str(gls_sort$,7%,3%) = "007"
            if str(tb_sort1$(tb%+2%),18%,3%) = "017" then          ~
                 str(gls_sort$,7%,3%) = "007"
            if str(tb_sort1$(tb%+2%),18%,3%) = "018" then          ~
                 str(gls_sort$,7%,3%) = "007"                 
        

	          if str(tb_sort1$(tb%+2%),18%,3%) = "019" then          ~
                 str(gls_sort$,7%,3%) = "027"
	          if str(tb_sort1$(tb%+2%),18%,3%) = "027" then          ~
                 str(gls_sort$,7%,3%) = "027"                       
      	    if str(tb_sort1$(tb%+2%),18%,3%) = "028" then          ~
                 str(gls_sort$,7%,3%) = "027"                       
       	    if str(tb_sort1$(tb%+2%),18%,3%) = "036" then          ~
                 str(gls_sort$,7%,3%) = "027"                       
	          if str(tb_sort1$(tb%+2%),18%,3%) = "048" then          ~
                 str(gls_sort$,7%,3%) = "027"                       
           ...
           if str(sort1$,1%,3%)  = "048" then                      ~
            ...
            ff% = file_max% + 2%              /* (AWD001) Max Num Of files */
            fx% = ff%                          /* (AWD001)                 */
         return
