        REM *************************************************************~
            *                                                           *~
            *  IIIII  N   N  V   V  U   U  DDDD    SSS   PPPP   FFFFF   *~
            *    I    NN  N  V   V  U   U  D   D  S      P   P  F       *~
            *    I    N N N  V   V  U   U  D   D   SSS   PPPP   FFFF    *~
            *    I    N  NN   V V   U   U  D   D      S  P      F       *~
            *  IIIII  N   N    Y     UUU   DDDD    SSS   P      F       *~
            *                                                           *~
            *-----------------------------------------------------------*~
            * INVUDSPF - Display HNYQUAN Info for Part Specified.       *~
            *-----------------------------------------------------------*~
        ...
        sub "INVUDSPF"   (part$,         /* Part Number to Display     */~
                          #1,            /* INVMASTR Channel           */~
                          #2,            /* INVHSTRF Channel           */~
                          #3,            /* STORNAME Channel           */~
                          #4 )           /* INVDETAL Channel           */
...
*        back to the caller.  Also the Caller must open the files.
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim cms2v$50
            cms2v$ = "REV:01.00 03/03/06 For New Part Number            "
        REM *************************************************************
...
L09000: REM *************************************************************~
            ...
            *************************************************************
            ...
            str(line2$,62) = "INVYUDSPF: " & str(cms2v$,,8)
...
L09390:              FMT CH(45), CH(32)
            ...
            if f1%(2) = 1% then L09510
                u3% = 2%
                call "ASKUSER" (u3%, "INVHSTRF DISPLAY",                 ~
...
L09520:     str$, str1$ = str(plowkey$,46%,3%)
            convert val( str(plowkey$,49%,4%), 4%) to year$, pic(000000)
...
*        RETURN indicates to show detail for month where cursor is at
            ...
            if mm% < 1% then main_screen
               if mm% > periods% then main_screen
                if dates$(mm%) = " " or dates$(mm%) = blankdate$ ~
                  then main_screen
                                                       /* (PAR000)     */
                call "INVDDISP" (part$, "D", dates$(mm%), dates2$(mm%),  ~
                                 str$, str$, "ALL", "   ", " ", #1, #4)
                goto main_screen

        first_year                                     /* (PAR000)     */
            plowkey$ = part$ & str(str$) & all(hex(00))
            call "PLOWNEXT" (#2, plowkey$, 48%, f1%(2))
        ...
        prev_year
            plowkey$ = part$ & str(str$) & all(hex(00))
            first%   = 1%
L10390:     call "PLOWNEXT" (#2, plowkey$, 48%, f1%(2%))
            if f1%(2%) = 0% then L10460
                yyyy% = val( str( plowkey$, 49%, 4%), 4%)
...
L10490:     year$ = yy1$
            gosub load_screen
            goto  main_screen                          /* (PAR000)  */

        next_year
            convert year$ to yyyy%                     /* (PAR000)  */
            plowkey$ = part$ & str(str$) & bin(yyyy%,4%)
            call "PLOWNEXT" (#2, plowkey$, 48%, f1%(2%))
            if f1%(2%) = 1% then L10600
                errormsg$ = "Already displaying last year on file."
                goto main_screen
L10600:     convert val( str(plowkey$,49%,4%), 4%) to year$, pic(000000)
*           year$ = str( year$, 1%, 4% )
            gosub load_screen
            goto  main_screen                          /* (PAR000)  */
...
L10680:     gosub'101
        ...
        load_screen
            ...
            convert year$ to yyyy%                         /* (PAR000)  */
            readkey$ = part$ & str(str$) & bin(yyyy%,4%)
...
L30160:         FMT XX(52), 52*PD(14,4)                    /* (PAR000)  */
...
*        Now see if there is anything on file for this store.
            convert year$ to yyyy%                         /* (PAR000)  */
            plowkey$ = part$ & str(str1$) & bin(yyyy%,4%)
            call "READ100" (#2, plowkey$, f1%(2%))
            if f1%(2%) = 1% then L50260
                plowkey$ = part$ & str(str1$) & all(hex(00))
                call "PLOWNEXT" (#2, plowkey$, 48%, f1%(2%))
                if f1%(2%) = 1% then L50260
                     errormsg$ = "No data on file for this store."
                     return
L50260:     convert val( str(plowkey$,49%,4%), 4%) to year$, pic(000000)
            str$ = str1$
            gosub load_screen
            return                                         /* (PAR000)  */
