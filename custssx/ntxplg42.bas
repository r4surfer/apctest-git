...
*                                                                        *~
*                                                                        *~
*        NTXPLG42 - Create File with Data for Saw Optimization           *~
*                   with Weld information also. Generic Tiger Stop       *~
*                                                                        *~
...
*05/15/2018  ! (CR1482) create for Dept 031 NTXLineal              ! MES *~
**************************************************************************

        sub "NTXPLG42" (size%,           /* Batch Size (No. Windows)  */ ~
...
/* (AWD002) */
        ...
        dim f2%(10%),                    /* = 0 if the file is open    */~
           ...
           dept% = 0%
            convert scr_dept$ to dept%, data goto tiger_dept
...
tiger_dept:
            ...
            ff% = 3% : ff_nam$ = "@SAW031@"     /* DEPT 031 */
            
            if dept% <> 41% then goto NotDept041
                ff% = 7% : ff_nam$ = "@SAW041@"
                
NotDept041:
            if dept% <> 70% then goto NotDept070
                ff% = 9% : ff_nam$ = "@SAW070@"
...
L01410:
        REM - Adjust Schedule
                                             /*    *** IMPORTANT ***   */
            scr_prod$ = "3"                  /* For Casement Series    */
                                             /*    *** IMPORTANT ***   */
...
L02320:   errormsg$ = "(Error) When Writing Schedule Quantity"
        ...
        build_detail_saw
          ...
          str(bat_rec$,38%,15%) = sa_die_matl$(sa%)     /* Part No.    */
...
REM                  if i% >  cw% then goto L03260    /* Width  Eq        */
REM                  str(sa_part$(sa%),11%,1%) = "1" /*CR1482 removed     */
            ...
            if str(desc$, p%+2%, 4%) <> "TWIN" and                       ~
               str(desc$, p%+2%, 4%) <> "TRPL" then goto L03830
                  ...
                  if str(desc$,p%+2%,4%) = "TRPL" then                   ~
                     str(co$, cnt%+1%,3%) = "/TR"
               cnt% = cnt% + 3%
L03830:     if str(dtl_part$,9%,2%) <> "09" then goto L03890
