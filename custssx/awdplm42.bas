...
*                  (PAR001)  05/23/2006  North East  Dept (006)        *~
*                            Create Frame's for new Sub Part Info      *~
*                            no Sash's                                 *~
*                                                                      *~
*        *Special Size Test for Window Height of Sash's on the Sampson *~
*                 Sash Welder. Don't pass when Window Height less than *~
*                 28.5. Sampson Sash Welder Limitation.                *~
*                                                                      *~
*        (Below Test is Commented out)                                 *~
*        *Special Size Test for set_up$ = '1' for saws                 *~
*                 Add 13/16 or .8125 to Width or Height.               *~
*                 Saw adjustment. (From APCPLL42)                      *~
*                                                                      *~
*                                                                      *~
*                  New Flex Line Processing for 560 Line               *~
*                  ( As of 05/23/2006 - RHH  Dept 006 )                *~
*                                                                      *~
*        AWDPLM42 - Create File with Data for Saw Optimization         *~
*                   with Weld information also.                        *~
*                                                                      *~
*         Subs Replaced - New Department                               *~
*                                                                      *~
*        (PAR001)                                                      *~
*            File Layout                                               *~
*                 FR930318                                             *~
...
*                                                                      *~
*                                                                      *~
*05/21/2012! (AWD001) mod for additional part data                 !CMG*~
*03/19/2013! (AWD002) mod to awdcutcc addl data                    !CMG*~
*08/11/2017! (CR1002)  - new top bottom parameter                  !RDB*~
************************************************************************

        sub "AWDPLM42" (size%,           /* Batch Size (No. Windows)  */ ~
        ...
        dim readkey$24,                  /* GENCODES Primary Key      */ ~
            ...
            sub_scr$120,                 /* Screen Text SubPart(PAR001)*/ ~
            sub_prt$60,                  /* Print Text SubPart(PAR001)*/ ~
            apc_sze$20,                  /* Size                      */ ~
            ...
            dtl_sub_part$20,             /* New sub Part No   (PAR001)*/ ~
            dtl_sub_info$9,              /* New Sub Info Fields(PAR001)*/~
            dtl_new_part$45,             /* New Part Number   (PAR001)*/ ~
            casing$1,                    /* New N.E. Product  (PAR001)*/ ~
            save_part$25,                /* MFG Part Number Weld      */ ~
            save_part_s$25,              /* MFG Part Number Saw       */ ~
            save_new_part$45,            /* MFG Part NO Weld  (PAR001)*/ ~
            save_new_part_s$45,          /* MFG Part NO Saw   (PAR001)*/ ~
            ...
            cr_addl$(500%)5,             /* (AWD001) addl raw material*/ ~
/*AWD002*/  s_f$(500%)1,                 /* Sash / Frame               */~
/*AWD002*/  die$(500%)15,                /* Die Number                 */~
/*AWD002*/  adj(500%),                   /* Adjustment amt             */~            
            cp$(500%)2,                  /* Number of Pieces to Cut   */ ~
            cc$(500%)1,                  /* Cut Piece Yes or No       */ ~
            tw$1,                        /* WIDTH CUT PARTS           */ ~
            th$1,                        /* HEIGHT CUT PARTS          */ ~
            tsw$1,                       /* Width Type Sub Part(PAR001)*/~
            tsh$1,                       /* Heigth Typ Sub Part(PAR001)*/~
...
L01410:
        REM - Adjust Schedule
            scr_prod$ = "5"                  /* For 560 Series         */
            hit% = 0%                        /* Set to (1) When Header */
            bat_no% = 0% : count% = 0%       /* is Built               */
            bat$ = "00"

            init(" ") save_part$, save_part_s$, save_new_part$, save_new_part_s$
                                             /* Starting Schedule      */
...
L01630:     dt_ref$   = str(wrk_rec$,1%,8%)   /* Part Reference Number*/
            ...
            dtl_sub_info$ = str(wrk_rec$,101%,9%)
...
CALC_CUT1:
            convert casing% to casing$, pic(#)
                                                   ...
                                                   /* are not Casing or  */
                                                   /* BullNose           */
            if casing$ = "0" then goto create_next
            ...
            if count% = 0% then gosub build_schedule   /* CREATE BATCH */
               ...
               gosub build_saw_recs
                   ...
                   next sa%
                                              ...
                                              /* (PAR001)              */
                  goto CALC_CUT2              /* NO Sash Records       */
        REM   *************************************************************
...
L02750:   errormsg$ = "(Error) Building End Record??"
        ...
        build_saw_recs
                                                   /* (PAR001)       */
          if save_new_part_s$ <> dtl_new_part$ then goto L02850
             for i% = 1% to sa_max%
...
L02850:   save_new_part_s$ = dtl_new_part$         /* NEWFAMILY Info */
          init(" ") sa_type$(), sa_piece$(), sa_cut$(), sa_part$(),      ~
           ...
           call "AWDDESCR" (dtl_part$, dtl_sub_part$, apc_scr$, apc_prt$,~
                                 sub_scr$, sub_prt$, apc_sze$, #6, err% )

          sa_d2$ = str(apc_prt$,1%,30%)

          if casing$ = "1" then str(sa_d2$,1%,4%) = "CASE"
          if casing$ = "2" then str(sa_d2$,1%,4%) = "BULL"

          sa% = 0%
                                                         /* (AWD001) */
                                                         /* (AWD002) */
          init(" ") tb_w$()   /* CR1002 */ 
          call "AWDCUTCC" ( dtl_new_part$, 0%, cw%, ch%, csw%, csh%,   ~
                             eq$(), ct$(), cr$(), cr_addl$(), cp$(),   ~
                             cc$(), col$(), ct(), sh$(), tw$, th$,     ~
        ...
        REM *RHH*                               /* REMOVE 'LA' RECORDS */
            ...
            jj% = pos("KLMN" = str(eq$(i%),6%,1%))
            if jj% = 0% then goto L03430
                                               /* (PAR001)             */
        ...
        REM          convert ct(i%) to str(sa_cut$(sa%),1%,7%), pic(###.###)
               ...
               if cut_type% <> 2% then goto L03260
                  ...
                  if i% <= cw% then goto L03260    /* Height Eq        */
                     str(sa_part$(sa%),11%,1%) = "0"   /* Jamb Material*/
                     if c_o$ = "CO" or c_o$ = "OR" then                 ~
...
L04500: end
       ...
       build_weld                               /* Find Weld Records F/S */
                                                ...
                                                /* (PAR001)              */
                                                /* Dept - No Sash        */
          if dtl_new_part$ = save_new_part$ then return
             save_new_part$ = dtl_new_part$
...
not_casing
           if casing$ <> "2" then goto not_bullnose
              tsw$ = "M" : tsh$ = "N"
not_bullnose
           call "AWDCUTLD" (scr_prod$, cw%, ch%, csw%, csh%, tw$, th$, tsw$, tsh$,~
                             #2, err% )
        return
