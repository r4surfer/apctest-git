        ...
        REM *************************************************************
            ...
            void_flag$ = "N"
            if type$ <> "PV" then L01957
            void_flag$ = "Y"
            type$ = "PO"

L01957:     if been_here_before% = 1% then L02000
                been_here_before% = 1%
...
*                              " ", #3, F3%, RETURNCODE1%)
        ...
        if void_flag$ = "Y" then goto post_void
...
post_void:
            call "GLPRTSUB" (glmod$, jnlid$, pstseq%, userid$, date$,    ~
                             cacct$, gltext$, adjust, 0, #10, f10%)


            str(gltext$, 67, 1) = "H"

            call "GLPRTSUB" (glmod$, jnlid$, pstseq%, userid$, date$,    ~
                             aacct$, gltext$, 0, adjust, #10, f10%)
