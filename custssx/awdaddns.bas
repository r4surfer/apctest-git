        REM *************************************************************~
            *                                                           *~
            *   AAA   W   W  DDDD    AAA   DDDD   DDDD   N   N   SSS    *~
            *  A   A  W   W  D   D  A   A  D   D  D   D  NN  N  S       *~
            *  AAAAA  W W W  D   D  AAAAA  D   D  D   D  N N N   SSS    *~
            *  A   A  WW WW  D   D  A   A  D   D  D   D  N  NN      S   *~
            *  A   A  W   W  DDDD   A   A  DDDD   DDDD   N   N   SSS    *~
            *                                                           *~
            *-----------------------------------------------------------*~
            * AWDADDNS - Enter additions to inventory and write to the  *~
            *            additions buffer.                              *~
            *-----------------------------------------------------------*~
        ...
        sub "AWDADDNS" (part$, store$, qty$, serialnumber$, userid$,     ~ 
                        err%,                                            ~
                         #7,                    /* SYSFILE2           */ ~
                         #6,                    /* STORENAME          */ ~
                         #1,                    /* USERINFO           */ ~
       ...
       dim part$25, store$3, qty$10, serialnumber$8
        ...
        dim                                                              ~
            ...
            hnydte$8,                    /* THIS USERS INVENTORY DATE  */~
...
*        Retrieve Inventory Date
         if sysdates% = 0% then goto L09350
...
*        Set TRAN$ for Edit Mode Field Number Computation.
            ...
            store$(1%) = store$
            qty = 0.00
            convert qty$ to qty, data goto exit_program

            convert qty to qty$(1), pic(-######0.00)
            qty$(1%)   = qty$
            descr$(1%) = serialnumber$
            index%(1%) = 1%
            seq$(1%)   = "  1"
            c%, index%, currentline%, maxlines%, temp%, index% = 1%
            init(" ") errormsg$
...
REM                call "HNYCDIST" (mode$, part$(c%), partdescr$,           ~
REM                     "HNYADDNS Inventory Additions Cost Distribution", ~
REM                     #7, cost$(c%), totlcost$(c%), totlcost)
...
L51150:             FMT XX(44), CH(6)
        ...
        exit_program
            ...
            err% = 0%
            if errormsg$ <> " " then err% = 2%
            if err% <> 0% then end
REM            call "PUTPARM" addr("E",/* Enter or Display                     */~
                                "JNLID   ",     /* PRNAME                   */~
                                1%,             /* Nbr of Fields this PRNAME*/~
            ...
            rlib$, rvol$ = " " 
            comp% = 0%
REM            call "PROCLINK" ("AWDADDJN", rlib$, rvol$, return%, comp%)
            call "AWDADDJN"  (userid$, comp%)
            if comp% <> 0% then err% = 2%
