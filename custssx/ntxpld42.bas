...
*             (PAR000) 01/15/06 CR347            NTX Dept (016)        *~
*             Models (N54)    copy of dept 053 NC  AWDPLFU2            *~
*																	   *~	                    
...
* 02/10/2015! (AWD002) mod for dept 016                          ! MES *~
* 05/17/2017!(SR80554) mod for Oriel Op shape jambs              ! MES *~    
* 08/11/2017! (CR1002)  - new top bottom parameter               ! RDB *~
* 11/13/2017! (CR1193) - Specified CLMR no holes drilles         ! MES *~
* 02/22/2022! CR3020 - Add NEWFAMIL2 and NEWFLEXW2 for model 511 ! RDB *~
************************************************************************

        sub "NTXPLD42" (size%,           /* Batch Size (No. Windows)  */ ~
        ...
        dim readkey$24,                  /* GENCODES Primary Key      */ ~
            ...
            jpart$10,                    /* (AWD002) Sturtz cluster   */ ~
            jmodel$10,                   /* (AWD002) Joseph Model NEWFAMILY */ ~
            tb_w$(500%)1,                /* T/B cut (CR1002)          */  ~
            clmr$3                       /* (CR1193) Specified CLMR   */
...
/*(AWD002)*/            
            ...
            clmr$     = str(dtl_part$,20,3)   /* (CR1193) Spec CLMR   */
...
REM          nosash$ = str(model$, 2%, 2%)                 /* (AWD003)   */
          if j% = 2% and model$ = "743"  ~
          		then goto L02635  						/* (AWD003)   */ 
          if j% = 2% and model$ = "759"  ~
          		then goto L02635  						/* (AWD003)   */ 
...
/*(AWD002) non std changes to frame jambs*/
  			if cut_type% <> 2% then goto NoFab  /* Only Frames (AWD002) */
                 	sh%, slider%, opshape% = 0%
                	 if model$ = "N54" then sh% = 1%         /*(AWD002*/
                    if model$ = "754" then sh% = 1%         /*(AWD002*/
                 	if model$ = "120"  then slider% = 2%     /*(AWD002)*/
                 	if model$ = "130" then slider% = 3%    /*(AWD002)*/
                    if str(model$,3%,1%) = "O" then opshape% = 1% /*SR80554*/
                                                       /* Skip Meeting Rail */
                 
REM                  	if eqNum$ = "33" and slider% = 0%  then goto NoFab   
                 	if i% > cw% and eqNum$ = "01" and slider% <> 0%  ~
                         then goto NoFab  /*slider jambs*/
                    if opshape% = 1% then goto NoFab        /*SR80554*/
                  	p% = 0%
                 	 p% = pos(sa_part$(sa%) = " ")
...
REM    Begin Slider H&S non std (AWD002)
		sliderhs
		 if i% > cw% then return
		 if eqNum$ <> "01" then return   /*jambs no fabrication change*/
		 if slider% = 2% and hng$ = "07" then jpart$ = "12025" 
		 if slider% = 3% and hng$ = "09" then jpart$ = "12006" 
...
REM Begin SH Jamb non std (AWD002) Not needed at this point for NTX
		shjamb
		 if i% <= cw% then return
REM		 if eqNum$ <> "31" then return /*
		 if c_o$ = "OR" and clmr$ = "000" then jpart$ = "N5404"
		 if c_o$ = "CO" and clmr$ = "000" then jpart$ = "N5403"
         if c_o$ <> "  " and clmr$ <> "000" then jpart$ = "N5422" /*(CR1193)*/
...
L03940: return
        ...
        check_cut                                    /* 1st Frame      */
            ...
            if dept% = 16% then str(readkey$,1%,9%)  = "NEWFAMILY" /*(AWD002)*/  
REM         if dept% = 59% then str(readkey$,1%,9%)  = "NEWFAMIL2" /*(AWD002)*/
            if dept% = 16% and model$ = "511" then ~
                 str(readkey$,1%,9%)  = "NEWFAMIL2" /* CR2030 */
...
L04260: return
        ...
        check_welder                                 /* 1st Frame      */
            ...
            if dept% = 16% then str(readkey$,1%,9%)  = "NEWFLEXWD" /*(AWD002)*/  
REM         if dept% = 59% then str(readkey$,1%,9%)  = "NEWFLEXW2" /*(AWD002)*/
            if dept% = 16% and model$ = "511" then ~
                 str(readkey$,1%,9%)  = "NEWFLEXW2" /* CR2030 */
