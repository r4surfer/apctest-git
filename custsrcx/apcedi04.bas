        REM *-----------------------------------------------------------*~
            *                                                           *~
            * AAAAAA  PPPP   CCCC  EEEE  DDDD  IIIIII   0000   44 44    *~
            * AA  AA  PP  P CC     EE    DD  D   II    0    0  44 44    *~
            * AAAAAA  PPPP  CC     EEEE  DD  D   II    0    0  4444444  *~
            * AA  AA  PP    CC     EE    DD  D   II    0    0     44    *~
            * AA  AA  PP     CCCC  EEEE  DDDD  IIIIII   0000      44    *~
            *-----------------------------------------------------------*~
            * APCEDI04 - Create Flat File To Send To Kleinschmidt.      *~
            *-----------------------------------------------------------*~
            *                  M O D I F I C A T I O N S                *~
            *---WHEN---+----------------WHAT----------------------+-WHO-*~
            * 12/01/93 ! Original                                 ! VRW *~
            * 06/25/97 ! Mods to support Various EDI Vendors,those! VRW *~
            *          !   not going thru Kleinschmidt. They may  ! VRW *~
            *          !   have their own EDI Software. New Routin! VRW *~
            *          !   called 'CHECK_PROCESS'                 ! VRW *~
            * 05/05/98 ! Y2K changes (a bit late)                 ! ERN *~
            *          !   Outgoing dates cannot be packed...     !     *~
            * 06/04/01 ! (EWD001) Mod to create new file for the  ! CMG *~
            *          !   IPNet Solution.                        !     *~
            *-----------------------------------------------------------*
        ...
        dim apcmc_key$22,                /* ARIEDIMC File Key          */~
            ...
            detail_send_unpk$226,        /*   with dates unpacked           Y2K */~ 
            ...
            trans_date$8,                /* Transmit date                   Y2K */~
            type$2,                      /* Record Type                */~
            work_date1$10, work_date2$10 /* Temp dates                      Y2K */    
        ...
        REM *************************************************************~
            ...
            * #4  ! EWDIPNSN ! EWD IPNet Send Flat File           EWD001*~
            *************************************************************~
            ...
            *************************************************************
            ...
            call "SETPRNT" ("LOWE", "LOWE", 0%, 0%)
            select printer (134)

            edi_sn% = 0%             /* (EWD001) - Easy way to turn-off edi */

            call "SHOSTAT" ("Opening EDI Files...")
            call "OPENCHCK" (#1, fs%(1%), f2%(1%), 1000%, rslt$(1%))

            if edi_sn% <> 1% then goto L01000                 /* (EWD001)  */
            call "OPENFILE" (#2, "OUTPT", f2%(2%), rslt$(2%), axd$(2%))

L01000:     call "OPENCHCK" (#3, fs%(3%), f2%(3%),    0%, rslt$(3%))

                                        /* (EWD001) - Add file open routine */
...
REM           if edi_id$=save_id$ then L01610
              if save_id$ <> " " then L01610
                 ...
                 save_id$=edi_id$
L01610:          gosub unpack_invoice_dates                                 /* Y2K */
                 gosub detail_edi                 /* (EWD001) */
                 gosub detail_ipn                 /* (EWD001) */
...
L01780: return
        ...
        unpack_invoice_dates    /* Unpack dates before they're sent.        Y2K */
            ...
            if str(detail_send_unpk$, 1, 1) <> "H" then goto L01800
                ...
                gosub unpack_date
            REM    if work_date2$ < "980610" then work_date2$ = "980701"
                str(detail_send_unpk$,  74, 6) = work_date2$

                work_date1$ = str(detail_send_unpk$,  80, 6)    /* Disc Due Dte */
                gosub unpack_date
            REM    if work_date2$ < "980610" then work_date2$ = "980630"
...
L01800:     work_date1$ = str(detail_send_unpk$, 212, 6)        /* EDI Create   */
        ...
        build_poack_recs
           if edi_sn% <> 1% then return            /*    (EWD001)       */
...
L02070:       gosub get_edi_duns
              ...
              detail_send_unpk$ = detail_send_item$                         /* Y2K  */
              work_date1$ = str(detail_send_unpk$, 212, 6)    /* EDI Date */
              gosub unpack_date
              str(detail_send_unpk$, 212, 6) = work_date2$                  /* Y2K  */
...
L02560:         convert old_cntl% to old_cntl$, pic(#########)
                ...
                if save_id$<=" " then L02770     /* Create Trailer Record */
                   ...
                   gosub trailer_edi              /* (EWD001) */
                   gosub trailer_ipn              /* (EWD001) */
...
L02770:            hd_str$=edi_duns$                  /* Destination Id */
                   ...
                   gosub header_edi               /* (EWD001) */
                   gosub header_ipn               /* (EWD001) */
                   hdtr_tot% = hdtr_tot%+1%        /* Add Total Headers */
                   rec_tot%  = rec_tot%+1%
                   grnd_tot% = grnd_tot%+1%
        return

                         /* (EWD001) Begin - Put every thing in subroutines */
        trailer_edi
...
L02890:            FMT CH(8), CH(6), CH(6), CH(4), CH(39), CH(167)
        ...
        detail_ipn
          str(detail_send_unpk$,218%,4%) = "SACA"
...
REM         close #2                                /* Close APCEDISN File */
