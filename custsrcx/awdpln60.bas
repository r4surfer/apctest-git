        REM *************************************************************~
            ...
            *                      AWDPLD60 (Tube Winds/Coil Report)    *~
            ...
            *10/09/2009! (AWD003) mod to Merge split balance system!CMG *~ 
            *          !    back together                         !     *~
            *10/14/2009! (AWD004) replace AWDPLA60 with AWDPLND60 ! CMG *~
            *          ! same rpt AWDPLD60 uses bal2 as well      !     *~
            *03/22/2010! (AWD005) mod for polystyrene             ! CMG *~
            *02/20/2011! (AWD006) mod for triple pane glass weight! CMG *~
            *03/05/2012! (AWD007) mod for third balace            ! CMG *~
            *04/17/2014! (AWD008) mod for laminate glass weight   ! MES *~
            *05/19/2014! (CUT001) mod to add dim fields to CUTCC  ! CMG *~
            *09/08/2014! (AWD009) mod for litelift balances       ! CMG *~
            *05/13/2015! (IM7691) mod for litelift balances. Added! PWW *~
            *          !          additional weight checks.       !     *~
            *07/13/2015! SR66916  mod to change glass height com- ! PWW *~
            *          !          par from hgt to dec_height      !     *~
            *09/25/2015! SR69147  mod to change glass height com- ! PWW *~
            *          !          par back, AGAIN, to use hgt.    ! PWW *~
            *          !          See Note below.                 !     *~
            *11/20/2017! CR1173   new strength for sound windows  ! RDB *~
            *08/17/2018! CR1655 mod to undo change AWD003         ! CMN *~
            *11/18/2019! CR2271 mod for dept 027 BMDH shoes       ! MES *~
            *12/03/2019! CR2353 mod to add schema and TX CR2271   ! MES *~
        ...
        dim cc$(3000)1,                  /* Tab Stop/Mark for Dsp/Rmvl */~
            seqn$5,                      /* Seq Number without Array   */~
...
/*AWD007*/  bal3_top$(3000)16,           /* Top Balance                */~
/*AWD007*/  bal3_bot$(3000)16,           /* Bottom Balance             */~
       ...
       dim                               /* (AWD005)                   */~
        ...
        REM *************************************************************~
            ...
            * #63 ! BCKSUBPT ! New Sub Part Number File                 *~
            *************************************************************~
...
/* (AWD005) */
            init(" ") so_inv$, item_no$
            so_inv$  = str(wk_rec$,24,8)
            item_no$ = str(wk_rec$,32,2)
...
/* (AWD005) */
        lookup_sub_part
            init(" ") bcksubpt_rec$, flds$(), info_flds$(), subPart$,~
                      infoPart$
...
sub_part2:
        ...
        call "AWDBKSUB"   (flag$,        /* Flag 0=SalesOrder 1=Invoice*/~
           ...
           convert str(bcksubpt_rec$,153%,8%) to dim1es, data goto badDim1es
badDim1es:
           convert str(bcksubpt_rec$,161%,8%) to dim2es, data goto badDim2es
badDim2es:
           convert str(bcksubpt_rec$,161%,8%) to dim3es, data goto badDim3es
badDim3es:
        return
...
/* (AWD005\) */
        ...
        write_work_file
            write #7, using L20300,                                      ~
                ...
                str(subPart$,1,20),         /* Subpart (AWD005)     */   ~
                dim1es,                     /* Dim1es               */   ~
                dim2es,                     /* Dim2es               */   ~
                dim3es,                     /* Dim3es               */   ~
...
L20300:     fmt ch(6), ch(3), ch(5), ch(30), ch(2), ch(5), ch(10), ch(9),~
                ch(25), ch(20), PD(14,4), PD(14,4), PD(14,4)
L20310:     return
...
REM                CALL "SHOSTAT" ("LOADING SELECTED RECORDS.....") STOP
                cnt% = 0%

L20400:     gosub clear_line
            read #7, key > wk_key$, using L20410, wk_key$, dsp$(i%),     ~
                cus$(i%), part$, subPart$(i%), dim1es(i%), dim2es(i%),   ~
                dim3es(i%),  eod goto L20998    /* (AWD005) */
L20410:             fmt ch(51), ch(10), ch(9), ch(25), ch(20), pd(14,4), ~
                        pd(14,4), pd(14,4)
...
REM CALL "SHOSTAT" ("I AM HERE " & STR(WK_KEY$,10%,5%))
...
/* (AWD007)*/
            ...
            call "STRING" addr("RJ", bal3_top$(i%),16%)  /* Both steps reqd.*/
            call "STRING" addr("CT", bal3_top$(i%),16%)  /* to center data. */
            call "STRING" addr("RJ", bal3_bot$(i%),16%)  /* Both steps reqd.*/
            call "STRING" addr("CT", bal3_bot$(i%),16%)  /* to center data. */
...
REM GOSUB CALC_BALANCE_COVER_NEW
            gosub load_screen                    
/*(AWD005) */
            if str(subPart$,5,1) = "2" then str(infA$(i%),10,1) = "E"
/*(CR2271) (CR2353)*/
       if schema% = 2% and str(sc_dept$,1,3) <> "015"                ~
            then goto not_BMDHshoe    
       if str(sc_dept$,1,3) <> "027" and str(sc_dept$,1,3) <> "028"  ~
            and schema% = 1% then goto not_BMDHshoe
 
       wgt_chk = 0.0
       convert wgt_bot$(i%) to wgt_chk, data goto not_BMDHshoe

       if wgt_chk > 41.8670 then str(infA$(i%),1,9) = "BMDH SHOE"
...
L20998:     if i% <= 3000% then gosub clear_line
            ...
            for x% = dept_max% + 1% to 3000%
                ...
                bal2_top$(x%), bal2_bot$(x%), infA$(x%),     ~
                bal3_top$(x%), bal3_bot$(x%) = " "
                                          /* (AWD005 add second info */
            next x%
            return
...
/* (AWD007) */
               ...
               bal3_top$(i%) = bal3_top$(i% - 1%)
               bal3_bot$(i%) = bal3_bot$(i% - 1%)
...
L22630:    if str(part$,11%,1%) = "5" then        ~
        ...
        clear_line
            ...
            bal_top$(i%), bal_bot$(i%), tdi_top$(i%), tdi_bot$(i%),       ~
                 ...
                 bal2_top$(i%), bal2_bot$(i%), infA$(i%), bal3_top$(i%),  ~
                 bal3_bot$(i%)  = "."
...
L41000:     gosub set_pf2   
            accept                                                       ~
               ...
               at (04,65), fac(hex(84))  , inf$(k% + 1%)        , ch(15),~ 
                                                                         ~
                                                                         ~
               at (05,22), fac(hex(84))  , bal3_top$(k% + 1%)   , ch(16),~
               at (05,42), fac(hex(84))  , bal3_bot$(k% + 1%)   , ch(16),~
/*(AWD005)*/   at (05,65), fac(hex(84))  , infA$(k% + 1%)       , ch(15),~ 
                                                                         ~
               ...
               at (06,65), fac(hex(84))  , inf$(k% + 2%)        , ch(15),~ 
                                                                         ~
                                                                         ~
               at (07,22), fac(hex(84))  , bal3_top$(k% + 2%)   , ch(16),~
               at (07,42), fac(hex(84))  , bal3_bot$(k% + 2%)   , ch(16),~
/*(AWD005)*/   at (07,65), fac(hex(84))  , infA$(k% + 2%)       , ch(15),~ 
                                                                         ~
               ...
               at (08,65), fac(hex(84))  , inf$(k% + 3%)        , ch(15),~ 
                                                                         ~
                                                                         ~
               at (09,22), fac(hex(84))  , bal3_top$(k% + 3%)   , ch(16),~
               at (09,42), fac(hex(84))  , bal3_bot$(k% + 3%)   , ch(16),~
/*(AWD005)*/   at (09,65), fac(hex(84))  , infA$(k% + 3%)       , ch(15),~ 
                                                                         ~
               ...
               at (10,65), fac(hex(84))  , inf$(k% + 4%)        , ch(15),~ 
                                                                         ~
                                                                         ~
               at (11,22), fac(hex(84))  , bal3_top$(k% + 4%)   , ch(16),~
               at (11,42), fac(hex(84))  , bal3_bot$(k% + 4%)   , ch(16),~
/*(AWD005)*/   at (11,65), fac(hex(84))  , infA$(k% + 4%)       , ch(15),~ 
                                                                         ~
               ...
               at (12,65), fac(hex(84))  , inf$(k% + 5%)        , ch(15),~
                                                                         ~
                                                                         ~
               at (13,22), fac(hex(84))  , bal3_top$(k% + 5%)   , ch(16),~
               at (13,42), fac(hex(84))  , bal3_bot$(k% + 5%)   , ch(16),~
/*(AWD005)*/   at (13,65), fac(hex(84))  , infA$(k% + 5%)       , ch(15),~ 
                                                                         ~
               ...
               at (14,65), fac(hex(84))  , inf$(k% + 6%)        , ch(15),~
                                                                         ~
                                                                         ~
               at (15,22), fac(hex(84))  , bal3_top$(k% + 6%)   , ch(16),~
               at (15,42), fac(hex(84))  , bal3_bot$(k% + 6%)   , ch(16),~
/*(AWD005)*/   at (15,65), fac(hex(84))  , infA$(k% + 6%)       , ch(15),~ 
                                                                         ~
               ...
               at (16,65), fac(hex(84))  , inf$(k% + 7%)        , ch(15),~
                                                                         ~
                                                                         ~
               at (17,22), fac(hex(84))  , bal3_top$(k% + 7%)   , ch(16),~
               at (17,42), fac(hex(84))  , bal3_bot$(k% + 7%)   , ch(16),~
/*(AWD005)*/   at (17,65), fac(hex(84))  , infA$(k% + 7%)       , ch(15),~ 
                                                                         ~
               ...
               at (19,02), fac(hex(a4))  , blank$               , ch(78),~
                                                                         ~
                                                                         ~
               at (19,22), fac(hex(84))  , bal3_top$(k% + 8%)   , ch(16),~
               at (19,42), fac(hex(84))  , bal3_bot$(k% + 8%)   , ch(16),~
/*(AWD005)*/   at (19,65), fac(hex(84))  , infA$(k% + 8%)       , ch(15),~ 
                                                                         ~
                                                                         ~
...
REM            at (05,21), fac(hex(84))  , bal2_top$(k% + 1%)   , ch(08),~
REM            at (05,42), fac(hex(84))  , bal2_bot$(k% + 1%)   , ch(08),~
REM            at (07,21), fac(hex(84))  , bal2_top$(k% + 2%)   , ch(08),~
REM            at (07,42), fac(hex(84))  , bal2_bot$(k% + 2%)   , ch(08),~
REM            at (09,21), fac(hex(84))  , bal2_top$(k% + 3%)   , ch(08),~
REM            at (09,42), fac(hex(84))  , bal2_bot$(k% + 3%)   , ch(08),~
REM            at (11,21), fac(hex(84))  , bal2_top$(k% + 4%)   , ch(08),~
REM            at (11,42), fac(hex(84))  , bal2_bot$(k% + 4%)   , ch(08),~
REM            at (13,21), fac(hex(84))  , bal2_top$(k% + 5%)   , ch(08),~
REM            at (13,42), fac(hex(84))  , bal2_bot$(k% + 5%)   , ch(08),~
REM            at (15,21), fac(hex(84))  , bal2_top$(k% + 6%)   , ch(08),~
REM            at (15,42), fac(hex(84))  , bal2_bot$(k% + 6%)   , ch(08),~
REM            at (17,21), fac(hex(84))  , bal2_top$(k% + 7%)   , ch(08),~
REM            at (17,42), fac(hex(84))  , bal2_bot$(k% + 7%)   , ch(08),~
REM            at (19,21), fac(hex(84))  , bal2_top$(k% + 8%)   , ch(08),~
REM            at (19,42), fac(hex(84))  , bal2_bot$(k% + 8%)   , ch(08),~
                    ...
                    for x% = 1% to dept_max%
                        ...
                        if cc$(x%) <> "X" then goto L41003
                          seqn$(x%), mdl$(x%), wdt$(x%), hgt$(x%),       ~
                              ...
                              wgt_top$(x%), wgt_bot$(x%), hgt_top$(x%),  ~
                              hgt_bot$(x%), loc_top$(x%), loc_bot$(x%),  ~
                              bal2_top$(x%), bal2_bot$(x%), bal3_top$(x%),~
                              bal3_bot$(x%) = " "
                          dept_max% = dept_max% - 1%
                          flg% = 1%
L41003:                 if cc$(x%) <> "D" then goto L41005
                          ...
                          call "EWDPLA58" (2%, str(dsp$(x%),1,8),        ~
                            ...
                            wgt_top$(x%),wgt_bot$(x%),hgt_top$(x%),      ~
                            hgt_bot$(x%),loc_top$(x%),loc_bot$(x%),      ~
                            sash_stop_bot$(x%), subPart$(x%), dim1es(x%),~
                            dim2es(x%), dim3es(x%),                      ~
...
/*(AWD007)*/
                    ...
                    call "LINSMASH" (bal3_top$())
                    call "LINSMASH" (bal3_bot$())
...
REM if part$ <> "E412QD0000950250371" then goto pww_cont3 
REM	v$ = "B"~
REM stop  
...
/*IM7691 - */                  PD(14,4), PD(14,4), PD(14,4), PD(14,4)
        ...
        calc_balance
REM             call "SHOSTAT" (" CALC BALANCE " & seqn$ ) stop
...
REM if str(wk_rec$,111,5) <> "00002"  str(wk_rec$,189,19) <> ~
REM "E412QD0000950250371" then goto pww_cont2~
REM  v$ = "B"
REM  stop 
...
/*IM7691 + Check additional range for weight limit  */
    ...
    if str(wk_rec$,111,5) <> "00159" and str(wk_rec$,189,19) <> ~
    "E412BQ5700960314614" then goto pww_cont
    v$ = "B"
    stop 
...
/*(AWD006)*/
/*(AWD008)*/  if strength$ = "3" and triple% = 1% then strength = 3.63
...
/*(AWD008)*/    if strength$ = "4" and laminate% = 1% then strength = 5.55
...
/* (AWD007)*/
REM         if v$ = "T" then bal2_top$(i%) = str(bal_rec$,12%,8%)
         if v$ = "T" then bal3_top$(i%) = str(bal_rec$,12%,16%)

         if v$ = "B" then bal_bot$(i%) = str(bal_rec$,4%,8%)
/* (AWD007)*/
REM         if v$ = "B" then bal2_bot$(i%) = str(bal_rec$,12%,8%)
         if v$ = "B" then bal3_bot$(i%) = str(bal_rec$,12%,16%)
        return
...
/* (AWD003) */
/* (CR1655) */
REM IF SC_DEPT$ = "049" AND STR(PART$,1,3) = "267" THEN STR(READBC$,1,3) = "215"
...
set_top_bot:
              ...
              if show% = 1% and bal_type$ = "6" then          ~
                           str(inf$(i%),15%,1%) = "*"

              if v$ = "T" and bal_type$ = "6" then            ~
                 convert location to loc_top$(i%), pic(-#####0.00##)
              if v$ = "B"  and bal_type$ = "6" then            ~
                 convert location to loc_bot$(i%), pic(-#####0.00##)
