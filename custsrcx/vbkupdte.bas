        REM *************************************************************~
            ...
            * 06/19/06 ! Changes to un-background the process.    ! DES *~
            *************************************************************
        ...
        dim                                                              ~
            ...
            User$3,                      /* User posting PO hold area  */~
...
REM         call "MSGCREAT" (#3, port$, "PORT.ID.VBKUPDTE", return%)
...
REM CMG           nextpokey$ = all(hex(00))
            call "PLOWALTS" (#9, nextpokey$, 1%, 0%, f1%(9))
            if f1%(9) = 1% then process_trans
	    /* if f1%(9) <> 1, EOF? */
            goto L65000
	   
REM             if message$  = "CANCEL" then L65000
REM             if inactive% > idle     then L65000
REM             call "MSGBFGND"(" ",                                     ~
REM                             port$, message$, 10%, inactive%, u3%)
REM             if u3% = 16 then L65000   /* Something's Whacko */
REM             if postflag% <> 0% then L10230
REM
REM                  call "JBJNLCLS" ("J1", userid$, modno$, jnlid$,     ~
REM                                                          pstseq%, 0%)
REM                  postflag% = 1%
REM L10230:         goto next_trans
...
/* CMG */
	     nextpokey$ = stamp$
            ...
            if User$ <> userid$ then goto next_trans                     
