        REM *************************************************************~
            ...
            * 07/20/98 ! Mod to Consolidate By Model              ! RHH *~
            * 04/18/02 ! Mod to change company name   (EWD001)    ! TLM *~
            *************************************************************

        dim                                                              ~
            qty_wa$8, qty_ha$7,          /* Average Width an Height Siz*/~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim cms2v$50
            cms2v$ = "06.04.03 07/20/98 Sales Quick Count By Model    "
        REM *************************************************************
...
REM            company$ = "   Ellison Window and Doors - Welcome   "
            ...
            str(line2$,62%) = "EWDPLN54: " & str(cms2v$,,8)
...
L55210: %!Mod!Avg  Wid!Avg Hgt!White !Bronz !Cocoa !Beige !NatOak!HilOak!~
...
L55270: %!###!########!#######!######!######!######!######!######!######!~
        ...
        print_totals
            ...
            if qty_t < 1 then qty_t = 1.0        /* Calc Avg Width and   */   
            qty_wa = round( qty_w / qty_t, 4)    /* Avg Height per Model */
            ...
            print using L55240
          print using L55270,sav_mod$,qty_wa$,qty_ha$, col$(1%),col$(2%),   ~
...
L61880:
        ...
        calc_average
           if rpt% = 0% then goto L61890      /* Skip for Screen Calcs */
              qty_w = 0.0 : qty_h = 0.0 : qty_t = 0.0
L61890:    qty_w = round(qty_w + (c_width * qtyshp),4) /* TOTAL WIDTH  */
...
L62310:     vals(12%)   = round(vals(12%) + qtyshp, 2) /* Total Color  */
            ...
            vals(14%)   = round(vals(14%) + qty_w,4)  /* Avg Width     */
            vals(15%)   = round(vals(15%) + qty_h,4)  /* Avg Height    */
            vals(16%)   = round(vals(16%) + qty_t,4)  /* avg Total     */
        return
...
L62940:     mod$ = str(wrk_key$,1%,3%)                 /* Model Code   */
            ...
            if len(sav_mod$) < 3 then sav_mod$ = mod$
            if mod$ = sav_mod$ then goto L63010
               gosub print_totals
               sav_mod$ = mod$

L63010:     qty_w = round(qty_w + vals(14%), 4)
            qty_h = round(qty_h + vals(15%), 4)
            qty_t = round(qty_t + vals(16%), 4)
            for k% = 1% to 13%               /* Accum. Totals for Model */
