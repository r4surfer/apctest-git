        REM *************************************************************~
            ...
            *---WHEN---+----------------WHAT----------------------+-WHO-*~
            * 01/16/95 ! New Program for (APC) - Last Mod Date    ! RHH *~
            * 03/06/95 ! Mod to verify Cutomer as EDI             !     *~
            * 03/20/95 ! Mod Made to (APCEDI03) to Seperate Freight RHH *~
            *          !   Charges from Invoice. Put into FRT$    !     *~
            * 07/12/95 ! Mod Made to (APCEDI03) to Send S.O. Numb.t RHH *~
            *          !   in the Invoice Header Record - HOWSHIP !     *~
            *          !                                          !     *~
            * 10/31/97 ! Changed version ID to reflect 60403      ! DJD *~
            *          !                                          !     *~
            * 11/11/97 ! Added fourth key to ARIMASTR (60403)     ! DJD *~
            *          !                                          !     *~
            * 01/27/98 ! Y2K COMPLIANCE                           ! DJD *~
            *          !                                          !     *~
            * 09/08/99 ! (EWD001) - Change arguments to APCEDI03  ! RHH *~
            *          !    add channel for 'HNYMASTR' #9         !     *~  
            *************************************************************

        dim                                                              ~
            edi_cust$9, edi_cust_nam$30, /* CUSTOMER CODE              */~
            edi_inv$8, sku_code$3,       /* INVOICE NUMBER             */~
            dsc_due_dt$6, edi_dte$8,     /* DISCOUNT DUE DATE          */~
            net_due_dt$6,                /* NET DUE DATE               */~
            edi_seq$3, ari_key$17,       /* LINE ITEM SEQUENCE NUMBER  */~
        ...
        REM *************************************************************~
            ...
            * #6  ! APCEDIMC ! EDI Master Control File                  *~
...
L11190:     gosub'101(fieldnr%, 2%)     /* Display & Accept Screen     */
        ...
        create_edi
	/* build start key for arimastr */
	    init(hex(00)) ari_key$
	    str(ari_key$,1,2) = edi_cst$  
	    t1% = edi_dte% / 10000%
	    t2% = t1% * 10000%
	    yyyy% = edi_dte% - t2%  
            mm% = t1% / 100%	      
            dd% = t1% - (mm% * 100%)
	    tst_dte = yyyy% * 10000% + mm% * 100% + dd% 
read_next:
            read #1, key > ari_key$, using cusinvfm, edi_cust$, edi_inv$, ~
                     post_date, eod goto thats_all_folks
            if str(edi_cust$,1,2) <> edi_cst$ then goto thats_all_folks
	    str(ari_key$,1,9)  = edi_cust$
	    str(ari_key$,10,8) = edi_inv$
	    ar_key$ = ari_key$
	    if post_date <> tst_dte then goto read_next 
cusinvfm:   FMT CH(9), CH(8), POS(533), PD(11,1)     
...
*        Define the Input Message for the Screen/Field Indicated
        ...
        dataput
        call "APCEDI03" (#1,               /* ARIBUFFR File - Headers */ ~
...
L40200:     accept                                                       ~
               ...
               at (06,20), fac(lfac$(1%)), edi_cst$            , ch(09),~
               at (06,40), fac(hex(84)),   edi_cust_nam$        , ch(30),~
               at (07,02), "Post Date .  :",                             ~
               at (07,20), fac(lfac$(2%)), edi_dte$             , ch(08),~
                                                                         ~
...
L50300: REM APC Invoice Number                    EDI_INV$
           edi_dte% = 0
	   if edi_dte$ > "        " then                ~
            convert edi_dte$ to edi_dte%, data goto cnv_err
cnv_err: return
