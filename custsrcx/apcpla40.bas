        REM *************************************************************~
            ...
            *09/01/2016! (CR456) undo RGA logic from (AWD013)     ! CMG *~
            *03/12/2020! CR2443  add report for loading (dept 108)! RDB *~
            *04/13/2020! CR2508  add report for staging (dept 106)! RDB *~
            *************************************************************
...
REM          SR$(6%) = "(6) - PRODUCTION IN RGA       "  /* (CR456) */
...
/*(CR456)  H1$(6%) = "NEW-PRODUCTION DETAIL SCREEN'IN RGA'     -APCPLA40" */
...
/* (CR456)  H2$(6%) = "NEW-PRODUCTION SUMMARY SCREEN'IN RGA'    -APCPLA40" */
...
/* (CR456)  H3$(6%) = "BUILD SCREEN FOR 'IN RGA' PRODUCT" */
...
/* (CR456)   H4$(6%) = "PRINTING PRODUCTION'IN RGA'REPORT" */
...
/* (CR456)  h6$(6%) = "Scanning Report of Products*In RGA     *" */
...
/*     "ENTER RPT SELECT,1=NOT SCAN,2=NOT STOCK,3=SCAN,4=STK,5=NOT SO,6=RGA"  (CR456) */~
...
/* CR2443 this is a check for Dt cost in eff report; did not want it here */

REM  CR2443    for i% = 1% to supp_max%                     /* Find Dept. */
REM                if dt_dept$ = supp$(i%) then goto L30360 /* Subscript  */
REM            next i%
...
L30263:     if str(sc_dept$,1%,3%) = "ALL" and dt_dept$ = "001"        ~
...
/* (CR456) */
REM   IF SC_SEL% = 6% THEN INPMESSAGE$ = "   R G A               S C A N N E D   - XXXXX"
...
REM    IF SC_SEL% < 1% OR SC_SEL% > 6% THEN GOTO L50510       /* (CR456) */
...
*       RHH   CONVERT CST(1%) TO C6$(T_MAX%), PIC(####.##-)
        ...
        write_work_file                                      /*  (EWD010)  */
              init(" ") wrk_date$
              wrk_date$ = dt_date$
              if sc_sel% > 2% and sc_sel% < 5% then wrk_date$ = dt_dte$
...
REM         if str(readkey$,10%,1%) <> "0" then goto load_dept_done
        ...
        load_supp_nxt
            ...
            if str(readkey$,1%,9%) <> "PLAN SUPP" then                   ~
               ...
               if str(readkey$,10%,3%) = "032" then goto load_supp_nxt
               kk% = kk% + 1%
...
L62240:     convert j1% to str(jdate1$,5%,3%), pic(000)
            ...
            call "DATE" addr("JG", str(jdate1$,,5%), bb_dte$, x%) /* Date and   */
            call "DATE" addr("JG", str(jdate2$,,5%), ee_dte$, x%) /*Next Days   */
                                                                  /*Prod. Date  */
...
L62710: REM  CHECK SHIFT
           if sc_id$ = "AL" then goto L62740    /* Shift Not Specified */
              if sc_id$ <> ad_shft$ then goto calc_scanned_nxt


L62740:
REM     for i% = 1% to supp_max%                     /* Find Dept. */
REM                if ad_dept$ = supp$(i%) then goto calc_scanned_nxt
REM            next i%

            if ad_dept$ = "108" then goto L63100        /* CR2443*/ 
            if ad_dept$ = "106" then goto L63100        /* CR2508*/ 
            if p_scan% <> 0% then goto L62790
                                                        /* Not Scanned */
...
L62920:
REM            for i% = 1% to supp_max%                     /* Find Dept. */
REM                if str(dt_rec$,42,3) = supp$(i%) then goto calc_scanned_nxt
REM            next i%

           if p_scan% <> 0% then goto L62940     /* Goto Scanned Product*/
...
REM  IF SC_SEL% = 6% THEN GOTO L63100     /* INCLUDE STOCK PROD (CR456) */
            if sc_sel% = 3% or sc_sel% = 6% then goto L63100     /* Include Stock Prod  */
