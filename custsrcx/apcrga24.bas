        REM *************************************************************~
            *                                                           *~
            *  Program Name      - APCRGA24  (Like APCRGA04)            *~
            ...
            *                      (APCRG24B) - By Customer             *~
            *                      (APCRG25B) - By Reason Code          *~
            *                      (APCRG26B) - By Salesman             *~
            *                      (APCRG27B) - By Production Date      *~
            *                      (APCRG28B) - By Status Code          *~
            *                      (APCRG29B) - By Auth. Userid         *~
            *-----------------------------------------------------------*~
        ...
        dim                                                              ~
            ...
            beg_date$8,                  /* Begin Date Range           */~
            end_date$8,                  /* End   Date Range           */~
        ...
        dim blankdate$6,                 /* Empty Date                 */~
            beg_datec$10,                /* Begin Date (Century)       */~
            end_datec$10                 /* End Date (Century)         */~
...
*        Define the Input Message for the Screen/Field Indicated
        ...
        scrn1_msg  :  data                                               ~
         ...
         "Enter the Valid Customer/Code/Salesman/ID or (ALL) ?         ",~
...
L31160: return
        ...
        by_customer
            call "APCRG24B" (#1, #2, rpt_value$, rpt_sum$)

        return

        by_reason_code
            call "APCRG25B" (#1, #2, rpt_value$, rpt_sum$)

        return

        by_salesman
            call "APCRG26B" (#1, #2, #6, rpt_value$, rpt_sum$)

        return

        by_production_date
            call "DATREVRS" (beg_datec$,beg_date$, errmsg$)
            call "DATREVRS" (end_datec$,end_date$, errmsg$)
            call "APCRG27B" (#1, #2, beg_date$, end_date$, rpt_value$,   ~
                             rpt_sum$)
        return

        by_status_code
            call "APCRG28B" (#1, #2, rpt_value$, rpt_sum$)

        return

        by_auth_id
            call "APCRG29B" (#1, #2, #7, rpt_value$, rpt_sum$)
        ...
        deffn'101(fieldnr%, edit%)
            ...
            on fieldnr% gosub L40200,         /* Report Type       */     ~
                              ...
                              L40190,         /* Start Date Range  */     ~
                              L40190          /* End   Date Range  */
            goto L40220
...
L40220:     accept                                                       ~
                ...
                at (08,23), fac(lfac$(4%)), beg_datec$          , ch(10),~
                                                                         ~
                at (09,02), "Ending Date        :",                      ~
                at (09,23), fac(lfac$(5%)), end_datec$          , ch(10),~
                                                                         ~
                at (11,24), "**********************************",        ~
...
L51040: REM - Beginning Date Range
            if rpttype$ <> "4"                                           ~
            or beg_datec$ =" "              then beg_datec$ = "N/A"
            if str(beg_datec$,1%,3%) = "N/A" then L51220
            today$ = date
            call "DATEOKC" (beg_datec$, date%, errormsg$)
                if date% = 0%               then L51200

            call "DATUNFMT" (beg_datec$)
            str(beg_date$,1%,6%) = str(beg_datec$,1%,6%)

            if beg_datec$ > today$           then L51200
            call "DATFMTC"  (beg_datec$)

            rpt_value$ = all(hex(20))
            if str(end_date$,1%,3%) = "N/A" then end_date$ = all(hex(20))
            
            goto L51220
L51200:     errormsg$ = "(Error) - Invalid Beginning Date ?"
            init(" ") beg_date$, end_date$, beg_datec$
L51220: return

L51240: REM - Ending Date Range
            if rpttype$            <> "4"                                ~
            or str(beg_date$,1%,3%) = "N/A" then end_date$ = "N/A"
            if str(end_datec$,1%,3%) = "N/A" then L51420
            if end_datec$ > " "              then L51300
               end_datec$ = beg_datec$ :      goto L51330
L51300:     call "DATEOKC" (end_datec$, date%, errormsg$)
                if date% = 0%               then L51400

L51330:     call "DATUNFMT" (end_datec$)
            str(end_date$,1%,6%) = str(end_datec$,1%,6%)

            if end_date$ > today$                                        ~
            or end_date$ < beg_date$        then L51400
            call "DATFMTC" (end_datec$)
