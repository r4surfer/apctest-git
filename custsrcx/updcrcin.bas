        REM THISPROGRAMWASGENERATEDUSINGTHEGENPGMPROGRAMWHICHISAPROPRIETR~
	        ...
	        * 11/18/97 ! Combined TEST1/RHHSRCE version with      !     *~
            *          ! T10000/SOURCE2 version into new version  ! DJD *~
            * 11/18/97 ! Revision For 60403 Changes               ! DJD *~
            *          ! Look for the word CHANGE in upper case   !     *~
            PRODUCTOFCAELUSASSOCIATESSPOKANEWASHINGTONALLRIGHTSRESERVED**

        dim                                                              ~
					 /* CHANGE 1 - NEXT 3 VARS     */~
					 /* WERE ADDED                 */~
            invnr$8,                     /* INVOICE NUMBER             */~
            rhh_key$21,                  /* CRCBUF2 - PRIMARY KEY      */~
            sav_key$17,                  /* SAVE KEY                   */~
					 /* END OF CHANGE 1            */~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim cms2v$50
            cms2v$ = "R6.04.03 11/18/97 Patch Release w/ Auto Replace   "
        REM *************************************************************
        ...
        REM *************************************************************~
            ...
            * #9  ! CRCBUFFR ! Check Buffer - HEADER                    *~
	    *     !          ! CHANGE 2 - ADDED CRCBUF2 TO FILES OPENED *~
            * #10 ! CRCBUF2  ! Check Buffer - DETAIL                    *~
...
L16660:     if rpt_user$ <> "ALL" and chkuser$ <> rpt_user$              ~
            ...
            print using L17240, cuscode$, cusname$, chknr$, chkdate$,     ~
                               ...
                               ina(4)
            line% = line% + 1%
		/* CHANGE 5 - ADDED GOSUB TO THIS NEW ROUTINE */
            gosub report_loop_detail               /* APC MOD 09/09/91 */
...
L17210: %--------- ------------------------------ -------- -------- ---- ~
        ~  -----------  -----------  -----------  -----------  -----------
	  /* CHANGE 6 - NEW PRINT USING FORMAT   */
L17240: %######### ############################## ######## ########  ### ~
        ~  -########.## -########.## -########.## -########.## -########.##
L17250: %######### ############################## ######## ######## #### ~
        ~  -########.##*-########.##*-########.##*-########.##*-########.#~
...
*        Test Data for USER ID
	  ...
	  /* CHANGE 7 - THE SUB REPORT_LOOP_DETAIL IS A NEW ROUTINE */
        report_loop_detail
            rhh_key$ = all(hex(00))
            str(rhh_key$,1%,9%)  = cuscode$
            str(rhh_key$,10%,8%) = chknr$
            sav_key$ = rhh_key$
            print
            line% = line% + 1%
L60070:     read #10,key > rhh_key$, using L60090, rhh_key$, invnr$,      ~
                                           r4, r2, r3, eod goto L60210
L60090:        FMT CH(21), POS(47), CH(8), POS(93), 3*PD(14,4)
            if str(rhh_key$,1%,17%) <> sav_key$ then goto L60210
               r4 = round(r4, 2)
               r2 = round(r2, 2)
               r3 = round(r3, 2)
               r1 = round( abs(r4) + abs(r3) + abs(r2), 2)
            if line% > 55% then gosub report_heading
            print using L17240, " ", "INVOICE = ",  invnr$, chkdate$,     ~
                               chkuser$, r1, r2, r3, 0.0, r4
            line% = line% + 1%
            goto L60070
