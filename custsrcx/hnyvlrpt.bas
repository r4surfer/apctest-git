        REM *************************************************************~
            ...
            * 10/18/99 ! (EWD001) Add Part Class for to Report    ! CMG *~
            *          !   Criteria                               !     *~
            *          !                                          !     *~
            *************************************************************
        ...
        dim                                                              ~
            ...
            class$04,                    /* From Part Class Code       */~
            fmclass$04,                  /* From Part Class Code       */~
            toclass$04,                  /* To Part Class Code         */~
        ...
        dim lopart$25, hipart$25, lostore$3, histore$3, locatcode$4,     ~
            hicatcode$4, bgkey$20, prompt$(8)28, rptid$6
        ...
        REM *************************************************************~
            ...
            * #03 ! HNYQUAN  ! INVENTORY STORE QUANTITY DETAIL FILE     *~
            * #04 ! STORNAME ! STORE NAMES AND ADDRESS                  *~
            * #05 ! CATEGORY ! Part Category File                       *~
            * #06 ! WORK2    ! WORK FILE                                *~
            * #07 ! GLMAIN   ! GENERAL LEDGER MASTER FILE               *~
            * #08 ! SYSFILE2 ! System Info (ranges for background)      *~
            * #09 ! GENCODES ! System Master Table Files                *~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            prompt$(4%) = "Part Class"      /* (EWD001) */
            prompt$(5%) = "Quantity Type"
            prompt$(6%) = "Report Sort Order"
            prompt$(7%) = "Summary or Detail? (S/D)"
            prompt$(8%) = "Print in Background? (Y/N)"
...
L11120:     fieldnr% = cursor%(1%) - 6%
            if fieldnr% < 1% or fieldnr% >  8% then editpg1         /* (EWD001) */
...
L15430:         FMT POS(17), CH(25), CH(3), CH(6), POS(69), 5*PD(14,4),  ~
            ...
            if f1%(2) <> 0% then L15510
                ...
                catcode$ = hex(00)
                goto L15530
L15510:     get #2 using L15520, catcode$, class$ 
L15520:         FMT POS(90), CH(4), POS(133), CH(4)
L15530:     call "DESCRIBE" (#4, storenr$, storedesc$, 0%, f1%(4))
            ...
            write #6 using L16080, wseq%, account$, partnr$, storenr$,   ~
                     partnr$, class$, catcode$, partnr$, descr$,         ~
...
L15930:        if f1%(6%) = 0 then wrap_up
            ...
            if fmclass$ = "ALL" then goto L16070
            if class$ <= fmclass$ or class$ > toclass$ then read_loop1
L16070:     if qty(qtyp%) = 0 then read_loop1
            okprint% = okprint% + 1%
...
L16770:     if newpart% = 0% then L16831
                ...
                print using L50420, partnr$, descr$, storenr$, class$,   ~
...
L16831:     if s_or_d$ <> "D" then return
            print using L50420, " ", " ", storenr$, class$, costmthd$,    ~
...
L19545:     print skip(3)
            ...
            print using L50567, prompt$(4%), fmclass$, toclass$
            print using L50567, prompt$(5%), qtytype$
            print using L50567, prompt$(6%), sortflag$
            print using L50567, prompt$(7%), s_or_d$
            print using L50567, prompt$(8%), bkground$
            print using L50790
            pageline% = 99%
            return
...
L20250: REM Default/Enable For Class Code        
            if fmclass$ = " " then fmclass$ = "ALL"
            return
...
*        Define the Input Message for the Screen/Field Indicated
        ...
        ~ Wildcard.",                                                    ~
         ...
         "Enter Class Code range, partial, 'ALL' or '?' Wildcard.",      ~
...
L41000:     accept                                                       ~
               ...
               at (10,30), fac(lfac$(4%)), fmclass$             , ch(04),~
               at (10,56), fac(lfac$(4%)), toclass$             , ch(04),~
                                                                         ~
               at (11,02), fac(hex(8c))  , prompt$(5%)          , ch(28),~
               at (11,30), fac(lfac$(5%)), qtytype$             , ch(01),~
                                                                         ~
               at (12,02), fac(hex(8c))  , prompt$(6%)          , ch(28),~
               at (12,30), fac(lfac$(6%)), sortflag$            , ch(01),~
                                                                         ~
               at (13,02), fac(hex(8c))  , prompt$(7%)          , ch(28),~
               at (13,30), fac(lfac$(7%)), s_or_d$              , ch(01),~
                                                                         ~
               at (14,02), fac(hex(8c))  , prompt$(8%)          , ch(28),~
               at (14,30), fac(lfac$(8%)), bkground$            , ch(01),~
                                                                         ~
...
L45400: REM Test Data For Class Code Range(FM)   FMCLASS$ TOCLASS$  /* (EWD001)     */
        if fmclass$ = "ALL" then return
          if fmclass$ <> " " and fmclass$ <> "?" then goto L45430
            if fmclass$ = "?" then gosub L45420
              

           fmclass$ = str(readkey$,10%,4%)
           if toclass$ = "?" then gosub L45420 
           toclass$ = str(readkey$,10%,4%)

        return

L45430:     readkey$ = all(hex(00))
            str(readkey$,1%,9%)   = "PARTCLASS"
            str(readkey$,10%,15%) = fmclass$   
            read #09,key = readkey$, eod goto L45410
            
            if toclass$ = " " then toclass$ = fmclass$

        REM Test Data For Class Code Range(TO)
            init(" ") readkey$
            str(readkey$,1%,9%)   = "PARTCLASS"
            str(readkey$,10%,15%) = toclass$   
            read #09,key = readkey$, eod goto L45410

            return
L45410: errormsg$ = "Invaild Class Code, Not on file."
        return

L45420: readkey$ = all(hex(00))
        readkey$ = "PARTCLASS"
        descr$ = hex(06)&"Select a Valid Class Code"
        call "PLOWCODE" (#09, readkey$, descr$, 9%, .30, f1%(9%))
        
        if f1%(9%) <> 1% then goto L45410
        return
...
L50364: %Part Number                Part Description                Store~
        ~  Class   Cost Method     Quantity    Avg Cost    Total Cost
