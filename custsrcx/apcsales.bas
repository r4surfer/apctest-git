        REM *************************************************************~
            ...
            * 07/23/97 ! Mod - Add Switch for 'Long' or 'Short'   ! RHH *~
            *          !       version of Report With or without  !     *~
            *          !       Gross Profit Information.          !     *~
            * 08/13/97 ! Mod - Change report bucket Definitions   ! RHH *~
            *          !       using tables APCSALE1 and APCSALES.!     *~
            * 11/11/97 ! Mod - Add new file for Labor Sub APCCST5B! RHH *~
            *          !       File - APCPLNDP                    !     *~
            * 03/30/98 ! Y2K modifications                        ! ERN *~
            * 05/09/00 ! Mod for what 'bucket' models go into.    ! CMG *~
            *          !     (EWD0001)                            !     *~
            * 07/17/01 ! Mod to honor dates entered on screen.    ! CMG *~
            *          !     (EWD0002)                            !     *~
            * 04/17/02 ! Mod to change company name (EWD0003)     ! TLM *~
            * 08/27/03 ! Mod for what 'bucket' models go into.    ! CMG *~
            *          !     (EWD0003)                            !     *~
            * 10/31/05 ! (AWD004) CR347 Mod for Sub Part          ! CMG *~
        ...
        dim                              /* Costing Variables          */~
            ...
            so_hows$2, bucket$2          /* USED BY CHECK SALES ROUTINE*/
...
L60390:                                        /* Save Integer for next*/
        ...
        generate_daily
            ...
            if order% = 1% then goto L60470
              if order% = 2% then goto generate_daily    /* (EWD0002)  */
               goto generate_next
...
L60520:     if bck_ord$ < beg_so$(inc% + 1%) then goto L60560
...
REM            if str(model$,1%,1%) = "9" then goto L64500
            ...
            if so_hows$ = "24" then goto L64440       
            pp% = pos("456" = sash$)
            if pp% <> 0% then goto L64440
/* (EWD0001)  -- Begin */
REM            if so_hows$ = "02" or so_hows$ = "03" or so_hows$ = "04" or  ~
REM               so_hows$ = "05" or so_hows$ = "06" or so_hows$ = "22"     ~
REM                                                     then goto L64480
/* (EWD0001)  -- End */
            init(" ") readkey$, bucket$
...
L64410: return
L64420: 
            dd% = 38%                              /* Screen Only     (EWD0003) */  
        return
L64440: 
            dd% = 39%                              /* Parts/Mulls/Sashs (EWD0003) */  
        return
L64460: 
            dd% = 40%                              /* Miscellaneous (EWD0003) */  
        return
L64500: 
            dd% = 35%                              /* Bay/Bow         (EWD0003) */              
        return
