        REM *************************************************************~
            *                                                           *~
            *  Program Name      - APCQUICK                             *~
            *  Creation Date     - 05/24/95                             *~
            ...
            *                      Specified Size                       *~
            *                                                           *~
            *  Special Comments  - VALS() - Buckets for Each Size       *~
            *                      (1 thru 6) - Color Quantities        *~
            *                      (7 thru 10)- Invoice Type Quantities *~
            *                      (11)       - Total for all Inv Types *~
        ...
        dim                                                              ~
            ...
            readkey$24, cnt$8, desc$30,  /* GENCODES Key               */~
            ...
            vals(15%), t_vals(15%),      /* REPORT VALUES / Totals     */~
            wrk_key$28,                  /* Primary Work Key           */~
            col$(6%)8,                   /* Print Color Quantities     */~
            typs$(4%)6,                  /* Invoice Type Quantities    */~
            t_typs$10, t_col$12, t_prc$14,             /* Print Totals */~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim cms2v$50
            cms2v$ = "06.04.03 11/06/97 Sales Quick Count Report       "
        REM *************************************************************
...
REM            company$ = "   Ellison Window and Doors - Welcome   "
            ...
            str(line2$,62%) = "APCQUICK: " & str(cms2v$,,8)
...
L55210: %!Model! Width !Height! White  !  Bronz !  Cocoa !  Beige !!Order~
        ~s!Invoic! Adj's!C-Memo!!Tot Inv Typ!! Total Color!  Total Price !
...
L55300: %! Report Average Width : ##########  Average Height : ##########~
        ...
        print_header                         /* GENERIC REPORT HEADING */
          ...
          print using L55150, beg_date$,end_date$,print_title$,"APCQUICK"
        ...
        print_detail
          ...
          print using L55240
          print using L55270, mod$, wd$, ht$, col$(2%),col$(3%), col$(5%),~
                             col$(6%), typs$(1%), typs$(2%), typs$(3%),  ~
                             typs$(4%), t_typs$, t_col$, t_prc$
        ...
        print_totals
            ...
            for k% = 1% to 4%
                convert t_vals(6% + k%) to typs$(k%), pic(######)
            ...
            convert t_vals(11%) to t_typs$, pic(##,###,###)

            convert t_vals(12%) to t_col$, pic(####,###,###)

            convert t_vals(15%) to t_prc$, pic(##,###,###.##-)

            if lcnt% > 55% then gosub print_header

            print using L55240
          print using L55270,"***"," Total "," **** ",  col$(2%),col$(3%),~
                            col$(5%),col$(6%), typs$(1%), typs$(2%),     ~
                            typs$(3%), typs$(4%), t_typs$, t_col$, t_prc$
          print using L55090
...
L62040:     qtys(p%,2%) = round(qtys(p%,2%) + qtyshp, 2)    /* Shipped */
        ...
        report_values                           /* Report Values for   */
            if chk% = 1% then goto L62270        /* Specific Size       */
               vals(p%+6%) = vals(p%+6%) + 1%   /* Count Invoice Types */
               vals(11%)   = vals(11%)   + 1%   /* Count Totals        */
               chk% = 1%

L62270:     k% = pos("123456" = cc$)            /* Convert Color Code  */
            if k% = 0% then goto L62310
               vals(k%) = round(vals(k%)  + qtyshp, 2) /* Color Shipped*/

L62310:     vals(12%)   = round(vals(12%) + qtyshp, 2) /* Total Color  */
            vals(15%)   = round(vals(15%)   + lineitm, 2)   /* Price   */
        return
        ...
        return
L62430:     call "SHOSTAT" ("Error - Cannot Open (APCQUICK)") : stop
        return
        delete_work
            call "FILEBGON" (#5)
        return
...
L62930:        FMT CH(28), 15*PD(14,4)
            mod$ = str(wrk_key$,1%,3%)                 /* Model Code   */
...
L63080:     for k% = 1% to 6%
            ...
            for k% = 1% to 4%
                convert vals(6% + k%) to typs$(k%), pic(######)

            next k%

            convert vals(11%) to t_typs$, pic(##,###,###)

            convert vals(12%) to t_col$, pic(####,###,###)

            convert vals(15%) to t_prc$, pic(##,###,###.##-)
