        REM THISPROGRAMWASGENERATEDUSINGTHEGENPGMPROGRAMWHICHISAPROPRIETR~
            *                                                           *~
            *  H   H  N   N  Y   Y  DDDD    SSS   PPPP   L      Y   Y   *~
            *  H   H  NN  N  Y   Y  D   D  S      P   P  L      Y   Y   *~
            *  HHHHH  N N N   YYY   D   D   SSS   PPPP   L       YYY    *~
            *  H   H  N  NN    Y    D   D      S  P      L        Y     *~
            *  H   H  N   N    Y    DDDD    SSS   P      LLLLL    Y     *~
            *                                                           *~
            *-----------------------------------------------------------*~
            * HNYDSPLY - Driver to display part movement history.       *~
            *-----------------------------------------------------------*~
            ...
            * 01/03/06 ! CR347 mods for sub part                  ! CMG *~
...
/*EWD001*/  loc$8,                       /* Primary Bin Location       */~
            part$25, part1$25,           /* Part Number                */~
            subpart$20,                  /* Sub Part Number CR347      */~
            subpart1$20,                 /* Sub part number 1 CR347    */~
            ...
            refsubpart$20,               /* Reference Sub Part CR347   */~
        ...
        REM *************************************************************~
            ...
            * #5  ! HNYQUAN  ! Inventory Quantity File                  *~
            * #6  ! HNYDETAL ! Inventory Details                        *~
            * #7  ! HNYHSTRY ! Inventory Usage History                  *~
            * #8  ! GENCODES ! System General Codes file.               *~
            * #9  ! HNYALTRS ! Alternate Parts File                     *~
            * #10 ! HNYPOOL  ! Inventory Pool File                      *~
            ...
            * #19 ! HNYHSTRF ! Inventory History on Fiscal Calendar     *~
            ...
            * #33 ! INVMASTR ! Inventory Master File Finished Goods     *~
            * #34 ! INVQUAN  ! Inventory Quantities File  Finished Goods*~
            * #35 ! INVDETAL ! Inventory Details                        *~
            *************************************************************~
...
/* CR347 - part number */
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            call "DATEFMT" (date$)
            tmp_date$ = date
            str(line2$,62) = "HNYDSPLY: " & str(cms2v$,,8)
...
L09160:         FMT POS(110), CH(1)
        ...
        inputmode
            ...
            init(" ") subpart$, subpart1$, refsubpart$

            ref% = 0%
            alts% = 0%

            call "PLOWNEXT" (#1, part$, 0%, f1%(1))
            if f1%(1) = 1% then get #1 using L10130, part$, descr$        ~
                           else let part$, descr$ = " "
L10130:         FMT CH(25), CH(32)
...
*        Now get which option is desired to be executed.
        ...
        selections_2
                ...
                if keyhit%  =  1% then inputmode
                if keyhit%  =  3% then core_track
                if keyhit%  =  5% then call_bombrwsb
                if keyhit%  =  6% then call_piptsub
                ...
                if keyhit%  = 11% then see_alternates
                if keyhit%  = 12% then see_atc
                if keyhit%  = 14% then see_prices
                if keyhit%  = 16% then exit_program
                if keyhit%  = 21% then call_rtedsply
...
L10610:     call "HNYUDISP" (part$, #1, #7, #4, #6)
            goto selections

L10631:     call "HNYUDSPF" (part$, #1, #19, #4, #6)
            goto selections

        quantity_info
            call "HNYQDISP" (part$, #1, #5, #10, #11)
            goto selections


        display_text
            call "TXTDSPLY" (#2, f2%(2), "001", "Part: " & part$ & ", " &~
                             descr$, textid$, text$())
            goto selections
...
L11920:         errormsg$ = "ERROR - No Route for Effective BOM - " &    ~
        ...
        REM *************************************************************~
            ...
            *************************************************************
        load_part
            get #1 using L30080, part$, descr$, generic$, uom$, cat$,     ~
 /*EWD001*/                     textid$, serialed$, loc$, spec$, type$
L30080:         FMT CH(25), CH(32), CH(16), CH(4), POS(90), CH(4), XX(4),~
                    CH(4), POS(131), CH(1), POS(155), CH(8), /*EWD001*/  ~
                    POS(166), CH(4), XX(10), CH(3)
...
L40574:        close ws
        ...
        set_pf_input
           ...
           pf$(3) = "                (22)Part Xref:                    "&~
                    "             (16)Exit Program"
           pfkeys$ = hex(01ffffffffffffffffffffff0d0e0f10ffffff1600)
           return
...
L42750:        close ws
        ...
        set_pf_edit
           ...
           pf$(1) = "(1)Next Part (3)Core (5/21)BOM/Rte (8)Movements (1"&~
                    "1)Alternates (13)Instructions"
           pf$(2) = "(26)See Text (6)Trace PIP    (9)Usage Summary   (1"&~
                    "2)See A.T.C. (15)Print Screen"
           pf$(3) = "(27)MPS Use  (7)Master Data (10)Qtys (28)Ser #s (1"&~
                    "4)Part Price (16)Exit Program"
           str(pf$(1%),63%,1%) = hex(84)  /* Extra PF Info Available */
           gosub set_pf_extra
           pfkeys$ = hex(01ff03ff05060708090a0b0c0d0e0f10151a1b1c1d00)
            ...
            if part$ = " " then L50190
                ...
                if f1%(1) = 1% then L50230
                     ...
                     if f1%(1) = 0% then L50190
                          call "READ100" (#1, part$, f1%(1))
                          if f1%(1) = 1% then L50230
L50190:     call "GETCODE" (#1, part$, descr$, 0%, 0.32, f1%(1))
            if f1%(1) = 1% then L50230
