        REM *-----------------------------------------------------------*~
            ...
            *---WHEN-----+--------------WHAT----------------------+-WHO-*~
            * 01/09/2007 ! copied from awdedi04 for Landsing      ! DES *~
            * 05/14/2013 ! (AWD001) add schema                    ! CMG *~
            *-----------------------------------------------------------*
        ...
        REM *************************************************************~
            ...
            *-----+----------+------------------------------------------*~
            * #1  ! APCEDILN ! AWD EDI Master IBSA File              *~
            * #3  ! GENCODES ! GENCODES Table File                      *~
            * #4  ! AWDLNSEN ! EWD IPNet Send Flat File                 *~
            *************************************************************~
...
/* AWD000 */  if rec_type$ = "I" then gosub fix_color_desc                
                 gosub detail_edi                 
                 gosub detail_ipn                 
...
fix_color_desc:                
          for l% = 82% to 130%
	     x% = l%
             if str(detail_send_unpk$,l%,1%) = "*" then l% = 132%
             if l% = 132% then goto blank_color_desc 
          next l%
	  return

blank_color_desc:
          for l% = x% to 132%
             if str(detail_send_unpk$,l%,1%) = " " then goto color_exit
             str(detail_send_unpk$,l%,1%) = " " 
          next l%
...
L02890:            FMT CH(8), CH(6), CH(6), CH(4), CH(39), CH(167)
        ...
        detail_ipn
          ...
          if str(detail_send_unpk$,1%,1%) = "H" then            ~
              str(detail_send_unpk$,210%,2%) = "AT"
          if str(detail_send_unpk$,1%,1%) = "H" and schema% = 2% ~
             then str(detail_send_unpk$,210%,2%) = "AW"
