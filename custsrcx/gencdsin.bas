        REM THISPROGRAMWASGENERATEDUSINGTHEGENPGMPROGRAMWHICHISAPROPRIETR~
            *************************************************************~
            * NOTICE - THIS PROGRAM IS PHYSICALLY NAMED GENCDSIN ON DISK*~
            *          IT REPLACES THE CAELUS ORIGINAL VERSION.         *~
            *-----------------------------------------------------------*~
            * EWDPLN61 - Controls System 'Standard Codes File Management*~
            ...
            *---WHEN---+----------------WHAT----------------------+-WHO-*~
            * 09/23/98 ! Copied & modified GENCDSIN; add security.! BWS *~
            * 10/01/98 ! Add PFKey for display only version.EWD001! BWS *~
        ...
        dim                                                              ~
            ...
            plowkey$50,                  /* PLOW KEY                   */~
            sav_key$(200)24,             /* Key Save Array for Screen  */~
            title$79,                    /* For Screen Titles          */~
            userid$3,                    /* Current User Id            */~
            valusr$1                     /* Valid User ID Flag         *~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            plowkey$ = all(hex(00))                          
            str(plowkey$,1%,9%) = ".USERACSS"
            str(plowkey$,10%,3%) = userid$
            read #1, key > plowkey$, using L08100, plowkey$
L08100:         fmt ch(24)                                   
            if str(plowkey$,10%,3%) <> userid$ then security_exit

            pfdescr$(1,1) = "(ENTER)Manage code typed or at cursor   " & ~
                            "                       (13)Instructions"
            pfdescr$(2,1) = "(2)FIRST Screen        (10)MOVE Screen T" & ~
                            "o Entry Typed          (15)Print Screen"
            pfdescr$(3,1) = "(4/5)PREV/NEXT Screen  (12)Print Codes  " & ~
                            "                       (16)EXIT        "
            ...
            pfkeys$(1) = hex(ff02ff0405ffffffff0aff0c0dff0f1000)
...
/*EWD001*/     at (22,40), "(9)Display Only (ALL)    (13)Instructions",  ~
               at (23,65), "(15)Print Screen",                           ~
               at (24,65), "(16)EXIT Program",                           ~
                                                                         ~
/*EWD001*/     keys(hex(00090d0f10)),                                    ~
               key (keyhit%)

            if keyhit% = 16 then L65000

            if keyhit% <>  9 then L10420            /* EWD001 - New */
                call "PROCLINK" ("EWDPLN63","        ","      ",0%,0%)
                goto get_fileid

L10420:     if keyhit% <> 13 then L10430
...
L10470:     if keyhit% <> 0% then get_fileid
                gcode$ = ".USERACSS" & userid$ & file$          
                call "PLOWCODE" (#1, gcode$, filedescr$, 12%, 0.30, f1%(1))
                file$ = str(gcode$,13)
                if f1%(1) <> 1 then L10140

        /* Verify File ID entered in logical file 'FILE DIR'           */
            plowkey$ = str(hx00$) & file$
...
L11080:     gosub'101(0%)      /* Get what is to be done               */
                ...
                if keyhit%  =  4 then gosub load_previous     
...
L11310:     gosub'101(l%)      /* Get field entries.                   */
        ...
        REM *************************************************************~
            ...
            *-----------------------------------------------------------*~
            *  Set FACs on for line modification; default description.  *~
            *************************************************************

        deffn'051(l%)
            dfac$(l%) = hex(80)
            if file$ <> ".USERACSS" or descr$(l%) <> " " then goto L28050
                call "EWDUSRVL" (str(code$(l%,1%),,3%),valusr$)
                if valusr$ = "N" then errormsg$ = "Warning: Invalid User"
                gcode$ = str(hx00$) & str(code$(l%,1%),4%,9%)
                call "DESCRIBE" (#1, gcode$, descr$(l%), 0%, f1%(1))
L28050:   return
...
L29210:     code$(1,1) = code$(2,1)
        ...
        load_previous     /* Load from top of previous page            */
            if maxsc% = 1% then goto L30100
            for x% = maxsc% to 1% step -1
                if sav_key$(x%) < str(file$) & str(code$(2%,1%))         ~
                then sc% = x%                                            ~
            else next x%
L30100:     if len(sav_key$(sc%)) = 24 then                              ~
             plowkey$ = str(sav_key$(sc%),,23)                           ~
                    & bin(val(str(sav_key$(sc%),24,1)) - 1)              ~
             else   plowkey$ = sav_key$(sc%) & hex(00)
            goto load_data
...
L30300:     last% = last% + 1  :  l% = last%
            ...
            if sav_key$(sc%) = " " and plowkey$ > sav_key$(sc% - 1%)     ~
                then sav_key$(sc%) = plowkey$                     
            if sav_key$(sc%) = " " then sc% = sc% - 1%                   
            maxsc% = max(sc%, maxsc%)                                    
...
L30610:   /* Not on file-- setup with defaults               */
        ...
        get_record   /* Get record from buffer and format for display  */
            get #1 using L30850, code$(l%,1), descr$(l%), mcl$(l%),       ~
                                filler$(l%)
        return
...
L30850:         FMT  XX(09),             /* Logical File ID            */~
        ...
        save_data
            ...
            put #1 using L31600, file$, code$(l%,1), descr$(l%), mcl$(l%),~
                                filler$(l%)
...
L32070:     errormsg$ = "CODE: " & code$(l%,1) & " (" & descr$(l%) & ")"
            ...
            if keyhit1% <> 24% then L32070
                call "DELETE" (#1, str(file$) & code$(l%,1), 24%)
                code$(1,1) = code$(l%,1)
                gosub screen_align
                return clear
                goto screen_loop
...
L33500:              errormsg$ = "Table Not Found/No Access: " & file$
                     return

L33520: /* Set up for printing within requested parameters.         */
            gcode$ = hx00$ & file$                          
            read #1, key = gcode$, eod goto L33500                    
            get #1 using L33620, filedescr$         /* Specific File    */
L33620:         FMT XX(24), CH(30)
            break% = 9%

        /* Setups to print report                                   */
            page% = 0  :  line% = 857
...
L33770:     get #1 using L33780, code$(1,1), descr$(1)
L33780:         FMT XX(9), CH(15), CH(30)
...
L40360: accept                                                           ~
          ...
          at(20,12), fac(cfac$(15)),     code$(15,1),                    ~
                                                                         ~
          at(06,30), fac(dfac$( 1)),             descr$( 1),             ~
          at(07,30), fac(dfac$( 2)),             descr$( 2),             ~
          at(08,30), fac(dfac$( 3)),             descr$( 3),             ~
          at(09,30), fac(dfac$( 4)),             descr$( 4),             ~
          at(10,30), fac(dfac$( 5)),             descr$( 5),             ~
          at(11,30), fac(dfac$( 6)),             descr$( 6),             ~
          at(12,30), fac(dfac$( 7)),             descr$( 7),             ~
          at(13,30), fac(dfac$( 8)),             descr$( 8),             ~
          at(14,30), fac(dfac$( 9)),             descr$( 9),             ~
          at(15,30), fac(dfac$(10)),             descr$(10),             ~
          at(16,30), fac(dfac$(11)),             descr$(11),             ~
          at(17,30), fac(dfac$(12)),             descr$(12),             ~
          at(18,30), fac(dfac$(13)),             descr$(13),             ~
          at(19,30), fac(dfac$(14)),             descr$(14),             ~
          at(20,30), fac(dfac$(15)),             descr$(15),             ~
                                                                         ~
...
L65000: REM *************************************************************~
        ...
        security_exit                     
           comp% = 2%
           hdr$     = "******* SECURITY ACCESS DENIED *******"
           msg$(1%) = "You do not have access to any tables in the"
           msg$(2%) = "GENCODES File.  Contact MIS for assistance."
