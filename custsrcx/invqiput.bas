        REM CAELUSASSOCIATESSPOKANEWASHINGTONALLRIGHTSRESERVEDCAELUSASSOC~
            *                                                           *~
            *----------------------------------------------------------Q*~
            * INVQIPUT - Used normally at setup to enter the original   *~
...
/*par000*/  part$45, partdescr$34,       /* Part Number                */~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim cms2v$50
            cms2v$ = "R7.00.00 02/02/06 Year 2000 Compliancy            "
        REM *************************************************************
            mat f2% = con
        ...
        REM *************************************************************~
            ...
            *-----+----------+------------------------------------------*~
            * #01 ! INVMASTR ! Inventory Parts Master  FINISHED GOODS   *~
            * #02 ! INVQUAN  ! Inventory Quantities File  FINISHED GOODS*~
            * #03 ! STORNAME ! Store Master                             *~
            * #04 ! SYSFILE2 ! CMS System Information File              *~
            * #05 ! GLMAIN   ! General Ledger Chart of Accounts         *~
            * #10 ! INVPOOL  ! LIFO/FIFO Pools Records                  *~
...
L03020:     accept                                                       ~
                ...
                at (04,21), "INVQIPUT is an Administrative Function.",   ~
                at (05,17), "It does NOT post to G/L & leaves NO audit tr~
        ~ail!",                                                           ~
                at (10,02), "Press: (RETURN)- Continue with the INVQUAN f~
        ~ile in SHARED mode.",                                            ~
                at (11,02), "                 CAUTION: Updates may confli~
        ~ct with other INVQUAN file users.",                              ~
                at (12,02), "                 NO Editing of Serialized Pa~
        ~rts is allowed in SHARED mode.",                                 ~
                at (14,02), "            (1)- Continue with the INVQUAN f~
        ~ile used EXCLUSIVELY. No other",                                 ~
                at (15,02), "                 user may use INVQUAN while ~
...
L03460:         convert f2%(2) to stat$, pic (###)
                u3% = 0%
                call "ASKUSER" (u3%, "*** INVQUAN FILE ERROR ***",       ~
                     "An error has occurred attempting to open INVQUAN", ~
...
L16180:     gosub'233(screenline%)
        ...
        REM *************************************************************~
            ...
            *************************************************************
        datasave
*        If we are in 'HNYEDT' mode then just save part of INVQUAN
            if admin% = 1% then L19110
                gosub invedt_save        /* PAR000 */
                goto  inputmode
...
*        Def/Enable Lot Number                  LOT$
            inpmessage$ = "Enter Lot Number.  Press PF-8 to see lots"  & ~
                          " on file for this Part / Store."
                                  /* PAR000 */
            call "LOTENABL" (str(part$,1%,25%), le%, ll%, #04, #01)
...
*        Define Screen, Field Cross Ref and Field Enable Settings.
        ...
        dataload                       /* PAR000 */
            plowkey$ = str(part$,,45) & str(store$,,3) & lot$
            call "READ100" (#02, plowkey$, f1%(2))
            if f1%(2) = 1% then L30200
                for n% = 1% to 18%   /* No INVQUAN- get default values */
                     p% = n%
                     call "INVGLGET" (part$, store$, lot$,  ~
                         acct$(n%), p%, #01, #02)
                next n%
                get #01 using L30160, bin$, costtype$       /* PAR000 */
L30160:              FMT POS(175), CH(8), POS(327), CH(1)
                minimum$ = "0" : maximum$ = "0"
                goto L30460
...
L30200:     get #02 using L30250,                                         ~
                     ...
                     userpot$, potdate$         /* PAR000 */
L30250:         FMT POS(17), CH(45), CH(3), CH(6), XX(10), CH(8),        ~
...
*        Load Pool Records
            ...
            startpipquantity = 0
* PAR000
L30730:     call "PLOWNEXT" (#10, poolkey$, 54%, f1%(10))
            if f1%(10) = 0% then return
                ...
                get #10 using L30810, pqty, porig(i%), pcost, pcosts$(i%),~
                                     ...
                                     ptext$(i%)
L30810:             FMT POS(59), 3*PD(14,4), CH(96), CH(6), 2*CH(9),     ~
                        CH(40)            /* PAR000  */
...
L31240:     call "PLOWNEXT"(#10, poolkey$, 54%, f1%(10))
            if f1%(10) = 0% then L31350
                get #10 using L31270, pcqty, pcorig, pccosts()
L31270:              FMT XX(58), 2*PD(14,4), XX(8), 12*PD(14,4)    /* PAR000 */
...
L31970:         FMT CH(2), CH(40), CH(20), CH(25), PD(14,4), CH(5)
        ...
        REM *************************************************************~
            ...
            * --------------------------------------------------------- *~
            * Write INVQUAN & HNYPOOL Records Back to Disk.             *~
            *************************************************************
        save_invquan:
            newcost = 0
...
L32230:         FMT CH(16), CH(45), CH(3), CH(16), CH(8), 7*PD(14,4),    ~
        ...
        write_pipout:
            call "HNYEXSUB" (lot$, store$, str(part$,1%,25%), datexpr$, qty(1))
            return
...
*        pools)
            ...
            if spq% = 0% then L32540           /*PAR000*/
                call "READ101" (#11, str(part$,1%,25%), f1%(11))  /* PIPMASTR */
...
L32450:                   FMT POS(27), 490*BI(4), 4*PD(14,4)
                     ...
                     rewrite #11
                     call "PIPFLAGS" (str(part$,1%,25%), 1%, 1%, 0, #11, #13)
                                         /*PAR000*/
L32540:     poolkey$ = part$ & str(store$) & str(lot$) &    hex(00000000)

            call "DELETE" (#10, poolkey$, 54%)
            if maxlines% = 0% then return
            k% = 1%
...
L32720:              FMT 12*PD(14,4)
                ...
                write #10 using L32920,                                   ~
                     ...
                     " "                 /* Filler                     */
L32920:                   FMT CH(45), CH(3), CH(6), BI(4), 3*PD(14,4),   ~
                              CH(96), CH(6), 2*CH(9), CH(40), CH(58) /* PAR000 */
            next i%
            return
...
*          READKEY$ = STR(STORE$) & LOT$
            call "SERSAVE"                                               ~
                       (1%,              /* Line Item Pointer.         */~
                        ...
                        serkey$,         /* Source Transaction Key     */~
                        1%,              /* # Trans to Create File for */~
/*PAR000*/  str(part$,1%,25%),           /* Part Code                  */~
...
L33240:     put #02 using L33270, bin$, minimum$, maximum$, acct$(),      ~
                                ...
                                userexpr$, exprdate$, userpot$, potdate$
L33270:         FMT POS(81), CH(8), POS(241), 2*CH(10), 18*CH(9),        ~
                    POS(424), CH(6), PD(14,4), CH(4), CH(200), CH(3),    ~
                    CH(6), CH(3), CH(6)
...
L34290:     print using L34510, str(part$,1%,25%), partdescr$, store$, lot$, ~
                               origqty         , origcost,               ~
                               qty(1)          , newcost,                ~
                               qty(1) - origqty, newcost - origcost
            print using L34515, str(part$,26%,20%)
            rline% = rline% + 2%              /* PAR000*/
            select ws
            return
...
L34515: %####################
        ...
        deffn'101(fieldnr%)
            gosub set_pf1
            line2$ = " "
            str(line2$,62%) = "INVQIPUT: " & str(cms2v$,,8%)
...
L40210:     accept                                                       ~
               ...
               at (04,02), fac(hex(94)), errormsg$              , ch(79),~
                                                                         ~
/*PAR000*/     at (06,02), "Part Number:",                               ~
/*PAR000*/     at (06,15), fac(lfac$( 1)), str(part$, 1%,25%)   , ch(25),~
/*PAR000*/     at (06,42), fac(lfac$( 1)), str(part$,26%,20%)   , ch(20),~
/*PAR000*/     at (05,42), fac(hex(8c)),   partdescr$           , ch(34),~
               at (07,03), "Store Code:",                                ~
...
L41020:     return
        ...
        deffn'102(fieldnr%, edit%)
            ...
            str(line2$,,79%) = "Part: " & part$ & "  Store: " & store$ & ~
                                                  "  Lot: " & lot$
            screen%  = 3%
...
L47740:     line1$ = "LIFO/FIFO Pools"
            ...
            if poolnum% <> 2% then L47820                /* PAR000 */
                call "HNYCDIST" ("M", str(part$,1%,25), partdescr$,      ~
                                 str(line2$,,60),                        ~
...
L49705:     if keyhit1% <> 16% then L49770
                errormsg$ = " "
                qty(1) = pctqty            /* Set INVQUAN = HNYPOOL    */
...
*        Test for PART NUMBER                               PART$
            ...
            if f1%(1) = 1% then L50190
                ...
                return
L50190:     call "SERENABL" (str(part$,1%,25%), ser_flag%, u3%, #04, #01)
...
L50570:     if admin% = 1% then L50600
                ...
                return
L50600:     call "LOTVALID"(str(part$,1%,25), store$, lot$, #04,       ~
                           #01, #02, errormsg$)   /*PAR000 */
            return
...
*              READKEY$ = STR(STORE$) & LOT$             /* PAR000 */
                call "SERINSUB" (str(part$,1%,25%), store$, lot$,      ~
                    qty(1), 1%, 1%,                                    ~
...
L56150:         gosub cost_method_lookup
                ...
                rqst% = 2% : temp = 0
                call "STCCOSTS" (str(part$,1%,25%), " ", #4, rqst%, temp, std())
                return
