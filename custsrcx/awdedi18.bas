        REM *-----------------------------------------------------------*~
            *                                                           *~
            * AAAAAA  W   W DDDD   EEEE  DDDD  IIIIII    1      8888    *~
            * AA  AA  W   W DD  D  EE    DD  D   II     11     8    8   *~
            * AAAAAA  W W W DD  D  EEEE  DD  D   II      1      8888    *~
            * AA  AA  WW WW DD  D  EE    DD  D   II      1     8    8   *~
            * AA  AA  W   W DDDD   EEEE  DDDD  IIIIII  11111    8888    *~
            *-----------------------------------------------------------*~
            * AWDEDI18 - Create Flat File To Send To IPNET              *~
            *-----------------------------------------------------------*~
            *                  M O D I F I C A T I O N S                *~
            *---WHEN-----+--------------WHAT----------------------+-WHO-*~
            * 02/22/2007 ! Copied from awdedi04 for ENAP          | DES *~
            * 03/30/2000 ! AWD001 stop orphaned "I" records       | DES *~
            * 10/11/2013 ! (AWD002) mod for schema lookup         | CMG *~
            * 10/31/2014 ! (AWD003) mod for EDI Identifier        | CMG *~
            * 04/09/2021 ! CR2814 Fix APCEDIEA status update      | RDB *~
            *-----------------------------------------------------------*


        dim apcmc_key$22,                /* ARIEDIMC File Key          */~
            hold_key$22,                 /* ARIEDIMC File hold key     */~
            axd$(64%)4,                  /*                            */~
            ...
            type$2,                      /* Record Type                */~
            invSys$3,                    /* Invoice System ANC NTX     */~
            work_date1$10, work_date2$10 /* Temp dates                 */


              
        dim ediId$6                      /* EDI Identifier    (AWD003) */
        ...
        REM *************************************************************~
            ...
            *-----+----------+------------------------------------------*~
            * #1  ! APCEDIEA ! AWD EDI Master IBSA File              *~
            * #3  ! GENCODES ! GENCODES Table File                      *~
            * #4  ! AWDENSEN ! EWD IPNet Send Flat File                 *~
            *************************************************************~
...
/* (AWD003) */
            ediId$ = "APCBUI"
REM            if schema% = 2% then ediId$ = "NTXEDI"
/* (\AWD003) */
        return
...
/* <AWD001> */
           if str(detail_send_item$,1%,1%) = "H" then               ~
               hold_key$ = apcmc_key$

           if str(detail_send_item$,1%,1%) <> "I" then  goto continue 
           if str(hold_key$,10,8) = str(apcmc_key$,10,8) then ~
	                                                      goto continue
	         goto read_next_inv
continue:
...
/* </AWD001> */
           ...
           if edi_id$ <> "050" then goto read_next_inv
           ...
           if type$<>"SI" then L01750
              ...
              invSys$ = str(detail_send_item$,224%,3%)  /* CR2814 */
              sav_type$="TI"
...
L02890:            FMT CH(8), CH(6), CH(6), CH(4), CH(39), CH(167)
        ...
        detail_ipn
          ...
          if str(detail_send_unpk$,1%,1%) <> "H" then notDetailIPNHeader
            if str(detail_send_unpk$,224%,3%) = " " and schema% = 1% then  ~
                               str(detail_send_unpk$,224%,3%) = "ANC"
            if str(detail_send_unpk$,224%,3%) = " " and schema% = 2% then  ~
                               str(detail_send_unpk$,224%,3%) = "NTX"
notDetailIPNHeader:
...
L01620:          FMT CH(226)
        ...
        reset_transmit_flag
              ...
              put #1, using L03030, sav_type$, part1$, edi_id$, part2$,    ~
                                  ...
                                  invSys$, edi_cust$
L03030:       FMT CH(2), CH(4), CH(3), CH(12), CH(211), CH(6), CH(6), CH(03),  ~
                  CH(9)
