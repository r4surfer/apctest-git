        REM *************************************************************~
            ...
            * 04/06/98 ! y2k compliance and r60403 changes (none) ! DJD *~
            * 11/08/90 ! Special APC Mod to build and store       ! RHH *~
            *          ! Descriptions. Printer Description and    ! RHH *~
            *          ! Size Description. 32410-32417, 1002-1006 ! RHH *~
            * 11/09/90 ! Mod to Calculate Units and store in      ! RHH *~
            *          ! in Variable Field (3). Routine at (60000)! RHH *~
            * 04/14/98 ! Mods to Update Caelus's R6.04.03 Version ! RHH *~
            *          !   with all the (EWD) Mods                !     *~
            * 05/05/08 ! (AWD001) slow moving indicator           ! CMG *~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim cms2v$50
            cms2v$ = "R6.04.02 11/13/95 Precious Metals (EWD) Mods     "
        REM *************************************************************
            mat f2% = con
        ...
        REM *************************************************************~
            ...
            * #26 ! INVMASTR ! Inventory Master File Finished Goods     *~
            * #27 ! INVQUAN  ! Inventory Quantities File  Finished Goods*~
            * #28 ! INVDETAL ! Inventory Details  Finished Goods        *~          
            *************************************************************
...
L12065:           if keyhit%  =  1% then gosub startover
            ...
            fieldnr% = cursor%(1%) - 4%
	    if fieldnr% = 15% and cursor%(2%) > 40% then fieldnr% = 16%
            if fieldnr% < 2% or fieldnr% > 16% then L12040
...
L12160:     gosub'101(fieldnr%, 2%)
            ...
            for fieldnr% = lastfieldnr% to lf2%     /* 15% */
                gosub'151(fieldnr%)
                     lastfieldnr% = fieldnr%
...
L12270:           if keyhit%  =  1% then gosub startover
            ...
            fieldnr% = cursor%(1%) - 4%
            if fieldnr% < 1% or fieldnr% > 16% or fieldnr% = 9%          ~
                                                       then edit_page_two
...
L20700   /* cut length in inches */
            cut_ln = 0
            convert cut_ln$ to cut_ln, data goto bad_cut
bad_cut:    convert cut_ln to cut_ln$, pic(0000)                
            return
...
*        Define the Input Message for the Screen/Field indicated.
        ...
        scrn1_msg : data                                                 ~
         ...
         "Enter the Cut Length (in inches) for this Part.", ~
         "Enter the Color Code for this Part."
...
*        Define Screen, Field Cross Ref and Field Enable Settings.
            ...
            scr%(1%,15%) = 15% : set%(15%) =  2%   /* Cut Length       */
            scr%(1%,16%) = 16% : set%(16%) =  2%   /* Cut Length       */
...
*        Part already on file
            ...
            get #2, using L31020, description$, generic$, stkunit$,       ~
                        ...
                        ovrshppct, ovrshpqty, vfv$(), formula_name$, slow$, ~
			cut_ln$, color_cd$              
	    frm = 051
            gosub L50955         /* color            */
                                                    /* (AWD001) */
...
L31020:     FMT XX(25),          /* Part Number                        */~
                ...
                CH(01),          /* Slow Moving Flag                   */~
		POS(183), CH(04),/* cut length                         */~
		POS(187), CH(01) /* cut length                         */
...
*        Convert Alphas to Numerics
            ...
            if len(part$) < 19% then goto L32410  /* BUILD DESCRIPTIONS */
               call "APCDESCR" (part$, apc_scr$, apc_prt$, apc_sze$, #25,~
                                                           er% )
               ...
               gosub calculate_units
                                         /* (EWD) - End Mods           */
L32410:     if dfltsedit% = 1% then junkkey$ = all(hex(00))
...
L33070:     FMT CH(25),              /* Part Number                    */~
                ...
                10*CH(12),           /* Formula Variable Fields        */~
                CH(16),              /* Default Formula Name           */~
/*AWD001*/      CH(01),              /* Slow moving Lineal             */~
                CH(026),             /* Filler                         */~
		POS(183), CH(04),    /* cut length                     */~
		POS(187), CH(01) /* cut length                         */
...
/*AWD001*/     at (13,45), fac(fac$( 9%)), slow$                , ch(01),~
               at (14,02), "Part Type",                                  ~
               ...
               at (19,02), "Cut Length (in inches):",                    ~
               at (19,30), fac(fac$(15%)), cut_ln$              , ch(04),~
               at (19,35), fac(hex(84)), x$,                       ch(04),~
               at (19,40), "Color code:",                                ~
               at (19,52), fac(fac$(16%)), color_cd$            , ch(01),~
               at (19,55), fac(hex(8c)), color_desc$            , ch(24),~
                                                                         ~
...
/* (AWD001) */
            if slow$ <> " " and slow$ <> "0" and slow$ <> "1" then ~
                        goto L50675
            return
L50675:     errormsg$ = "Please enter Slow moving 0 or 1??"
            init(" ") slow$
            return
...
L50955  /* color      */
                init(" ") color_desc$
                readkey$ = "COLOR    " & color_cd$
                read #20, key=readkey$, using COLORFMT, color_desc$, ~     
                                                       eod goto L50960 
COLORFMT:       FMT POS(30), CH(24)
                return  

L50960:     if frm <> 151 then return                                        
            mfgdescr$ = hex(06) & "Select Color Code"
            readkey$ = "COLOR    "
            call "PLOWCODE" (#20, readkey$, gen_desc$, 9%, .3, f1%(20%))
                if f1%(20%) = 1% then L50970
                   errormsg$ = "Invalid Color Code"
                   goto L50960
L50970:     color_desc$ = str(gen_desc$,6,12)
            color_cd$ = str(readkey$,10,1)
            return
