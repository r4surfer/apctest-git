...
DT_FMT:      FMT CH(256)
             ...
             goto noSHOSTAT
             if dt_bar$ <> "038696320700010003" and                   ~
                   dt_bar$ <> "038586840100020002" then goto noSHOSTAT
...
notDisplay:
             ...
             due_date$ = dt_date$
             due_time$ = "1500"
             if support% = 1% then due_time$ = "2300"
        ...
        isSkip
          skip% = 1%
          if skipDept$ = "001" or skipDept$ = "006" then return
...
REM if ad_st1$ > "12" and ad_st$ <> " " then goto complete_next
        ...
        complete_done
          onTime% = 1%
          if str(ad_dte$,1%,6%) > str(dt_date$,1%,6%) and   ~
              adj_time$ > due_time$ then onTime% = 0%
          if str(ad_dte$,1%,6%) > str(dt_date$,1%,6%) then onTime% = 0%
