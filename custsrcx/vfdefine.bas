        ...
        dim                                                              ~
            ...
            gencodes1$(10)9,             /* GENCODES file names        */~
            gencodes2$(10)9,             /* GENCODES file names        */~
            gendescr$(10)32,             /*          file descriptions */~
            gendescr1$(10)32,            /*          file descriptions */~
            gendescr2$(10)32,            /*          file descriptions */~
            ...
            key$20, data$3,              /* SYSFILE2 record key        */~
            line2$79,                    /* Second screen line         */~
            msg$(10)40,                  /* Info Message for VF        */~
            msg1$(10)40,                 /* Info Message for VF        */~
            msg2$(10)40,                 /* Info Message for VF        */~
            pf16$16, pf8$34, pf9$25,     /* PF Key Prompts             */~
            prompt$(10)20,               /* VF Prompt                  */~
            prompt1$(10)20,               /* VF Prompt                  */~
            prompt2$(10)20,               /* VF Prompt                  */~
            reqd$(10)1,                  /* Required / Optional        */~
            reqd1$(10)1,                  /* Required / Optional        */~
            reqd2$(10)1,                  /* Required / Optional        */~
            size%(10),                   /* Field Size (1-20)          */~
            size1%(10),                   /* Field Size (1-20)          */~
            size2%(10),                   /* Field Size (1-20)          */~
            type$(10)2,                  /* Field Type                 */~
            type1$(10)2,                 /* Field Type                 */~
            type2$(10)2,                 /* Field Type                 */~
...
*        Write out files and defaults to SYSFILE2.
	    ...
	    multi$ = "N"
            ...
            customer : file$ = "CUSTOMER" : fields% = 10%: fields2% = 6%
...
*        Write data to file (only if not there)
        ...
        inputmode
            init(" ") errormsg$, inpmessage$, key$, type$(), prompt$(),  ~
                      ...
                      gendescr$(), pf8$, pf9$
            init(" ") type1$(), prompt1$(), reqd1$(), msg1$(),  ~
                      gencodes1$(), gendescr1$()   
            init(" ") type2$(), prompt2$(), reqd2$(), msg2$(),  ~
                      gencodes2$(), gendescr2$()   
            mat size% = zer
...
L11060:     gosub'111(0%, 0%)
                  ...
                  if keyhit%  =  9 then gosub toggle_recs    
                  if keyhit%  = 16 then       datasave
                  if keyhit% <>  0 then       L11060
L11110:     fieldnr% = cursor%(1) - 8%
            fieldsx% = fields%
	    if page% = 2% then fieldsx% = fields2%
            if fieldnr% < 1% or fieldnr% > fieldsx% then L11060
...
toggle_recs:       
           if multi$ <> "Y" then return
	   if page% = 2% then goto set_pg_2
           page% = 2%
           gencodes1$() = gencodes$()
           gendescr1$() = gendescr$() 
           msg1$()      = msg$()
...
L11280:     fieldnr% = cursor%(1) - 8%
            fieldsx% = fields%
	    if page% = 2% then fieldsx% = fields2%
            if fieldnr% < 1% or fieldnr% > fieldsx% then L11230
...
L30260:
           ...
           type1$()     = type$()
            ...
            str(key$,3,1) = "3"
            call "READ100" (#1, key$, f1%(1))
            get #1 using L30080, file$, filedescr$, fields2%, type2$(),     ~
                                size2%(), prompt2$(), reqd2$(), gencodes2$()
            str(key$,3,1) = "4"
            call "READ100" (#1, key$, f1%(1))
            if f1%(1) = 0% then L30280
                get #1 using L30140, msg2$()

            for f% = 1% to fields%
                if gencodes2$(f%) = " " then L30270
                     genkey$ = hex00$ & gencodes2$(f%)
                     call "READ100" (#2, genkey$, f1%(2))
                     if f1%(2) = 0% then                                 ~
                          gendescr22$(f%) = "** Not Defined **"            ~
                                    else                                 ~
                          get #2 using L30240, gendescr2$(f%)
L30270:     next f%
L30280:
...
/* if multi$ = "y" and page=2 save page2 data */
           if multi$ <> "Y" or page% = 1% then goto L31110       
	   /* save page2 data */
           gencodes2$() = gencodes$()
           gendescr2$() = gendescr$() 
           msg2$()      = msg$()
...
/* if multi$ = "y" then need to know which version of array to write
   and if need to write VF3 & VF4 records
   right now just for CUSTOMER file      @@@           */
            if multi$ <> "Y" then return

            key$ = "VF3:" & str(file$)
            if newonly% = 1% then L31195
                call "DELETE" (#1, key$, 11%)

L31195:     write #1 using L31140, key$, filedescr$, fields2%, type2$(),    ~
                                  size2%(), prompt2$(), reqd2$(),           ~
                                  gencodes2$(), " ",      eod goto L31200

            key$ = "VF4:" & str(file$)
            if newonly% = 1% then L31197
                call "DELETE" (#1, key$, 11%)

L31197:     write #1 using L31190, key$, msg2$(), " "
            ...
            *************************************************************
            ...
            deffn'111(fieldnr%, err%)    /* Edit VF Characteristics    */
                ...
                if fieldnr% = 0% then                                    ~
                       ...
                       pf8$ = " "
                pf9$ = " "
                if fieldnr% = 0% and multi$ = "Y" and page% = 1% then    ~
                       pf9$ = "(9)Edit alternate records"         
                if fieldnr% = 0% and multi$ = "Y" and page% = 2% then    ~
                       pf9$ = "(9)Edit main records "      
...
L40420:   accept                                                         ~
            ...
            at (06,23), fac(hex(8c)),   data$                   , ch(03),~
            at (06,28), fac(hex(8c)),   filedescr$              , ch(32),~
                                                                         ~
