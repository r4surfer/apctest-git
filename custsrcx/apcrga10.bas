        REM *************************************************************~
            *                                                           *~
            *  Program Name      - APCRGA10                             *~
            *  Creation Date     - 12/18/95                             *~
            *  Last Modified Date- 10/31/97                             *~
            *  Description       - This Program Creates the RGA         *~
            *                      Labels by RGA No.                    *~
            *                                                           *~
            *  Special Notes     - Compliment program to APCRGA02,      *~
            ...
            *---WHEN---+----------------WHAT----------------------+-WHO-*~
            * 12/18/95 ! New Program for (APC) - Last Mod Date    ! JBF *~
            * 08/14/96 ! Add Processing to support UBI 601 printer! JBF *~
            *          !                                          !     *~
...
/*PAR000*/              varc,     indexed,  recsize =  512,              ~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            str(line2$,62%) = "APCRGA10: " & str(cms2v$,,8%)
...
*        Define the Input Message for the Screen/Field Indicated
        ...
        initialize_variables
            init(" ") errormsg$, inpmessage$, beg_nbr$, end_nbr$,        ~
                rga_number$, r_number$, rga_item$, dtlkey$, r_cuscode$,  ~
...
L31880: return
        ...
        select_log_report
            title$  = "** APCRGA10 - RGA Barcode Reprint Log **"
            pageno% = 0%
            lcnt%   = 99%
            call "TIME"    (xtime$)
            call "SETPRNT" (" ", "RG10", 0%, 0%)
        ...
        deffn'101(fieldnr%, edit%)
            ...
            on fieldnr% gosub L40170,         /* Start RGA Nbr.    */     ~
                              L40170          /* End   RGA Nbr.    */
            goto L40190
...
L50110: REM - Beginning RGA Number
            ...
            convert beg_nbr$ to beg_nbr%,  data goto L50220

            convert beg_nbr% to beg_nbr$,  pic(0000)
...
L60300:         FMT POS(10), CH(30)
            ...
            c_desc$     = cus_desc$
        return
L60330:     init(" ") cus_desc$, c_desc$
        return

        lookup_part                           /* Check HNYMASTR        */
            init(" ") part_desc$, apc_prt$, apc_sze$, apc_scr$, dt_flag$
            read #9,key = rga_part$, using L60400, part_desc$, apc_prt$,  ~
                apc_sze$, eod goto L60430
L60400:         FMT XX(25), CH(32), POS(606), CH(60), CH(20)

            goto L60570
L60430:     err% = 0%
            if len(rga_part$) > 18% then L60520
...
L60520:     call "APCDESCR" (rga_part$, apc_scr$, apc_prt$, apc_sze$,    ~
                             #11, err%)
...
L60570: return
        ...
        lookup_bcklines
            ...
            convert rga_line$ to xx%, data goto L60640
L60640:
            convert xx%       to str(bck_key$,17%,3%), pic(###)

            read #8,key = bck_key$, using L60690, bck_text$,              ~
                eod goto L60710
L60690:         FMT POS(242), CH(04)
...
L60710: return
        ...
        lookup_text                           /* Look Up Text Id       */
            ...
            str(text_key$,9%,1%) = "1"
            sav_key1$            = text_key$
            read #10,key > text_key$, eod goto L60940

            get  #10, using L60880, text_key$, atext$()
L60880:         FMT CH(11), POS(64), 2*CH(70)

            if str(sav_key1$,1%,9%) <> str(text_key$,1%,9%) then L60940
