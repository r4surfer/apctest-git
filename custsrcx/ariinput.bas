        REM THISPROGRAMWASGENERATEDUSINGTHEGENPGMPROGRAMWHICHISAPROPRIETR~
            ...
            *-----------------------------------------------------------*~
            * *CHANGE 1 (EWD) *                                         *~
            * ARIINPUT - Invoice Entry. APC VERSION 35 PRICE CODES      *~
            *************************************************************~
            ...
            *          ! (EWD) - Begin                            !     *~
            * *CHANGE 2! --- THESE LINES ARE CARRIED OVER FROM    !     *~
            *          !     OLD ARIINPUT.BAS DURING THE          !     *~
            *          !     60403 UPGRADE                        !     *~
            * 07/05/90 ! APC VERSION - RE-COMPILE WITH APC MODS   ! RHH *~
            *          !               TO 'ARIINPTS'              !     *~
            * 11/04/92 ! APC VERSION - SET DEFAULT TO "EACH" FOR  ! RHH *~
            *          !               STKUOM$() AND PRICEUOM$()  !     *~
            *          ! (EWD) - End   LINE NUMBERS (23216,23312) !     *~
            * 11/01/00 ! Mod to add part description from APCDESCR! CMG *~
            *          !     (EWD001)                             !     *~
            * 12/06/00 ! Mod to be able to pull up old invoice and! CMG *~
            *          !     edit for only 'C'redits (EWD002)     !     *~     
            * 07/06/06 ! (AWD003) mod for ESC                     ! CMG *~
            * 09/27/06 ! (AWD004) use invoice billxref            ! DES *~
            *02/26/2013! (AWD005) additional changes for ESC      ! CMG *~
        ...
        dim                                                              ~
            ...
            escacct$12, escacctdescr$30, /* ESC Account   AWD005       */~
...
/*AWD005*/  currency$4, currdesc$(3)32,  /* Currency code& description */~
...
/*EWD001*/  apc_scr$120,                 /* Screen Description         */~
/*EWD001*/  apc_prt$60,                  /* Print Description          */~
/*EWD001*/  apc_sze$20,                  /* Size Description           */~
            energy$14                    /* ESC                        */
        ...
        call "SHOSTAT" ("Opening Files, One Moment Please")
            ...
            call "OPENCHCK" (#08, fs%(08%), f2%(08%),   0%, rslt$(08%))
                ...
                /*CHANGE 3 (EWD) */
            call "OPENCHCK" (#09, fs%(09%), f2%(09%),1200%, rslt$(09%))
            /*CHANGE 4*/
            call "OPENCHCK" (#10, fs%(10%), f2%(10%),1200%, rslt$(10%))
                /* (EWD) - End    */
...
L10470:     mostin% = 0%
            for fieldnr% = 1% to 14%     /*-----HEADER SCREEN 3--------*/
...
** Not sure if should be 16 or 14 on fieldnr%
                if fieldnr% <   1% or   fieldnr% >  16% then edithdr3
...
L19215:     grossinv, taxamt, taxable, netinv, escamt = 0
            lines%, deles% = 0%
            inpmessage$, delemsg$ = " "
...
/*AWD005*/  escamt     = round(grossinv * energy     * .01, 2)
*       Do i need to modify netinv for ESC???
...
/* (AWD005) */
            ...
            escacct$ = acct$  :  escacctdescr$ = acctdescr$
...
L28280: data                                                             ~
        ...
        /* Screen 3                                                    */~
         ...
         "Enter G/L Account Code for Energy Surcharge.                 ",~
                                                                         ~
        /* Screen 4                                                    */~
        ...
        init_for_input
            init(" ") acct$, acctdescr$, aracct$, aracctdescr$, artype$, ~
                      ...
                      pm_inv$, cus_part_type$(), escacct$, escacctdescr$

            init (hex(00)) sn_lot1was$()

            cartons, fcamt, frtamt, grossinv, invdiscamt, invdiscpct,    ~
            netinv, taxamt, taxpct, weight, energy, escamt = 0
...
L30180:         get #23, str(readkey$,,9)
                if invtype$ = "C" then L30240         /* (EWD002) */
...
L30240
*        Invoice DOES EXIST.  Load it up. (EWD002) - Make sure correct file Num 
         if invtype$ = "C" and invonfile% = 0% then f% = 15%  
         if invtype$ = "C" and invonfile% = 0% then f1% = f% + 1%
            get #f% using L30250, temp$    /* Invoice Type     */
L30250:         FMT POS(891), CH(1)
            if temp$ = invtype$ or invtype$ = "C" then L30270  /* (EWD002) */
...
L30270:     prtmsg$ = "Loading Invoice..."
            ...
            get #f% using L35365, cuscode$, invnr$, po$, so$, bol$,       ~
                     ...
                     pm_so$, pm_inv$, energy, escamt, escacct$ /*(AWD005) */

            for u3% = 1% to 30%
                if termsdisc$(u3%) = blankdate$ then termsdisc$(u3%) = " "
                if termsnet$(u3%)  = blankdate$ then termsnet$(u3%)  = " "
            next u3%
 /* (EWD002) - Do not need SO if Credit */
            if invtype$ = "C" and invonfile% = 0% then str(so$,1%,16%) = " "
...
L30392:           if invtype$ = "C" and invonfile% = 0% then str(invnr$,1%,8%) = " "
                  str(line2$,22) = "Invoice: " & invnr$
                  if invnr$ = " " and (s% > 1% or f% > 2% or edit% = 2%) then  ~
                       str(line2$,22) = "Invoice: -NEW-"
                  if maxlines% + total_e_lines% <= 100% then return
                  gosub extra_load_conflict
                  return
...
/* (EWD002) Do not need openqty, make sales acct end in '2', & init PO item Ref */
        if invtype$ = "C" and invonfile% = 0% then gosub change_lines
...
L30535:         call "TXTFUTIL" (#22, f2%(22), "LOAD", textidl$(c%))
                if invtype$ <> "C" then L30570
                   if invonfile% = 0% then L30570   /* (EWD002) Do not make neg  */
                     order      (c%) = -order      (c%)  /* (EWD002) system does */
                     ship       (c%) = -ship       (c%)  /* automatically if Credit */
                     openqty    (c%) = -openqty    (c%)
                     linediscamt(c%) = -linediscamt(c%)
                     lineext    (c%) = -lineext    (c%)
...
L30870:     next i%
            ...
            call "CONVERT" (energy    , 2.2, energy$    )
...
/*AWD004*/  gosub fix_billxref
           ...
           put #f% using L35365, cuscode$, invnr$, po$, so$, bol$,       ~
                ...
                energy, escamt, escacct$, " "
                

            if f% = 9% then put #9 using L34100, energy, escamt, escacct$,~
                                                 session$, invtype$,      ~
                                                 cuscode$, invnr$
L34100:         FMT POS(1811), PD(14,4), PD(14,4), CH(09),  ~
...
L35365: FMT                 /* FILE: ARIMASTR                          */~
            ...
            CH(1),          /* PM Surcharge INV Flag                   */~
            PD(14,4),       /* ESC                                     */~
            PD(14,4),       /* ESC AMT (AWD005)                        */~
            CH(9),          /* ESC Account (AWD005)                    */~
            CH(165)         /* Filler (Internal, unused space)         */
...
L42135:     accept                                                       ~
               ...
               at (19,49), fac(hex(8c)),   discacctdescr$       , ch(32),~
                                                                         ~
/*AWD005*/     at (20,02), "Energy Surcharg Account",                    ~
/*AWD005*/     at (20,30), fac(lfac$(14)), escacct$             , ch(12),~
/*AWD005*/     at (20,49), fac(hex(8c)),   escacctdescr$        , ch(32),~               
                                                                         ~
               at (21,02), fac(hex(a4)),   inpmessage$          , ch(79),~
               at (22,02), fac(hex(8c)),   pf$(1)               , ch(79),~
               at (23,02), fac(hex(8c)),   pf$(2)               , ch(79),~
...
L46230:     accept                                                       ~
               ...
               at (13,30), fac(hex(84)), taxamt,     pic(-##,###,###.00),~
/*(AWD005)*/   at (14,02), "Energy Amount",                              ~
/*(AWD005)*/   at (14,30), fac(hex(a4)), escamt,     pic(-##,###,###.00),~               
               at (15,02), "Net Invoice Amount",                         ~
               at (15,30), fac(hex(84)), netinv,     pic(-##,###,###.00),~
                                                                         ~
...
/*(AWD005)*/   at (14,49), "Energy Sur Percent=",                         ~
/*(AWD005)*/   at (14,70), fac(hex(84)), energy,         pic(-###,###.00),~               
               at (16,02), "A/R Type   ",                                ~
...
REM <AWD004> 
fix_billxref:
                if (stlmnt$ < "00000000") then return
                arimastr_key$ = str(cuscode$,,9) & stlmnt$
                read #15, key = arimastr_key$, using L50300, billxref$, ~
                                   eod goto L50300
L50300:         FMT POS(849), CH(09)
                return
REM </AWD004> 
...
L52320
*        Price Code Default                    PC$   /* (EWD) - Begin */
            if (pc$ >= "A" and pc$ <= "Z") or                            ~
               (pc$ >= "0" and pc$ <= "9") then L52345
...
L52900
*        ESC Account                            ESCACCT$
            escacctdescr$ = hex(06) & "Select Sales Discounts Account."
            call "GETCODE" (#3, escacct$, escacctdescr$, 0%, 0, f1%(3))
            if f1%(3) = 1% then return
                errormsg$ = "Invalid Energy Surcharge Account"
                return                
...
L53245
*        Part Description                      DESCR$()  EWD001 - Begin
           if descr$(c%) <> " " then return
           err% = 0%
           call "APCDESCR" (part$(c%), apc_scr$, apc_prt$, apc_sze$, #51,    ~
                                                                  err% )
           call "SPCESMSH" (apc_prt$,1%)
           call "SPCESMSH" (apc_sze$,1%)

           descr$(c%) = apc_prt$ & apc_sze$
            return                                    /* EWD001 - End */
