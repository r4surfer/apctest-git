        REM *************************************************************~
            *                                                           *~
            *  Program Name      - AWDPL604                             *~
            *  Creation Date     - 06/27/2008                           *~
            *  Written By        - David Speight                        *~
            *                                                           *~
            *  Description       - New Kanban cards  (laserprinter)     *~
            *                                                           *~
            *-----------------------------------------------------------*~
            *                  M O D I F I C A T I O N S                *~
            *---WHEN---+----------------WHAT----------------------+-WHO-*~
            *06/27/2008! (New) Program For Kanban cards           ! DES *~
            *************************************************************
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim apc$40, pname$21                                              
            apc$   = "(AWD) Generate Kanban Cards      "
            pname$ = "AWDPL604 - 07/02/2008"              /* (AWD004)  */
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            select #2,   "LANDRU", varc, consec,recsize=128 
...
browse_next:
	...
	str(aes_key$,26,6) = aes_seq$       
   	read #1, key 1% > aes_key$, USING KANBANFMT,              ~  
	           aes_part$, aes_seq$, aes_desc$, aes_pour$, aes_qty$,    ~ 
		   eod goto L21175
...
REM         if u3% = 1% then return
	...
	push_on_stack
	    ...
	    if queue_cnt% > 0% then skip_set 
            yellow% = 0%
            if aes_pour$ = "INVY" then yellow% = 1%
            if aes_pour$ = "TEXAS" then yellow% = 2%
skip_set:
            if aes_pour$ = "TEXAS" and yellow% = 0% then goto stack_mismatch
            if aes_pour$ =  "INVY" and yellow% = 0% then goto stack_mismatch
            if aes_pour$ <> "INVY" and yellow% = 1% then goto stack_mismatch
            if aes_pour$ <> "TEXAS" and yellow% = 2% then goto stack_mismatch
...
stack_mismatch:
	...
	print_stack
        ...
        deffn'101(fieldnr%, edit%)
              ...
              gosub set_pf1
	      gosub L40165
	      queue_msg$ = "000 white  cards to be printed."
	      if queue_cnt% = 1% then str(queue_msg$,16,1) = " "
	      convert queue_cnt% to str(queue_msg$,1,3), pic (##0)     
	      if yellow% = 1% then str(queue_msg$,5,6) = "yellow"
	      if yellow% = 2% then str(queue_msg$,5,6) = "blue  "
L40150:
...
L50080: Rem Part Number                                           
	...
	aes_key$ = aes_part$ & "            " 
   	     read #1,key 1% >= aes_key$, USING KANBANFMT,              ~  
	           aes_part$, aes_seq$, aes_desc$, aes_pour$, aes_qty$,    ~ 
		   eod goto L50085
KANBANFMT: FMT CH(25), CH(06), CH(64), CH(16), CH(06)                       
...
L50090: Rem Part Number                                           
        init(" ") aes_descr$
   	     read #1,key 0% = aes_seq$, USING KANBANFMT,              ~  
	           aes_part$, aes_seq$, aes_desc$, aes_pour$, aes_qty$,    ~ 
		   eod goto L50095
        return
...
L01100:    open nodisplay #2, output, space = 100%,                    ~
		   ...
		   if aes_seq$ <= "      " then goto skip_write
             init(" ") seq_rec$
             seq_rec$ = aes_part$ & '|"' & aes_desc$ & '"|' & aes_qty$ &  ~
			"|" & aes_pour$ & "|" & aes_seq$ & "||"
