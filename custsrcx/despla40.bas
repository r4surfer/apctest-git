        REM *************************************************************~
            ...
            *                      Exact Same Program as (APCPLA40)     *~
            *                                                           *~
            ...
            *          !       range specified                    !     *~
            * 04/05/05 ! (AWD012) Mod to the reclen of APCPLNSD   ! CMG *~
            * 10/26/06 ! (AWD013) Mod for RGA                     ! DES *~
            * 02/20/08 ! (AWD014) scanned for dept 032 st 15      ! DES *~
            *************************************************************
        ...
        dim                              /* (APCPLNDT) - FILE          */~
            ...
            dt_date$6, dt_date1$8,    /* Scheduled/Scanned Date (Y2K, LDJ)*/~
            tmp_date$6,               /* Scheduled/Scanned Date (Y2K, LDJ)*/~
            ...
            dt_dte$6, dt_dte1$8,         /* Production Date  (Y2K, LDJ)*/~
            dt_dte1x$8,                  /* Production Date  (Y2K, LDJ)*/~
        ...
        dim                              /* (Program) - Variables      */~
            ...
            h1$(6%)50, h2$(6%)50,        /* Screen/Report Headers      */~
            h3$(6%)40, h4$(6%)40,        /* Screen/Report Headers      */~
            h5$79, h6$(6%)40,            /* Screen/Report Headers      */~
            tab$(6%)9,                   /* Lookup Table Names         */~
            ...
            sc_date$10, sc_dte$6,     /* Beginning Production Date (Y2K, LDJ)*/~
            ed_date$10, ed_dte$6,        /* Ending Production Date (Y2K, LDJ)*/~
            ...
            sav_dte$6, sav_date$10,  /* Save Production Date Totals(Y2K, LDJ)*/~
            ...
            x$10, datex$10,            /* Temporary Date variables (Y2K, LDJ)*/~
        ...
        dim c1$(6000%)5, count$26,       /* Product Sequence No.       */~
            ...
            c4$(6000%)9,                 /* Customer Code              */~
            c5$(6000%)5,                 /* Load Number                */~
            c6$(6000%)6,                /* Production Date (EWD006)(Y2K, LDJ)*/~
            c7$(6000%)5,                 /* Parent Load     (EWD009)   */~
            bo$(6000%)1,                 /* BackOrder Code (EWD006)    */~
            tt_dte$6, bb_dte$6, ee_dte$6,/* Calc Scanned Dates         */~
            ap$2, jdate1$7, jdate2$7     /* 'AM/PM' - Julian Dates              (Y2K, LDJ)*/
        dim des_key$33,tmp1$18, tmp2$18  /* AWD014 */
	dim des_tbl$(1000%)18            /* AWD014 */
	dim des_st$2, des_bar$18, logmsg$256
...
REM                     keypos = 19,   keylen = 31,                      ~
REM                     alt key 1, keypos =  1, keylen = 31
...
/* (AWD012) - Mod to key and reclen */
        ...
        REM *************************************************************~
          ...
          sr$(6%) = "(6) - Production In RGA       "

          tab$(1%) = "PLAN DEPT" : tab$(4%) = "PLAN SHFT"
          tab$(2%) = "PLAN PROC"

          h1$(1%) = "New-Production Detail Screen'Not Scanned'-APCPLA40"
          h1$(2%) = "New-Stock Detail Screen 'Not Scanned'    -APCPLA40"
          h1$(3%) = "New-Production Detail Screen 'Scanned'   -APCPLA40"
          h1$(4%) = "New-Stock Detail Screen   'Scanned'      -APCPLA40"
          h1$(5%) = "New-Prod Detail (No Stock) 'Not Scanned' -APCPLA40"
          h1$(6%) = "New-Production Detail Screen'In RGA'     -APCPLA40"

          h2$(1%) = "New-Production Summary Screen'Not Scanned-APCPLA40"
          h2$(2%) = "New-Stock Summary Screen  'Not Scanned'  -APCPLA40"
          h2$(3%) = "New-Production Summary Screen  'Scanned' -APCPLA40"
          h2$(4%) = "New-Stock Summary Screen    'Scanned'    -APCPLA40"
          h2$(5%) = "New-Prod Summary (No Stock) 'Not Scanned'-APCPLA40"
          h2$(6%) = "New-Production Summary Screen'In RGA'    -APCPLA40"
          ...
          h3$(6%) = "Build Screen for 'In RGA' Product"
          ...
          h4$(6%) = "Printing Production'In RGA'Report"
          ...
          h6$(6%) = "Scanning Report of Products*In RGA     *"
...
*        Define the Input Message for the Screen/Field Indicated
     ...
     "Enter Rpt Select,1=Not Scan,2=Not Stock,3=Scan,4=StK,5=Not SO,6=RGA",~
 ...
 /* (Y2K, LDJ) */
        ...
        dataload
            ...
            call "DATEFMT" (dt_date1$)             /* (EWD006)         */       /* (Y2K, LDJ) */
            dt_dte1$ = dt_dte$
            call "DATEFMT" (dt_dte1$)              /* Scan Date  EWD006*/       /* (Y2K, LDJ) */
            dt_dte1x$ = dt_dte$                                                 /* (Y2K, LDJ) */
            call "DATEFMT" (dt_dte1x$)                                          /* (Y2K, LDJ) */
...
/*AWD014*/  if sc_sel% = 3% and sc_dept$ = "032" then goto L30261
/*AWD013*/  if sc_sel% <> 1% or sc_dept$ <> "032" then goto L30262     
            for i% = 1% to supp_max%                     /* Find Dept. */
                if dt_dept$ = supp$(i%) then goto L30360    /* Subscript  */
            next i%
            if dt_st$ = "15" then goto L30263
            if dt_st$ > "11" or dt_dept$ <> "032" then goto L30360
            goto L30263

L30261:
/* ??? */   des_bar$ = dt_bar$                   /* AWD014 */
            gosub check_scanned_rga              /* AWD014 */
            if des_sw > 0.0 then goto L30360
            return     
            if des_sw > 0.0 then goto L30263
            goto L30360

L30262:     if dt_st$ > "11" then goto L30360    /* Not Scanned Values  */
            if str(sc_dept$,1%,3%) = "ALL" and dt_dept$ = "001"        ~
                      then goto L30264          /* (EWD005)            */
L30263:        nn_tot%(tt%) = nn_tot%(tt%) + 1% /* Not Scanned Total   */
L30264:        if str(sc_dept$,1%,3%) = "ALL" and dt_dept$ = "001"     ~
...
L30490:
            if sc_dept$ = "ALL" then return         /* Totals Only     */
...
L40230:     accept                                                       ~
               ...
               at (03,30), fac(lfac$(1%)), sc_date$             , ch(10),       /* (Y2K, LDJ) */~
                                                                         ~
               at (03,42), fac(hex(84)), ed_prompt$             , ch(23),       /* (Y2K, LDJ) */~
               at (03,67), fac(lfac$(1%)), ed_date$             , ch(10),       /* (Y2K, LDJ) */~
                                                                         ~
               at (04,02), "Selection Codes (1 thru 5):",                ~
               at (04,30), fac(lfac$(2%)), sc_sel$              , ch(01),~
               at (04,42), fac(hex(84)), sc_sel_d$              , ch(30),       /* (Y2K, LDJ) */~
                                                                         ~
               at (05,02), "Shift Code or (AL) = All  :",                ~
               at (05,30), fac(lfac$(3%)), sc_id$               , ch(02),~
               at (05,42), fac(hex(84)), sc_id_d$               , ch(30),       /* (Y2K, LDJ) */~
                                                                         ~
               at (06,02), "Prod. Dept. Code or (ALL) :",                ~
               at (06,30), fac(lfac$(4%)), sc_dept$             , ch(03),~
               at (06,42), fac(hex(84)), sc_dept_d$             , ch(30),       /* (Y2K, LDJ) */~
                                                                         ~
               at (07,02), "Beg. Prod. Load or (ALL)  :",                ~
               at (07,30), fac(lfac$(5%)), sc_load$             , ch(05),~
               at (07,42), "End Load:",                                         /* (Y2K, LDJ) */~
               at (07,52), fac(lfac$(5%)), ed_load$             , ch(05),       /* (Y2K, LDJ) */~
                                                                         ~
...
L40830:        if keyhit% <> 11% then goto L40870
                 if mod(dt_sc%,2%) = 0% then dt_sc% = 1% else dt_sc% = 2%
                  goto L40070
...
L42050:     gosub set_pf2
            accept                                                       ~
               ...
               at (01,73), fac(hex(8c)), date$                  , ch(08),~
                                                                         ~
               at (03,02), fac(hex(84)), bg_prompt$             , ch(26),~
               at (03,30), fac(hex(84)),   sc_date$             , ch(10),       /* (Y2K, LDJ) */~
                                                                         ~
               at (03,42), fac(hex(84)), ed_prompt$             , ch(23),       /* (Y2K, LDJ) */~
               at (03,67), fac(hex(84)),   ed_date$             , ch(10),       /* (Y2K, LDJ) */~
                                                                         ~
               ...
               at (07,42), "End Load:",                                         /* (Y2K, LDJ) */~
               at (07,52), fac(hex(84)), ed_load$               , ch(05),       /* (Y2K, LDJ) */~
                                                                         ~
               at (08,02),                                               ~
               ...
               at (09,01),                                               ~
                  " ----- ------------------------- ------------------",~
               at (08,53),                                               ~
                  "Cust.     Load  Prd Dt PLoad",                                   /* (Y2K, LDJ) */~
               at (09,53),                                               ~
                  "--------- ----- ------ -----",                                  /* (Y2K, LDJ) */~
/*(EWD006)*/   at (10,01), fac(hex(84)), bo$(i%  )              , ch(01),~
               at (10,02), fac(hex(84)), c1$(i%  )              , ch(05),~
               at (10,08), fac(hex(84)), c2$(i%  )              , ch(25),~
               at (10,34), fac(hex(84)), c3$(i%  )              , ch(18),~
               at (10,53), fac(hex(84)), c4$(i%  )              , ch(09),~
               at (10,63), fac(hex(84)), c5$(i%  )              , ch(05),~
               at (10,69), fac(hex(84)), c6$(i%  )              , ch(06),       /* (Y2K, LDJ) */~
/*(EWD009)*/   at (10,76), fac(hex(84)), c7$(i%  )              , ch(05),~
                                                                         ~
/*(EWD006)*/   at (11,01), fac(hex(84)), bo$(i%+1%)             , ch(01),~
               at (11,02), fac(hex(84)), c1$(i%+1%)             , ch(05),~
               at (11,08), fac(hex(84)), c2$(i%+1%)             , ch(25),~
               at (11,34), fac(hex(84)), c3$(i%+1%)             , ch(18),~
               at (11,53), fac(hex(84)), c4$(i%+1%)             , ch(09),~
               at (11,63), fac(hex(84)), c5$(i%+1%)             , ch(05),~
               at (11,69), fac(hex(84)), c6$(i%+1%)             , ch(06),       /* (Y2K, LDJ) */~
/*(EWD009)*/   at (11,76), fac(hex(84)), c7$(i%+1%)             , ch(05),~
                                                                         ~
/*(EWD006)*/   at (12,01), fac(hex(84)), bo$(i%+2%)             , ch(01),~
               at (12,02), fac(hex(84)), c1$(i%+2%)             , ch(05),~
               at (12,08), fac(hex(84)), c2$(i%+2%)             , ch(25),~
               at (12,34), fac(hex(84)), c3$(i%+2%)             , ch(18),~
               at (12,53), fac(hex(84)), c4$(i%+2%)             , ch(09),~
               at (12,63), fac(hex(84)), c5$(i%+2%)             , ch(05),~
               at (12,69), fac(hex(84)), c6$(i%+2%)             , ch(06),       /* (Y2K, LDJ) */~
/*(EWD009)*/   at (12,76), fac(hex(84)), c7$(i%+2%)             , ch(05),~
                                                                         ~
/*(EWD006)*/   at (13,01), fac(hex(84)), bo$(i%+3%)             , ch(01),~
               at (13,02), fac(hex(84)), c1$(i%+3%)             , ch(05),~
               at (13,08), fac(hex(84)), c2$(i%+3%)             , ch(25),~
               at (13,34), fac(hex(84)), c3$(i%+3%)             , ch(18),~
               at (13,53), fac(hex(84)), c4$(i%+3%)             , ch(09),~
               at (13,63), fac(hex(84)), c5$(i%+3%)             , ch(05),~
               at (13,69), fac(hex(84)), c6$(i%+3%)             , ch(06),       /* (Y2K, LDJ) */~
/*(EWD009)*/   at (13,76), fac(hex(84)), c7$(i%+3%)             , ch(05),~
                                                                         ~
/*(EWD006)*/   at (14,01), fac(hex(84)), bo$(i%+4%)             , ch(01),~
               at (14,02), fac(hex(84)), c1$(i%+4%)             , ch(05),~
               at (14,08), fac(hex(84)), c2$(i%+4%)             , ch(25),~
               at (14,34), fac(hex(84)), c3$(i%+4%)             , ch(18),~
               at (14,53), fac(hex(84)), c4$(i%+4%)             , ch(09),~
               at (14,63), fac(hex(84)), c5$(i%+4%)             , ch(05),~
               at (14,69), fac(hex(84)), c6$(i%+4%)             , ch(06),       /* (Y2K, LDJ) */~
/*(EWD009)*/   at (14,76), fac(hex(84)), c7$(i%+4%)             , ch(05),~
                                                                         ~
/*(EWD006)*/   at (15,01), fac(hex(84)), bo$(i%+5%)             , ch(01),~
               at (15,02), fac(hex(84)), c1$(i%+5%)             , ch(05),~
               at (15,08), fac(hex(84)), c2$(i%+5%)             , ch(25),~
               at (15,34), fac(hex(84)), c3$(i%+5%)             , ch(18),~
               at (15,53), fac(hex(84)), c4$(i%+5%)             , ch(09),~
               at (15,63), fac(hex(84)), c5$(i%+5%)             , ch(05),~
               at (15,69), fac(hex(84)), c6$(i%+5%)             , ch(06),       /* (Y2K, LDJ) */~
/*(EWD009)*/   at (15,76), fac(hex(84)), c7$(i%+5%)             , ch(05),~
                                                                         ~
/*(EWD006)*/   at (16,01), fac(hex(84)), bo$(i%+6%)             , ch(01),~
               at (16,02), fac(hex(84)), c1$(i%+6%)             , ch(05),~
               at (16,08), fac(hex(84)), c2$(i%+6%)             , ch(25),~
               at (16,34), fac(hex(84)), c3$(i%+6%)             , ch(18),~
               at (16,53), fac(hex(84)), c4$(i%+6%)             , ch(09),~
               at (16,63), fac(hex(84)), c5$(i%+6%)             , ch(05),~
               at (16,69), fac(hex(84)), c6$(i%+6%)             , ch(06),       /* (Y2K, LDJ) */~
/*(EWD009)*/   at (16,76), fac(hex(84)), c7$(i%+6%)             , ch(05),~
                                                                         ~
/*(EWD006)*/   at (17,01), fac(hex(84)), bo$(i%+7%)             , ch(01),~
               at (17,02), fac(hex(84)), c1$(i%+7%)             , ch(05),~
               at (17,08), fac(hex(84)), c2$(i%+7%)             , ch(25),~
               at (17,34), fac(hex(84)), c3$(i%+7%)             , ch(18),~
               at (17,53), fac(hex(84)), c4$(i%+7%)             , ch(09),~
               at (17,63), fac(hex(84)), c5$(i%+7%)             , ch(05),~
               at (17,69), fac(hex(84)), c6$(i%+7%)             , ch(06),       /* (Y2K, LDJ) */~
/*(EWD009)*/   at (17,76), fac(hex(84)), c7$(i%+7%)             , ch(05),~
                                                                         ~
/*(EWD006)*/   at (18,01), fac(hex(84)), bo$(i%+8%)             , ch(01),~
               at (18,02), fac(hex(84)), c1$(i%+8%)             , ch(05),~
               at (18,08), fac(hex(84)), c2$(i%+8%)             , ch(25),~
               at (18,34), fac(hex(84)), c3$(i%+8%)             , ch(18),~
               at (18,53), fac(hex(84)), c4$(i%+8%)             , ch(09),~
               at (18,63), fac(hex(84)), c5$(i%+8%)             , ch(05),~
               at (18,69), fac(hex(84)), c6$(i%+8%)             , ch(06),       /* (Y2K, LDJ) */~
/*(EWD009)*/   at (18,76), fac(hex(84)), c7$(i%+8%)             , ch(05),~
                                                                         ~
/*(EWD006)*/   at (19,01), fac(hex(84)), bo$(i%+9%)             , ch(01),~
               at (19,02), fac(hex(84)), c1$(i%+9%)             , ch(05),~
               at (19,08), fac(hex(84)), c2$(i%+9%)             , ch(25),~
               at (19,34), fac(hex(84)), c3$(i%+9%)             , ch(18),~
               at (19,53), fac(hex(84)), c4$(i%+9%)             , ch(09),~
               at (19,63), fac(hex(84)), c5$(i%+9%)             , ch(05),~
               at (19,69), fac(hex(84)), c6$(i%+9%)             , ch(06),       /* (Y2K, LDJ) */~
/*(EWD009)*/   at (19,76), fac(hex(84)), c7$(i%+9%)             , ch(05),~
                                                                         ~
                                                                         ~
               at (22,02), fac(hex(94)),   h5$                  , ch(79),~
...
/*AWD014*/  if sc_sel% = 3% and sc_dept$ = "032" then xx% = 1%    
            ...
            if sc_sel% > 2% and sc_sel% < 5% then goto L43700
               ...
               x2% = nn%(xx%,2%)                             /* Sashs  */
               x3% = nn%(xx%,3%)                             /* Parts  */
            if sc_sel% = 1% or sc_sel% = 5% then         ~
               h5$="Windows Not Scanned = XXXXX Sash = XXXX Parts = XXXX"~
                             else                                        ~
               h5$=" Stock  Not Scanned = XXXXX Sash = XXXX Parts = XXXX"

            convert x1% to str(h5$,23%,5%),pic(#####)

            convert x2% to str(h5$,36%,4%),pic(####)

            convert x3% to str(h5$,49%,4%),pic(####)
               goto L43840                                                ~
...
L44050:     gosub set_pf3
            accept                                                       ~
               ...
               at (21,40), fac(hex(84)), txt$(24% + dp%)        , ch(36),~
                                                                         ~
               at (22,02), fac(hex(94)),   inpmessage$          , ch(79),~
               at (23,02), fac(hex(8c)),   pf$(1%)              , ch(79),~
               at (24,02), fac(hex(8c)),   pf$(2%)              , ch(79),~
                                                                         ~
               keys(pfkeys$), key(keyhit%)
...
L44880:        close ws
        ...
        set_pf3
           ...
           inpmessage$ = "   S t o c k   N o t   S c a n n e d   - XXXXX"
               if sc_sel% = 6% then                      ~
           inpmessage$ = "   R G A               S c a n n e d   - XXXXX"
...
L45100:        for i% = 1% to dept_max%
           ...
           inpmessage$ = "S t o c k  P r o d u c t  S c a n n e d- XXXXX"
               if sc_sel% = 6% then                      ~
           inpmessage$ = "   R G A               S c a n n e d   - XXXXX"
...
L50190:     date% = 0%
            call "DATEOKC" (sc_date$, date%, errormsg$)                         /* (Y2K, LDJ) */
            if date% = 0% then return
            x$ = sc_date$
            call "DATUFMTC" (x$)                                                /* (Y2K, LDJ) */
            sc_dte$ = str(x$,1%,6%)
...
L50290:     date% = 0%
            call "DATEOKC" (ed_date$, date%, errormsg$)                         /* (Y2K, LDJ) */
            if date% = 0% then return
            x$ = ed_date$
            call "DATUFMTC" (x$)                                                /* (Y2K, LDJ) */
            ed_dte$ = str(x$,1%,6%)
...
L50450:     sc_sel% = 1%
            ...
            if sc_sel% < 1% or sc_sel% > 6% then goto L50510
               sc_sel_d$ = sr$(sc_sel%)
        return
...
L55290: %!Seq. !<--- Bar Code --->!Load ! Customer!Dep!Scan DTE!T i m e !Sf~
        ~t!Mdl!Status !
...
L55490: %! Total for all Departments     Windows: #####-  Sashs: ####-  P~
        ...
        load_screen
            ...
            des_bar$ = dt_bar$                /* AWD014 */
REM	    des_sw = 1.0
REM         if sc_sel% = 3% and sc_dept$ = "032" then gosub check_rga_table
REM	    if des_sw = 0.0 then return
            t_max% = t_max% + 1%
            c1$(t_max%) = dt_seq$
            c2$(t_max%) = dt_part$
            c3$(t_max%) = dt_bar$
            c4$(t_max%) = dt_cust$
            c5$(t_max%) = dt_load$
REM         c6$(t_max%) = dt_date1$
            str(tmp_date$,1%,2%) = str(dt_date1$,1%,2%)
            str(tmp_date$,3%,2%) = str(dt_date1$,4%,2%)
            str(tmp_date$,5%,2%) = str(dt_date1$,7%,2%)
            c6$(t_max%) = tmp_date$
            str(tmp_date$,1%,2%) = str(dt_date1$,1%,2%)
            str(tmp_date$,3%,2%) = str(dt_date1$,4%,2%)
            str(tmp_date$,5%,2%) = str(dt_date1$,7%,2%)
            c7$(t_max%) = pload$                               /*EWD009*/
REM         if sc_sel% > 2% and sc_sel% < 5% then c6$(t_max%) = dt_dte1$
            if sc_sel% > 2% and sc_sel% < 5% then c6$(t_max%) = tmp_date$
*       RHH
*       RHH   CONVERT CST(1%) TO C6$(T_MAX%), PIC(####.##-)
        ...
        write_work_file                                      /*  (EWD010)  */
              init(" ") wrk_date$
              wrk_date$ = dt_date$
              if sc_sel% > 2% and sc_sel% < 5% then wrk_date$ = dt_dte1$
...
screen_wrk_first
            ...
            c4$(t_max%) = dt_cust$
            c5$(t_max%) = dt_load$
REM         c6$(t_max%) = wrk_date1$
            str(tmp_date$,1%,2%) = str(wrk_date1$,1%,2%)
            str(tmp_date$,3%,2%) = str(wrk_date1$,4%,2%)
            str(tmp_date$,5%,2%) = str(wrk_date1$,7%,2%)
            c6$(t_max%) = tmp_date$
            c7$(t_max%) = pload$                             
...
L60443:       des_sw = 0.0  /* already printed */
              l% = des_cnt% + 1%

L60444:    next l%

L60445:    FMT CH(33), POS(32), CH(02)
        return

        check_backorder                  /*  (EWD006) */
           init(" ") sc_key$, sc_sav$, pload$
           str(sc_key$,1%,8%) = str(dt_key$,1%,8%)
           str(sc_key$,9%,2%) = str(dt_key$,9%,2%)
...
L60520:       print using L55350
              print using L55320, dt_seq$, dt_bar$, dt_load$, dt_cust$,   ~
                               dt_dept$, dt_dte1x$, dt_time$, dt_shft$,         /* (Y2K, LDJ) */~
                                 model$, dt_st_d$
...
REM         if rpt% = 0% then gosub load_screen_wrk                       ~
REM            else           gosub print_screen_wrk
REM         gosub print_totals
            sav_dte$ = sc_dte$
        generate_nxt
           tt_dte$ = sav_dte$
           gosub calc_scanned_units
           if sav_dte$ > ed_dte$ then goto generate_done
              goto generate_nxt
        generate_done
...
REM         if str(readkey$,10%,1%) <> "0" then goto load_dept_done
        ...
        calc_scanned_units
           ...
           yr% = 0%
           datex$ = tt_dte$                                                     /* (Y2K, LDJ) */
           call "DATFMTC" (datex$,yr%,x$)                                       /* (Y2K, LDJ) */
...
L62010:
           ...
           if mod(yr%,4) = 0% then leap_yr% = 366%  /* Leap Year       */
            ...
            call "DATE" addr("GJ",tt_dte$, str(jdate1$,,5%), x%)                /* (Y2K, LDJ) */
                                                         /* Convert to */       /* (Y2K, LDJ) */
                                                         /* Julian Date*/
            call "DATJULCV" (jdate1$)                                           /* (Y2K, LDJ) */
            convert str(jdate1$,5%,3%) to j1%, data goto L62140                 /* (Y2K, LDJ) */
L62140:
            ...
            if j2% <= leap_yr% then goto L62240
               j2% = 001%
            convert str(jdate2$,1%,4%) to rhh%, data goto L62210                /* (Y2K, LDJ) */
L62210:
            convert (rhh% + 1%) to str(jdate2$,1%,4%), pic(0000)                /* (Y2K, LDJ) */

L62240:     convert j1% to str(jdate1$,5%,3%), pic(000)                         /* (Y2K, LDJ) */

            convert j2% to str(jdate2$,5%,3%), pic(000)                         /* (Y2K, LDJ) */
                                               /* Begin with Production*/
            call "DATJULCV" (jdate1$)                                           /* (Y2K, LDJ) */
            call "DATJULCV" (jdate2$)                                           /* (Y2K, LDJ) */
            call "DATE" addr("JG", str(jdate1$,,5%), bb_dte$, x%) /* Date and   */      /* (Y2K, LDJ) */
            call "DATE" addr("JG", str(jdate2$,,5%), ee_dte$, x%) /*Next Days   */      /* (Y2K, LDJ) */
                                                                  /*Prod. Date  */      /* (Y2K, LDJ) */
...
/*AWD013*/ if sc_dept$ = "032" and sc_sel$ = "1" then goto calc_scanned_first
/*AWD014*/ if sc_dept$ = "032" and sc_sel$ = "3" then goto calc_scanned_first
           if str(sc_dept$,1%,3%) = "ALL" then goto calc_scanned_first
        ...
        calc_scanned_nxt
/*AWD014*/ if sc_dept$ = "032" and sc_sel$ = "3" then goto calc_scanned_first
           read #1, using L62450, ad_rec$, eod goto calc_scanned_done
L62450:        FMT CH(64)
        calc_read_first
           ad_key$ = str(ad_rec$,19%,33%)
           if rpt% <> 0% then goto L62540
              count% = count% + 1%
...
L62540: REM INIT(" ") DB$
        ...
        REM  CHECK DEPARTMENT
   	   if sc_dept$ = "032" and sc_sel$ = "1" then goto L62710 /*AWD013*/
   	   if sc_dept$ = "032" and sc_sel$ = "3" then goto L62710 /*AWD014*/
           if sc_dept$ = "ALL" then goto L62710
              if sc_dept$ <> ad_dept$ then goto calc_scanned_nxt


L62710: REM  CHECK SHIFT
L62720:    if sc_id$ = "AL" then goto L62740    /* Shift Not Specified */
              if sc_id$ <> ad_shft$ then goto calc_scanned_nxt

/* <AWD013> */
L62740:                                                             
/*AWD014*/  if sc_sel$ = "3" and sc_dept$ = "032" then goto L62810 
            if sc_sel$ <> "1" or sc_dept$ <> "032" then goto L62750 
            for i% = 1% to supp_max%                     /* Find Dept. */
                if ad_dept$ = supp$(i%) then goto calc_scanned_nxt         
            next i%
            if ad_st$ = "15" then goto L62810
            if ad_st$ < "12" and ad_dept$ = "032" then goto L62810          
            goto calc_scanned_nxt
/* </AWD013> */

L62750:    if p_scan% <> 0% then goto L62790
                                                        /* Not Scanned */
              if ad_st$ > "03" and ad_st$ < "12" then goto L62810
                 goto calc_scanned_nxt
L62790:    if ad_st$ <> "12" then goto calc_scanned_nxt     /*Scanned  */

L62810:    init(" ") dt_key$, dt_rec$
...
L62870:       FMT CH(256)                       /* Check Load Selection*/
           if str(sc_load$,1%,3%) = "ALL" then goto L62920
              if str(dt_rec$,1%,5%) < sc_load$ or                        ~
                 str(dt_rec$,1%,5%) > ed_load$ then goto calc_scanned_nxt

/* <AWD013> */
L62920:                                                             
           if sc_sel$ = "3" and sc_dept$ = "032" then goto L62921 
           if sc_sel$ <> "1" or sc_dept$ <> "032" then goto L62930 
            for i% = 1% to supp_max%                     /* Find Dept. */
                if str(dt_rec$,42,3) = supp$(i%) then goto calc_scanned_nxt
            next i%
           if str(dt_rec$,42,3) = "032" and str(dt_rec$,64,2) < "12" then ~
                   goto L62940
           if str(dt_rec$,64,2) = "15" then                             ~
                   goto L62940
           goto calc_scanned_nxt
/* </AWD013> */

L62921:
/* ??? */   des_bar$ = str(dt_rec$,24,18)        /* AWD014 */
            gosub check_scanned_rga              /* AWD014 */
            if des_sw > 0.0 then goto L62940
            goto calc_scanned_nxt

L62930:    if p_scan% <> 0% then goto L62940     /* Goto Scanned Product*/
...
LS2:    return
        ...
        open_work_file                                     /* (EWD010)  */
             ...
             call "WORKOPEN" (#10, "IO", 500%, f2%(10))
                if f2%(10) = 1% then no_work_file   /* Get out if open fails */
                workfile% = 1%
        no_work_file        
        return                                             /* (EWD010)  */      
        REM *************************************************************~
