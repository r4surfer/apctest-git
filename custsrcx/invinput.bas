        REM *************************************************************~
            *                                                           *~
            *  IIIII  N   N  V   V  IIIII  N   N  PPPP   U   U  TTTTT   *~
            *    I    NN  N  V   V    I    NN  N  P   P  U   U    T     *~
            *    I    N N N  V   V    I    N N N  PPPP   U   U    T     *~
            *    I    N  NN   V V     I    N  NN  P      U   U    T     *~
            *  IIIII  N   N    V    IIIII  N   N  P       UUU     T     *~
            *                                                           *~
            *-----------------------------------------------------------*~
            * INVINPUT - Add / Change Records (Parts) in the Inventory  *~
            *            Master File (INVMASTR).  Provides Add/Change/  *~
            ...
            *---WHEN---+----------------WHAT----------------------+-WHO-*~
            * 01/23/06 ! ORIGINAL         CR347 PAR000            ! CMG *~
            CAELUSASSOCIATESSPOKANEWASHINGTONALLRIGHTSRESERVEDCAELUSASSOC

        dim                                                              ~
            apc_scr$120,                 /* Screen Description  (EWD)  */~
            apc_prt$60,                  /* Printer Description (EWD)  */~
            sub_scr$120,                 /* Screen Description  (EWD)  */~
            sub_prt$60,                  /* Printer Description (EWD)  */~
            apc_sze$20,                  /* Long Form of Size   (EWD)  */~
            ...
            altpart$(100)45,             /* Alternate Part #s          */~
            ...
            desc$32,                     /* Descrip for Units Calc(EWD)*/~
            ...
            junkkey$80,                  /* Used in PLOWNEXT Routine   */~
            lastpart$45,                 /* Last Part Number input.    */~
            ...
            part$45,                     /* Part Number being input.   */~
            ...
            spart$45, sdescr$32,         /* Save part, Description     */~
            ...
            testget$(9)100,              /* Test string of INVINPUT Rec*/~
            testput$(9)100,              /* Test string of INVINPUT Rec*/~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim cms2v$50
            cms2v$ = "R6.04.02 02/02/06 Precious Metals (EWD) Mods     "
        REM *************************************************************
            mat f2% = con
        ...
        REM *************************************************************~
            ...
            *  #2 ! INVMASTR ! Inventory Master File   FINISHED GOODS   *~
            ...
            * #17 ! INVQUAN  ! Inventory Quantities File FINISHED GOODS *~
            ...
            * #25 ! AMTBOMIF ! Master Validity File      (EWD) Mod      *~  
            *************************************************************
...
L09140:     switchkey$ = "SWITCHS.COR"
            ...
            str(line2$,62%) = "INVINPUT: " & str(cms2v$,,8%)
            init(hex(01)) edttran$
...
*        Check For Inventory Defaults
            ...
            if dflts% = 1% then get #2, str(dflts$(),,1024)  /* PAR000 */
...
*        Get any maintenance required for the Appendix file
            call "HNYAPPND" (3%, str(part$,1%,25%), u3%) /*PAR000*/
            message$ = " "
            goto editmode
...
L12660:     next fieldnr%
        ...
        edit_appendix
L12685:     call "HNYAPPND" (4%, str(part$,1%,25%), err%) /*PAR000*/
...
*        Update screen display variables
            lastpart$ = part$
            lastsubpart$ = subpart$                 /* CR347 */
            if str(part$,,1) = hex(00) then lastpart$ = "Defaults"
* CR347 Mod            
            if str(part$,,1) = hex(00) then lastsubpart$ = " "
*        ...and Return
            goto inputmode
...
* !!! Category must not be blank for INVMASTR
...
L22420:              FMT POS(131), CH(1), CH(6), CH(1), PD(14,4)  /*PAR000*/
...
L22690:         FMT POS(151), CH(1)    /*PAR000*/
...
L22750:         FMT POS(152), CH(1)             /*PAR000*/
...
L22810:         FMT POS(223), PD(14,4)         /*PAR000*/
...
*        Default for DRAWING SIZE & REFERENCE
            if dflts% = 1% then get str(dflts$()) using L22890,           ~
                                                    draw_size$, draw_ref$
L22890:         FMT POS(157), CH(2), CH(16)        /*PAR000*/
            return
...
*        Default for Max. Receipt over Order Percent & Units
            if dflts% = 1% then get str(dflts$()) using L22940, ovrrecpct,~
                ovrrecqty
L22940:         FMT POS(231), PD(14,4), PD(14,4)       /*PAR000*/
...
L23060:         FMT POS(262), CH(1), CH(1), POS(354), CH(1)  /*PAR000*/
            return
...
L23120:         FMT POS(148), BI(2)         /*PAR000*/
...
*        Default for Overshipment Percent & Units
            if dflts% = 1% then get str(dflts$()) using L23180, ovrshppct,~
                ovrshpqty
L23180:         FMT POS(742), PD(14,4), PD(14,4) /*PAR000*/
...
*        Default for Default Formula Name
            if dflts% = 1% then get str(dflts$()) using L23248,           ~
                formula_name$
L23248:         FMT POS(878), CH(16)      /*PAR000*/
...
*        Part already on file
            ...
            call "PLOWNEXT" (#5, plowkey$, 45%, alts%)

            if copy% = 0% then retrieved% = 1%

            /* Capture record for later test for change */
            if f1%(2%)  = 1% then gosub get_invmastr


            if copy% = 0% then L30290
               vfs$ = " "             /* FOR COPY CLEAR VAR FLDS (EWD)*/
...
*        Format for Inventory Master File (INVMASTR)
L31020:     FMT XX(45),          /* Part and SubPart Number  CR347     */~
                CH(32),          /* Description                        */~
...
*        Next do Generic Xref
            genfiller$ = " "
            call "READ101" (#9, str(part$,1%,25%), f1%(9%))
...
*        Convert Alphas to Numerics
            ...
            junkkey$ = part$
                                         /* (EWD) - Begin Mods         */
            er% = 0%                     /* SPECIAL APC MOD - 11/08/90 */
            if len(part$) < 19% then goto L32410  /* BUILD DESCRIPTIONS */
               call "AWDDESCR" (str(part$,1%,25%), str(part$,26%,20%),   ~
                                  apc_scr$, apc_prt$, sub_scr$, sub_prt$,~
                                     apc_sze$, #25,er% )
               str(description$,17%,16%) = apc_sze$
               str(vfs$,101%,60%) = apc_prt$
               str(vfs$,161%,20%) = apc_sze$
               gosub calculate_units
                                         /* (EWD) - End Mods           */
L32410:     
            gosub save_invmastr            /*PAR000*/


*        We do NOT want to create a PIP for generic or nonplan parts
*        If STR(PTYPE$,1,3) < "200" THEN GOTO 31455 & Much Trouble
          if dfltsedit% = 1% then return         /*PAR000*/
            call "READ101" (#12, str(part$,1%,25%), f1%(12))
...
L32990:              FMT CH(1), CH(25), POS(1987), 4*PD(14,4), 3*BI(2)
                ...
                call "PIPFLAGS" (str(part$,1%,25%), 1%, today%, 0, #12, #18)
                return            /*PAR000*/
...
L32420:     put #2 using L33070, part$, description$, generic$,           ~
            ...
            if dfltsedit% = 1% and savedefaults% <> 16% then L32570
                ...
                call "HNYAPPND" (2%, str(part$,1%,25%), err%)  /*PAR000*/
...
*        INVMASTR format
L33070:     FMT CH(45),              /* Part and subpart Number CR347  */~
                CH(32),              /* Description                    */~
...
L33470:     FMT CH(25),              /* Part Number                    */~
        ...
        bal_pip
            ...
            init (hex(00)) junkkey$                 /*PAR000*/
            str(junkkey$,,25) = str(part$,,25)      /* PIP leave at 25 */
...
L33680:     init (hex(00)) junkkey$                /*PAR000*/
            str(junkkey$,,25) = str(part$,,25)     /*PIP leave at 25 */
...
L40230:     accept                                                       ~
               ...
               at (05,30), fac(fac$( 1%)), str(part$,1% ,25%)   , ch(25),~
               at (05,60), fac(fac$( 1%)), str(part$,26%,25%)   , ch(20),~
...
L50728:     if ptype$ = "?" and part_type_chk$ = "Y" then L50742
            ...
            convert ptype% to ptype$, pic(000)
* PAR000    finished goods must be 500 or over!
            if ptype% < 500% or ptype% > 999% then L50840
...
/*PAR000*/  str(junkkey$,42,20) = str(subpart$)
...
* CR347 Sub part number 
            if len(part$) >= 19% and subpart$ <> "  " and               ~
                                           category$ <>  " " then return
            ...
            if fs%(6%) = 0% then                                         ~
                    call "OPENCHCK" (#6, fs%(6%), f2%(6%), 0%, rslt$(6%))
            call "HNYPRCSB" (vencode$, str(part$,1%,25%) , 0%, #7, #2, #6, #8)
            return                       /*PAR000*/
...
L55250:         vfv$(i%) = vff$(i%)
        ...
        view_add_pm_items
            if pm_on$ <> "Y" then return      /*PAR000*/
            call "PMPTBLSB"( str(part$,1%,25%), #2, #20,"S")
            return
        ...
        calculate_units                  /* ONLY MANUFACTURED PARTS */
            ...
            call "DESCRIBE" (#20, readkey$, desc$, 0%, f1%(20%))
            if f1%(20%) = 0% then return           /* NO FACTOR DEFINED*/
            if str(desc$,21%,1%) <> " " then goto L60080
               str(desc$,21%,5%) = "00.00"
                                         /* Convert Width  */
L60080:     convert str(part$,13%,4%) to a1%,data goto L60090
L60090:                                  /* Convert Height */
            convert str(part$,17%,3%) to a2%,data goto L60110
L60110:                                  /* Always Round Up*/
            if str(part$,16%,1%) <> "0" then a1% = a1% + 10%
            if str(part$,19%,1%) <> "0" then a2% = a2% + 10%
                                    /* UNIT_I% = UNITED INCHES       */
            unit_i% = int(a1%/10) + int(a2%/10)
            convert str(desc$,21%,5%) to fact, data goto L60170
L60170:
            fact = round(fact,2)
