        REM *************************************************************~
            *                                                           *~
            *  IIIII  N   N  V   V   AAA   DDDD   DDDD   JJJJJ  N   N   *~
            *    I    NN  N  V   V  A   A  D   D  D   D    J    NN  N   *~
            *    I    N N N  V   V  AAAAA  D   D  D   D    J    N N N   *~
            *    I    N  NN   V V   A   A  D   D  D   D  J J    N  NN   *~
            *  IIIII  N   N    V    A   A  DDDD   DDDD    J     N   N   *~
            *                                                           *~
            *-----------------------------------------------------------*~
            * INVADDJN - Journal program for Inventory Additions. Prints*~
            ...
            * 01/30/06 ! (PAR000) CR347 Modfor New Part Number    ! RHH *~ 
        ...
        REM *************************************************************~
            ...
            * #11 ! INVPOOL  ! Inventory pools file             (PAR000)*~
            ...
            * #50 ! WRKFILE  ! INVPST2 work file (HNYADJPF)             *~
            ...
            *************************************************************
            ...
            call "PLOWNEXT" (#5, nextitemkey$, 3%, f1%(5%))
                 if f1%(5%) = 0 then L40000
                                                          /* (PAR000)  */
            call "INVADJOB" (date$,      /* Current System Date        */~
...
L10460:     REM POST QUANTITY ADDED TO QUANTITY ON HAND ("HNYPOST")
            ...
            if core_on% <> 1% then L10620          /* (PAR000) No Change*/
                call "CORVALSB" ("IA", str(part$,1%,25%), store$, lot$, quantity,    ~
...
L10620:     gosub consolidate_array_and_post
            ...
            call "LOTTRACK"                                              ~
                 ("D",             /* FROM FLAG  H=INVNTORY,V=VEN,C=CUS*/~
                  ...
                  str(part$,1%,25%),/* TO PART(IF BLANK THEM SAME)(PAR000)*/~
                  lot$,            /* TO LOT                           */~
                  store$,          /* TO STORE                         */~
                  " ",             /* TO WHATEVER                      */~
                  quantity,        /* QUANTITY MOVED                   */~
                  #3,              /* 'INVMASTR' FILE          (PAR000)*/~
                  #7)              /* 'SYSFILE2' FILE                  */
                                                   /* (PAR000)    ???  */
L10800:     procsrce$=" "
            ...
            if procsrce$ <> " " then L10830
                if vendor$ = " " and ponr$ = " " then L10830
                    procsrce$ = str(vendor$) & "P" & str(ponr$)
                                                   ...
                                                   /* May need to Chg  */
                                                   /* File- HNYPROC    */
L10830:     call "HNYPROCU" (str(part$,1%,25%), procsrce$, hnydate$, quantity, ~
...
L10888:         call "DELETE" (#5, nextitemkey$, 6%)
        ...
        process_serial_numbers
          ...
          str(part$,1%,25%),/* Part code                      (PAR000) */~
            serial$,        /* Serial number                           */~
            " ",            /* Job Number                              */~
            vendor$,        /* Vendor code                             */~
        ...
        load_gl_info
            ...
            put str(gl_post_info$(),,) using L13490,                      ~
                ...
                " ",                     /* Salesman code CH(4)        */~
                " ",                     /* Invoice Number CH(8)       */~
                str(part$,1%,25%),       /* Part Number CH(25) (PAR000)*/~
            ...
            *************************************************************
            ...
            totalcost = cost( 1) + cost( 2) + cost( 3) + cost( 4) +      ~
                ...
                if hnyflg$ = "V" or hnyflg$ = " " then L30260
                                                        /* (PAR000)     */
                     call "INVGLGET" (part$, store$, lot$, adjacct$, 6%, ~
                                      #3, #8)
...
*        Subroutine to get part data only if G/L Export is Active
        data_for_glexport
            ...
            get #3 using L30460, partgen$, uom$, partcat$, partclass$,    ~
                                parttype$
L30460:         FMT POS(78), CH(16), CH(4), POS(110), CH(4), POS(153),    ~
                    CH(4), POS(200), CH(3)
            return
                                                        /* (PAR000)     */  
        REM *************************************************************~
...
L31600:         gosub read_work_file
                ...
                if cntr% > 0% then L31760
                     get #51, using L31690, str(part$,1%,25%), store$, lot$, quantity,~
                                           totalcost, extension
...
L35330:     FMT XX(6), /* File #5 INVADDTF--SKIP KEY INFORMATION       */~
