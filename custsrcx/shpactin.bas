        REM THISPROGRAMWASGENERATEDUSINGTHEGENPGMPROGRAMWHICHISAPROPRIETR~
            ...
            *            quantities shipped and the Innvoice for        *~
            *            Session (APC000). Now By Load                  *~
            *                                                           *~
            *     NOTE - Rename to (SHPACTIN) in Library 'APCOBJ'       *~
            *            (APCINVBO) is Exactly the same as (SHPACTIN)   *~
            * ---------------------------------------------------------Q*~
            ...
            *          !   EDIT_SESSION test.                     !     *~
            * 10/14/99 ! (EWD002) - Mod fix status updatde        ! RHH *~
            *          !                                          !     *~   
            * 02/09/06 ! (PAR000) mod for subpart                 ! CMG *~    
            * 08/26/08 ! (AWD003) mods for daily warehouse        ! CMG *~
            *11/15/2012! (AWD004) mods for ship audit             ! CMG *~
            *05/09/2017! (CR0534) Session ID format change        ! RDB *~
...
/*PAR000*/  subp$(100%)25,               /* sub part numbers           */~
            ...
            s_desc$21,                   /* Status Code Description    */~
            hdr$40,                      /* ASKUSR - Header Message    */~
            msg$(3%)79,                  /* ASKUSR - Message (EWD) End */~
            weight$10,                   /* Shipment Weight            */~
            currdate$8,                  /* Edit Date Variable         */~
            nxt_session$6                /* Validate next session entry*/
...
/*AWD004*/  * #56 ! SHPAUDIT ! Shipping Audit File                      *~
...
* PAR000
                     if epart$(j%) <> epart$(i%) then L13068
                          tq = tq + lotqtys(j%,1%)
L13068:              next j%
                call "INVAVAIL" (#12, #18, epart$(i%), store$,            ~
...
REM            gosub lookup_sc_qty                 /* PAR000 */
            ...
            if textso$(c%) <> hex(ffffffff) and textso$(c%) <> " "       ~
                then txt$(c%) = "T"
* PAR000
            call "READ100" (#12, epart$(c%), stocked%(c%)) 
...
L31370:              FMT CH(45), 2*CH(3), CH(25), CH(3),  PD(14,4),      ~
            ...
            /* Post Inventory  */
                gosub place_hold
                                    ...
                                    /* post_dte$           (EWD) Mod   */
* PAR000 update inventory AWD way..... update inventory flag
/* 2 for call by so */
                call "APCPLB44" ("2", " ", so$, #45 )     /* APCPLNSC */
...
L37170:     call "HNYLCSUB"  (part$(idx%),                               ~
        ...
        deffn'101(fieldnr%, edit%)
            ...
            str(line2$,62) = "SHPACTIN: " & "R7.00.00"        /* (EWD) */
...
*        Freight Charges                       FRTAMT$
        ...
        get_load
            ...
            read #10,key 4% = or_key$, using L50500, or_rec$,          ~
                                           eod goto get_load_done
L50500:      FMT CH(170)
              load$ = str(or_rec$,94%,5%)  
        get_load_done                           /* (EWD002)          */
        return
 
        update_apcplnor                         /* (EWD002)          */
            init(" ") or_key$
            str(or_key$,1%,8%)  = str(so$,1%,8%)
            read #10,hold,key 4% = or_key$, using L50500, or_rec$,          ~
                                          eod goto update_apcplnor_done
              str(or_rec$,60%,2%)  = "18"
              str(or_rec$,62%,8%)  = date
              str(or_rec$,146%,8%) = date
              put #10, using L50500, or_rec$
              rewrite #10
...
/*CR1098*/          
        ...
        call  "APCINVSB" (cuscode$,      /* Customer Code              */~
                          ...
                          load$,         /* Load order shipped (AWD003)*/~
                          "SHPACTIN",    /* Program (AWD004)           */~
...
L60520:     currdate$ = date
            ...
            session$ = "170108" : load$ = "00000"
REM            nxt_day% = i% + 1%
REM            if i% = 7% then nxt_day% = 1%
...
L60750:     inv% = 0%
            ...
            if session_edit% = 1% then goto L60600
        return
           
L60790: gosub first_session_closed
        return

        first_session_closed    /* today session closed, find next weekday */
            if inv% = 0% then goto L60792 
            if first_ses% = 0% then goto L60792                    
L60791:       call "DATE" addr("G+",dte$, 1%, dte$, err%)
              currdate$ = dte$
              call "DATE" addr("GD", dte$, days$, inv% )
              if days$ = days$(6%) or days$ = days$(7%) then goto L60791  
              call "DATEFMT" (currdate$)
              session$ = str(currdate$,7%,2%)& ~
                       str(currdate$,1%,2%)&str(currdate$,4%,2%)
              first_ses% = 0%
...
L60792: return
        ...
        session_screen
           if session_edit% = 1% then goto L61300
           pfkey$ = hex(ffffffffffffffffffffffffff0effffffffff00ffff)
           date$ = date : call "DATEFMT" (date$)
L60940: accept                                                           ~
            at (01,02), "Shipment Pre-Invoicing",                        ~
            at (01,64), "Today:",                                        ~
            ...
            at (04,02), fac(hex(94)),  errormsg$                , ch(79),~
                                                                         ~
            at (06,02), "Current Invoice Session Selected:",             ~
            at (06,40), fac(hex(81)),   session$                , ch(06),~
                                                                         ~
                                                                         ~
            at (10,20), "****************************************",      ~
            at (11,20), "*               N o t e                *",      ~
            at (12,20), "* The appropriate invoicing session is *",      ~
            at (13,20), "* displayed above, if necessary You    *",      ~
            at (14,20), "* may override by typing in a valid    *",      ~
            at (15,20), "* 'Session Id'. After 11:00 PM(forward)*",      ~
            at (16,20), "*                                      *",      ~
            at (17,20), "*  FORMAT YYMMDD   Ex. 171205          *",      ~
            at (18,20), "*                      Dec 5 2017      *",      ~
            at (19,20), "*                                      *",      ~
            at (20,20), "*                                      *",      ~
            at (21,20), "*                                      *",      ~
...
L61240:     gosub edit_session
        ...
        call "SCREEN" addr ("C", u3%, "I", i$(), cursor%())
        return
L61300:     session_edit% = 0%
        return                                      /* Clear Edit Flag */

        edit_session
            first_ses% = 0%           /* turn off first session flag */
            session_edit% = 0%
            inv% = 0%
            init(" ") nxt_session$
            ...
            if sav_session$ = session$ then return
               if userid$ = "CMG" then goto L61420
               if userid$ = "RDB" then goto L61420
               
              if inv_time% < 2300% then goto L61440
               
               dte$ = date
               call "DATE" addr("G+",dte$, 1%, dte$, err%)
               currdate$ = dte$
               call "DATEFMT" (currdate$)
               nxt_session$ = str(currdate$,7%,2%)& ~
                       str(currdate$,1%,2%)&str(currdate$,4%,2%)
               if nxt_session$ <> session$ then goto L61440  
               
L61420:     session_edit% = 1%                      /* Session Changed */
        return
...
L61440:     inv% = 1%
        ...
        edit_load
           load% = 0%
           if load$ = "00000" then return
           init(" ") ld_key$
           ld_key$ = load$
           read #50,key = ld_key$, using L61560, ld_status$,              ~
                                                eod goto L61610
L61560:       FMT POS(94), CH(2)
           if ld_status$ < "16" then goto L61610
           convert load$ to load%, data goto L61610

        return
L61610:    s_desc$ = ld_key$ & " is not VALID?"
           errormsg$ = s_desc$
           init(" ") ld_key$, load$
        return
                                                      /* (EWD) - End   */
