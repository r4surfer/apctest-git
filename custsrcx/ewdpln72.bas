        REM *************************************************************~
            ...
            *                                                           *~
            *  Subroutine Used   -                                      *~
            *      (EWDPLB72)    - (1%) Single Label Print (Glass)      *~
            *      (EWDPLA72)    - (2%) Production Labels               *~
            *      (EWDPLB72)    - (3%) Re-Make Glass Labels            *~
            *      (EWDPLC72)    - (4%) Special Glass Labels (EWDPLN66) *~
            *      (EWDPLD72)    - (5%) Special Shapes Glass Labels     *~
            *      (EWDPLA72)    - (6%) Tempered Glass Production Labels*~
            *      (EWDPLB72)    - (7%) Tempered Glass Re-Make Labels   *~
            *      (EWDPLA72)    - (8%) Pre-Cut Glass Labels (AWD008)   *~
            *                                                           *~
...
L50400: REM Glass Beginning and Ending Barcode   gl_beg$, gl_end$
        ...
        print_single_label                        /* (EWD001) - Mod    */
            ...
            call "EWDPLB72" (gl_rec$,sel%, #1, #2, #3, er%)  
            init(" ") gl_rec$                     /* (PAR000)                */
            gl_rec$ = str(rm_rec$(),1%,256%)      /* Use Re-Make data for    */
                                                  /* Single Label Print      */

            call "EWDPLB72" (gl_rec$,sel%, #1, #2, #3, er%)/* Real Label     */
            ...
            gl_rec$ = "00000000000000000000000000000000000000000000000000"~
                    ...
                    & "00000000000000000000000000000000000000000000000000"
                                                  ...
                                                  /* (PAR000) Channel #1 Add */
            call "EWDPLB72" (gl_rec$, sel%, #1, #2, #3, er%) /* Dummy Label  */
        ...
        print_labels_next
           read #4,key > gl_key$, using L50500, gl_rec$,               ~
...
L50510: 
                                               ...
                                               /* (PAR001) (PAR002)       */
           if sel% = 2% then                                           ~
               call "EWDPLA72" (gl_rec$, sel%, #1, #2, er%) /* Production */

                                               /* (PAR000) Channel #1 Add */
           if sel% = 3% then                                           ~
               call "EWDPLB72" (gl_rec$, sel%, #1, #2, #3, er%)/* Re-Make */

                                               /* (PAR000) Channel #1 Add */
           if sel% = 4% then                                           ~
               call "EWDPLC72" (gl_rec$, sel%, #1, #2, #3, er%)/* Special */
                                                            /* (EWD002)   */ 
                                                            /* (EWD006)   */ 
                                                            /* (EWD007)   */ 
                                               /* (PAR000) Channel #1 Add */
           if sel% = 5% then                                           ~
               call "EWDPLD72" (gl_rec$, sel%, #1, #2, er%) /* Spec Shapes*/
                                                            /* (EWD002)   */
                                                            /* (EWD006)   */
                                                            /* (EWD007)   */
                                                            /* (EWD004)   */  
           ...
           if sel% = 6% then                   /* (IM8022) replaced #5 for #1*/~
               call "EWDPLA72" (gl_rec$, sel%, #1, #2, er%) /* Tempered   */

                                               /* (PAR000) Channel #1 Add */
                                               /* (AWD009) channel #5     */
           if sel% = 7% then                   /*(IM8022)*/                ~
               call "EWDPLB72" (gl_rec$, sel%, #1, #2, #3, er%)/* Re-Mak Temp*/
                                                            /* (EWD004)   */ 
                                               ...
                                               /* (PAR001) (PAR002)       */
           if sel% = 8% then                                           ~
               call "EWDPLA72" (gl_rec$, sel%, #1, #2, er%) /* Pre-Cut    */
        ...
        print_labels_done
                                              ...
                                              /* (PAR001) (PAR002)       */
           if sel% = 2% then                                           ~
               call "EWDPLA72" (gl_rec$, 99%, #1, #2, er%) /* Production */
                                              ...
                                              /* (PAR000) Channel #1 Add */
           if sel% = 5% then                                           ~
               call "EWDPLD72" (gl_rec$, 99%, #1, #2, er%) /* Spec Shape */
                                                           /* (EWD006)   */
                                                           /* (EWD007)   */
                                              ...
                                              /* (PAR001) (PAR002)       */
           if sel% = 6% then                                           ~
               call "EWDPLA72" (gl_rec$, 99%, #1, #2, er%) /* Tempered   */
                                                           /* (AWD008)   */  
                                              /* (PAR000) Channel #1 Add */
                                              /* (PAR001) (PAR002)       */
           if sel% = 8% then                                           ~
               call "EWDPLA72" (gl_rec$, 99%, #1, #2, er%) /* Pre-Cut    */
