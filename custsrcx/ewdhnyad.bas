REM         *************************************************************~
            * EWDHNYAD - Automatic Part Deletion Program.               *~
            *          - Two phase program.  Phase 1--Updates the Spec/ *~
            *            Obso Code in HNYMASTR to OBSO for each part    *~
            *            whose Part Type matches a Part Type in the     *~
            *            GENCODES Table OBSOPTYPE. Phase 2--Attempts to *~
            *            delete all HNYMASTR recs w/Spec/Obso Code=OBSO.*~
            *-----------------------------------------------------------*~
            ...
            *---WHEN---+----------------WHAT----------------------+-WHO-*~
            * 10/26/98 ! ORIGINAL - Copied & Mod HNYDLETE.        ! BWS *~
            *************************************************************
        ...
        dim                                                              ~
            ...
            deltkey$50,                  /* READ Key for Deletion      */~
            errormsg$79,                 /* ERROR MESSAGE              */~
            gencdkey$24,                 /* Key to GENCODES File       */~
            ...
            spc_obs$4,                   /* Special/Obsolete Flag      */~
            text$4, textid$4,            /* TEXT ID XREFs              */~
            type$3,                      /* Part Type (HNYMASTR)       */~
...
L01982:     gosub warnscreen
                ...
                if keyhit% <>  0% then L01982
    
            start% = 0%
            call "APCPAUSE" (start%, "EWDHNYAD") /* Allow Delayed Start */
            if start% <> 0% then end
        ...
        REM *************************************************************~
            ...
            * #64 ! GENCODES ! The Perrenial Favorite                   *~
            *************************************************************~
            *                                                           *~
            *       FILE SELECTION AND OPEN CALLS                       *
            ...
            call "OPENFILE" (#64,"SHARE",f2%(64), rslt$(64), axd$(64))
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            a_text$(1) = "Part Number Deleted Through EWDHNYAD"
...
L10000: REM *************************************************************~
            ...
            *************************************************************
            ...
            gencdkey$ = "OBSOPTYPE"
            readkey$, deltkey$ = all(hex(00))
            upd%, del% = 0%

            call "SHOSTAT" ("Updating Records....")

        main_loop_1
            read #1, key > readkey$, using L10100, readkey$, type$,      ~
                eod goto main_loop_2
L10100:             fmt ch(25), pos(180), ch(3)
            str(gencdkey$,10,3) = type$
            read #64, key = gencdkey$, eod goto main_loop_1 /*In Table?*/
            read #1, hold, key = readkey$, eod goto main_loop_1
            put #1, using L10130, "OBSO"
L10130:             fmt pos(166), ch(4)
            rewrite #1, data goto main_loop_1
            upd% = upd% + 1%
            goto main_loop_1

        main_loop_2
            if del% = 0% then                                            ~
              call "SHOSTAT" ("**Deleting Parts from Active Data Base**")
            init(" ") part$, spc_obs$
            read #1, key > deltkey$, using L10240, deltkey$, spc_obs$,   ~
                eod goto L65000
L10240:             fmt ch(25), pos(166), ch(4)
            if spc_obs$ <> "OBSO" then goto main_loop_2
            call "ALLFREE"
            part$ = deltkey$
            gosub check_it
            goto main_loop_2
        ...
        REM *************************************************************~
            ...
            *************************************************************
check_it:   gosub check_sizeruns
            print at(21,2,79);" "
REM         call "SHOSTAT" ("Checking to see if Part Code is in use")
            save_sakey% = sakey%
            ...
            sakey% = save_sakey%
            if inuse$ = "NO" then goto delete_it
            return
        ...
        REM CALL "SHOSTAT" ("Removing from Parts Master               ")
                call "READ101" (#1, part$, f1%(1))
                     if f1%(1) = 0% then L30080
                get #1 using L30050, text$
L30050:              FMT POS(98), CH(4)
            delete #1
            call "CDANPOST" (#1, "D")
L30080:     return
...
*        Now set up SA file for next part, just like before
        ...
        check_sizeruns
REM         call "SHOSTAT"("Searching for Part in Size Run Matrices")
            init (hex(00)) readkey$
...
L34870:         FMT POS(225), 72 * CH(25)
            ...
            "remove it from the matrix and then run this program again.",~
              "Part No. = " & part$ & " Press any key to acknowledge.")
        ...
        accept                                                           ~
               at (01,03),                                               ~
        "*****************  EWDHNYAD: AUTO-DELETE PART CODES        *****~
        ~************",                                                  ~
               at (02,03),                                               ~
        "* This Program will automatically delete records whose part type~
        ~ is in the *",                                                  ~
               at (03,03),                                               ~
        "* GENCODES Table OBSOPTYPE and records whose HNYMASTR Spec/Obso ~
        ~Code=OBSO. *",                                                  ~
               at (04,03),                                               ~
        "****************************************************************~
        ~************",                                                  ~
        ...
        "Program execution occurs as follows:",                          ~
               at (06,03),                                               ~
        "   (1) All HNYMASTR records are scanned. If PartType is in the",~
               at (07,03),                                               ~
        "       GENCODES table, the Spec/Obso Code is set to 'OBSO'.",   ~
               at (08,03),                                               ~
        "   (2) All HNYMASTR records are scanned & all with Spec/Obso =",~
               at (09,03),                                               ~
        "       'OBSO' are attempted to be deleted (ala HNYDLETE). ",    ~
               at (10,03),                                               ~
        "   NO FURTHER PROMPTS APPEAR! A total count of records updated",~
               at (11,03),                                               ~
        "   to 'OBSO' and of total parts deleted will be displayed.    ",~
               at (12,03),                                               ~
        "****************************************************************~
        ~************",                                                  ~
               at (13,03),                                               ~
        ...
        ~on         *",                                                  ~
               at (17,03),                                               ~
        "* - No Demand For Part.               ! - Customer Price Informa~
        ~tion       *",                                                  ~
               at (18,03),                                               ~
        "****************************************************************~
        ~************",                                                  ~
               at (19,03),                                               ~
        "****    If in doubt, DON'T CONTINUE THIS PROGRAM. Future re-use ~
        ~of the   ****",                                                 ~
               at (20,03),                                               ~
        ...
        "PF KEYS:",                                                      ~
               at (24,03),                                               ~
        "(RETURN)Continue                        (15)Print Screen    (16)~
        ~Exit Program",                                                  ~
                                                                         ~
               keys(hex(000d0f1011)),                                    ~
               key (keyhit%)
...
L65000: REM THISPROGRAMWASGENERATEDBYGENPGMAPROPRIETRYPRODUCTOFCAELUS****~
            ...
            * CIATES, INC., SPOKANE, WA.  ALL RIGHTS RESERVED.          *~
            ...
            convert upd% to temp1$, pic(####0)
            convert del% to temp2$, pic(####0)
            errormsg$ = "Updated: " & temp1$ & "   Deleted: " & temp2$
            call "ASKUSER" (2%, "***** EWDHNYAD Results *****",          ~
                errormsg$, "Display HNYMASTR and search for OBSO to "    ~
                & "find parts that could NOT be Deleted.",               ~
                "Press <ENTER> to End")
