        REM CAELUSASSOCIATESSPOKANEWASHINGTONALLRIGHTSRESERVEDCAELUSASSOC~
            ...
            *          !              APC MODS   (djd 11/18/97)   !     *~
            *          !                                          !     *~
            * 09/25/91 ! MOD ADD VENDOR PART NUMBER TO REPORT     ! RHH *~
            *          ! (VENPART$) LINES - 30200, 31411,31412,   !     *~
            *          !                    31420                 !     *~
            * 11/19/92 ! MOD ADD ORDERDATE TO REPORT PRTDATE3$    ! RHH *~
            *          !            LINES - 30156, 30158, 19555,  !     *~
            *          !                    19556, 19563          !     *~
            * 07/22/94 ! MOD REPLACE JOB NUMBER WITH ON-HAND      ! RHH *~
            *          !     QUANTITY                             !     *~
            * 05/24/95 ! MOD CHANGE ON HAND QUANTITY TO INTEGER   !     *~
            *          !     AND ALLOW FOR NEGATIVE QUANTITY      !     *~
            * 04/03/98 ! Mods Applied to Current Version for (EWD)! RHH *~
            *          !      Special Modifications               !     *~
            * 04/07/00 ! (EWD001) Mod to change Vendor Backlog    ! RHH *~
            *          !     report to use Due Date               !     *~     
            *************************************************************
        dim                                                              ~
            ...
            qtytotal$10,                 /* PRINT VARIABLE             */~
                                         /* CHANGE 2       (EWD) Mods  */~
            quankey$44,                  /* HNYQUAN PRIMARY KEY        */~
            ...
            userid$3,                    /* USERID OF CURRENT USER     */~
                                         /*  CHANGE 3 (NEXT 2 LINES!)  */~
            venpart$25,                  /* VENDOR PART NUMBER         */~
            prt_venpart$32,              /* PRINT DESCRIPTION OF PART  */~
                                         /* END CHANGE #3  (EWD)       */~
        ...
        REM *************************************************************~
            ...
            *     !          !  CHANGE 4  (EWD)                         *~
            *   7 ! HNYQUAN  ! INVENTORY QUANTITIES   (EWD) Mod         *~
...
L19110:         call "PLOWNEXT" (#mc%, nextpo$, 9%, f1%(mc%))
            ...
            REM    if odate% < fdate% or odate% > tdate% then L19110
                                                      /* (EWD001)      */
                   if ddate% < fdate% or ddate% > tdate% then L19110
                                                      /* (EWD001)      */
...
L19527:                 print using L19916,                               ~
                              ...
                              on_hand$
                              /* APC MOD - 07/22/94 - REMOVE (PRTJOB$) */
                              /* CHANGE 8 (ABOVE STARTING WITH ON_HAND) */
                              /* (EWD) Mod  */
                        ...
                        /* CHANGE 11 - WAS <> 0 then L19566  (EWD)*/
                        if flag3% <> 0 then L19567: bckline% = bckline% + 1

                        print using L19941, prtstore$: flag3% = 1

                        /* CHANGE 12 - NEW LINE! (EWD) */
                        goto L19570

L19567:                 print using L19916, " ", " ", " ", prt_venpart$,  ~
                                           " ", " ", " ", " "
                        bckline% = bckline% + 1%

/* CHANGE 10 - LINE REF CHANGE FROM L19566 TO 19570) (EWD) */
L19570:             REM NOW ADD UP LINE ITEM AMOUNTS INTO ACCUMULATORS.
...
L19700:     REM PAGE CONTROL SUBROUTINE
                ...
                if bckline% < 66 then return
                   ...
                   print using L19836
                   print using L19852
                   /* CHANGE 13 REMARKED OUT THE LINE BELOW  */
                   /* and changed to print using without the */
                   /* Pound sign                (EWD) Mods   */
REM                print using L19868, "#"
                   print using L19868
...
L19932: %!         !                              !  ##########    +-----~
        ...
        /* CHANGE 15 CHANGED FORMAT OF THE PRINT USING STATEMENT (EWD)*/
L19941: %!         !                              !  ##########    !#####~
        ~###########################!        !          !          !      ~
        ~  !
...
L30000: REM *************************************************************~
            ...
            *************************************************************
            ...
            call "DATEOK" (orderdate$, odate%, errormsg$)
                                                       /* (EWD001)    */
            call "DATEOK" (datedue$, ddate%, errormsg$)
                                                       /* (EWD001)    */
            call "GLFMT" (slsacct$)

            /* CHANGE 16 - CHANGED DATEFMT CALL AND ADDED THE PRTDATE */
            /* VARIABLE SETTING                                       */
            prtdate3$ = orderdate$                     /* (EWD) - MOD */
            call "DATEFMT" (prtdate3$)
...
L30200:     get #lc%, using L31220,                                       ~
                    ...
                    job$,           store$,         oh$, venpart$
               prt_venpart$ = "VEN PT:" & venpart$
            /* CHANGE 17 IS THE TWO LINES ABOVE THIS COMMENT (EWD) */
            /* ADDED VENPART$ AND THE SETTING OF PRT_VENPART       */
...
L31220:     FMT XX(9),                   /* VENDOR CODE                */~
                ...
                CH(1),                   /* ON HAND POSTING OPTION     */~
                                         /* CHANGE 18 TO REPLACE (EWD) */~
                                         /* NEXT THREE LINES           */~
                XX(19),                  /* SKIP APC MOD 09/25/91      */~
                CH(25),                  /* VENDOR PART NUMBER         */~
                CH(06),                  /* FILLER - CHG (50) TO (06)  */~
                CH(50),                  /*   DITTO                    */~
                CH(23)                   /*     AH SO...               */
            ...
            *************************************************************
            ...
            accept                                                       ~
               ...
               at (06,57), fac(hex(81)), lastcust$              , ch(09),~
                                                                         ~
               at (08,02), "Starting PO Due Date",                       ~
               at (08,24), fac(hex(81)), fromdate$              , ch(10),~
               at (08,37), "Ending PO Due Date",                         ~
...
L51260:     call "STRING" addr("CT", date_msg$, 35%)
        ...
        load_quantity
            ...
            quankey$ = " "
            str(quankey$,1%,25%)  = str(prtdescr$,1%,25%)
            str(quankey$,26%,3%)  = store$
            str(quankey$,29%,16%) = lot$
            read #7,key = quankey$, using L51400, on_hand, eod goto L51430
L51400:         FMT POS(69), PD(14,4)
            convert on_hand to on_hand$, pic(#######-)
