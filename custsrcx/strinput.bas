        REM *************************************************************~
            ...
            * 12/01/90 ! (EWD) Special text for Store (000)40335  ! RHH *~
            * 11/07/95 ! Special Mod for allowing Stock Products  ! RHH *~
            *          ! to be entered into the system and Tracked!     *~
            *          ! Next Stock Load No. = NEXTRCVNR$ pos(194)!     *~
            *          ! Next S.O. Number = NEXTSTKSONR$ pos(178) !     *~
            * 08/07/96 ! (New) Planning Mods for Warranty Code at ! RHH *~
            *          !   Position (217). Format description of  !     *~
            *          !   Store (000) at Line = 31570            !     *~
            * 12/17/97 ! (EWD) Mod for Upgrade to R6.04.03        ! RHH *~
            * 03/31/98 ! Mods for (Y2K) Version of Caelus (EWD)   ! RHH *~
            *          !   Version not Caelus.                    !     *~  
             *************************************************************
        ...
        dim                                                              ~
            ...
            scr_msg1$23,                 /* Screen Text Message(EWD)Beg*/~
            scr_msg2$23,                 /* Screen Text Message        */~
            scr_msg3$23,                 /* Screen Text Message        */~
            scr_msg4$23,                 /* Screen Text Message        */~
            scr_msg5$26,                 /* Screen Text Message        */~
            scr_msg6$23,                 /* Screen Text Message        */~
            scr_msg7$23,                 /* Screen Text Message        */~
            scr_msg8$23,                 /* Screen Text Message        */~
            nextstksonr$8,               /* NEXT Stock Sales Order No. */~
            warranty$8                   /* (New) Warranty No. (EWD)End*/
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim apc$40, pname$21                                /*(EWD)*/
            apc$   = "(EWD) Store Input and Definition Utility" /*(EWD)*/
            pname$ = "STRINPUT - Rev: R6.04"                    /*(EWD)*/
        REM *************************************************************
            mat f2% = con
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            str(scrn2$,62) = "STRINPUT: " & "R6.04.03"
...
*        APC WARRANTY CODE                              /* (EWD) Begin */
            if storeid$ = "000" then                                     ~
               inpmessage$ = "Enter New EWD Warranty Start Code? ."
             return                                     /* (EWD) End   */
...
L30490:     FMT XX(3),                   /* Skip Store ID (EWD) Begin  */~
...
L31450:     FMT CH(15),                  /* Record Id                  */~
                ...
                CH(200)                  /* Variable Fields            */
                                         /* (EWD) - Begin              */ 
                                         /* Format Definition for (000)*/
            FMT CH(3),                   /* Store ID                   */~
                5*CH(30),                /* Store Information          */~
                CH(8),                   /* NEXTINVR$-Next Load Number */~
                CH(16),                  /* NEXTPONR$-Nxt Complaint Ref*/~
                CH(8),                   /* NEXTSTKSONR$-Next Stock S.O*/~
                CH(8),                   /* NEXTSONR$-Next UPC Number  */~
                CH(7),                   /* NEXTRCVNR$-Next Stock Load */~
                CH(8),                   /* NEXTCRMEMO$-Next Alpha Load*/~
                CH(8),                   /* NEXTADJ$-Next Complaint No.*/~
                CH(8),                   /* Next Warranty Number       */~
                CH(76)                   /* Filler                     */
                                         /* (EWD) - End                */
...
L32000: REM *************************************************************~
            ...
            *************************************************************
            ...
            get #3, using L32400, storeid$, information$(),              ~
                nextinvnr$, nextponr$, nextstksonr$,nextsonr$,nextrcvnr$,~
                nextcrmemo$, nextadj$, warranty$
...
L40335: REM Screen Entry Point                /* (EWD) - Begin         */
            if storeid$ <> "000" then goto L40415
               scr_msg1$ = "Next (UPC) Code  (APC) "
               scr_msg2$ = "Next Load Number (APC) "
               scr_msg3$ = "Next Forecast Load No. "
               scr_msg4$ = "Next Complaint Number. "
               scr_msg5$ = "Next Complaint Reference. "
               scr_msg6$ = "Next Stock Load Number "
               scr_msg7$ = "Next Stock Sales Order "
               scr_msg8$ = "Next Warranty Number   "
               str(taxcode$, 1%,8%) = nextstksonr$
               str(glaccts$(17%),1%,8%) = warranty$ & "    "
               gldescrs$(17%) = " "
               goto L40450
L40415:     scr_msg1$ = "Next Sales Order Number"
            scr_msg2$ = "Next Invoice Number    "
            scr_msg3$ = "Next Credit Memo Number"
            scr_msg4$ = "Next Invoice Adjustment"
            scr_msg5$ = "Next Purchase Order Number"
            scr_msg6$ = "Next Receiver Number   "
            scr_msg7$ = "Default Tax Code       "
            scr_msg8$ = "STD Revaluation Acct   "
L40450:     accept                                                       ~
...
*        Test DEFAULT TAX CODE
            ...
            if storeid$ <> "000" then goto L50645   /* (EWD) - Begin */
               nextstksonr$ = str(taxcode$,1%,8%)
               taxcode$ = " "
               return
L50645:     call "GETCODE" (#6, taxcode$, taxcodedescr$, 1%,0, f1%(6))
            if f1%(6) = 1% then return
                errormsg$ = "Tax Code not found on file."
                return                              /* (EWD) - End   */

L50670
*        Test Std Cost Revaluation Acct             /* (EWD) - Begin */
            if storeid$ = "000" then goto L50675
            a% = 17% : gosub find_account
            return
L50675
*        Test Warranty Number
            warranty$ = str(glaccts$(17%),1%,8%)
            init(" ") gldescrs$(17%)
            return                                  /* (EWD) - End   */ 
...
*         Lines 6 - 16  Finish up rest of Account Numbers
                ...
                print$(4) = "Q.C. CLEARING   " : a% =15%: gosub print_line
                                                    /* (EWD) - Begin   */ 
                if storeid$ = "000" then print$(4) = "Warranty Number "  ~
                                    else print$(4) = "REVALUATION     " 
                a% =17%: gosub print_line
                                                    /* (EWD) - End     */
