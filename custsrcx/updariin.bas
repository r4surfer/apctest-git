...
/* <AWD001> */
            ...
            if errs% = 0% then L13060
                errormsg$ = "Duplicate SO's found - Can't close session" & ~  
                            ". Cust = " & tmp_cust$ & ", SO = " & so_key$
            return
L13060:
/* </AWD001> */
...
/* <AWD001> */
       check_for_dup_so: 
	    ...
	    read #9,key 2 > so_key$,using check_fmt, tmp_cust$, tmp_inv$, ~
			   tmp_po$, so_key$, eod goto check_end
       check_fmt: FMT CH(9), CH(8), CH(16), CH(16)
       check_loop:   
	    hld_so$   = so_key$
	    hld_cust$ = tmp_cust$
	    hld_inv$  = tmp_inv$ 
	    hld_po$   = tmp_po$  
	    hld_cust$ = tmp_cust$
	    read #9,using check_fmt, tmp_cust$, tmp_inv$, ~
			   tmp_po$, so_key$, eod goto check_end
            if hld_so$   =  so_key$ then goto check_error     
            goto check_loop

       check_error:
	    if so_key$ = "                " then return
            errs% = 1%
            plowkey$ = str(upd$) & str(session$)
            call "READ101" (#1, plowkey$, f1%(1))
            if f1%(1) = 0% then return
