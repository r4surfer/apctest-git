        REM *************************************************************~
            *                                                           *~
            *  EEEEE  W   W  DDDD    CCC    OOO   M   M  PPPP   TTTTT   *~
            *  E      W   W  D   D  C   C  O   O  MM MM  P   P    T     *~
            *  EEEE   W W W  D   D  C      O   O  M M M  PPPP     T     *~
            *  E      WW WW  D   D  C   C  O   O  M   M  P        T     *~
            *  EEEEE  W   W  DDDD    CCC    OOO   M   M  P        T     *~
            *                                                           *~
            *-----------------------------------------------------------*~
            * EWDCOMPT - COMPLAINT TRACKING LOOKUP/REPORTING            *~
            *                                                           *~
            *                                                           *~
            *-----------------------------------------------------------*~
            *                  M O D I F I C A T I O N S                *~
            *---WHEN---+----------------WHAT----------------------+-WHO-*~
            * 08/21/00 ! New Program for (EWD) - Last Mod Date    ! CMG *~
            *          !     Rewrite of APCCOMPT to only allow    !     *~
            ...
            *          !  utilize all eight digits. (EWD004)      !     *~
            * 01/17/06 ! (PAR000) CR347 mods for sub part         ! CMG *~            
            *************************************************************

        dim                              /* (APCCOMPT) COMPLAINT FILE  */~
            blankdate$10,                /* PD empty date              */~
/*(EWD004)*/comp_number$8,               /* COMPLAINT NUMBER           */~
/*(EWD004)*/blankfield$1,                /* Blank Field                */~
...
L19085:    check% = 0%
        ...
        check_data                                   /* CUSTOMER CODE  */
          ...
          if str(comp_rec$,44%,9%) = comp_cuscode$ then goto L19170
             return
                                                     /* COMPLAINT CODE */
L19170:   if str(comp_code$,1%,1%) = "A" then goto L19190
          if str(comp_rec$,82%,3%) = comp_code$ then goto L19190
             return
                                                     /* INITIATOR CODE */
L19190:   if str(comp_init$,1%,1%) = "A" then goto L19210
          if str(comp_rec$,85%,2%) = comp_init$ then goto L19210
             return
                                                     /* CSC/PSC CODE   */
L19210:   if str(comp_svcs$,1%,1%) = "A" then goto L19230
          if str(comp_rec$,97%,2%) = comp_svcs$ then goto L19230
             return
                                                     /* TRACKING CODE  */
L19230:   if str(comp_crd$,1%,1%) = "A" then goto L19250
          if str(comp_rec$,109%,2%) = comp_crd$ then goto L19250
             return
                                                     /* Wizard CODE    */
L19250:   if str(comp_wizd$,1%,1%) = "A" then goto L19270
          if str(comp_rec$,121%,2%) = comp_wizd$ then goto L19270
              return
                                                     /* COMPLAINT DATE */
L19270:   if str(beg_date$,1%,1%) = "A" then goto L19295
          if str(comp_rec$,87%,6%) < beg_dte$ or str(comp_rec$,87%,6%) > ~
                                            end_dte$ then return

                                                     /* Salesman Code */
L19295:   if str(comp_slmn$,1%,1%) = "A" then goto L19320
          if str(comp_rec$,40%,4%) = comp_slmn$ then goto L19320
             return
...
*        Define the Input Message for the Screen/Field Indicated
        ...
        scrn1_msg  :  data                /* EWD001 - Minor Mods */      ~
         "Enter Complaint Number for Look-up?                          "  
...
*        Define the Input Message for the Screen/Field Indicated
        ...
        scrn2_msg  :  data                /* EWD001 - Minor Mods */      ~
         ...
         "Enter a Valid CSC/PSC Code or (A) for All.                   ",~
         "Enter a Valid Tracking Code or (A) for All.                  ",~
        ...
        initialize_variables
            init(" ") errormsg$, inpmessage$, comp_number$, comp_so$,    ~
                      ...
                      comp_err_dte$, beg_mod$, end_mod$, comp_subp$
        ...
        dataload
          comp_cost$ = "0.00"        
            ...
            gosub unpack_comp_number               /*  (EWD004)  */                                   
            ...
            gosub lookup_part
            gosub lookup_complaintcode
            gosub lookup_initiatorcode
            gosub lookup_initiatordate
            gosub lookup_trackingcode
            gosub lookup_sourcecode
            gosub lookup_text
            gosub lookup_model
            gosub lookup_userid
            gosub lookup_errordate
            gosub lookup_wizardcode
            errormsg$ = " "
...
L35040: FMT                              /* (APCCOMPT) COMPLAINT FILE  */~
            ...
            CH(02),                      /* SERVICE CODE (SVCS CODE)   */~
            CH(06),                      /* SERVICE DATE               */~
            CH(04),                      /* SERVICE TEXT ID            */~
            CH(02),                      /* TRACKING CODE              */~
            CH(06),                      /* TRACKIING DATE (LAST MOD)  */~
            CH(04),                      /* TRACKING TEXT ID           */~
            CH(02),                      /* SOURCE CODE                */~
            CH(04),                      /* SOURCE TEXT ID             */~
...
L40350:     accept                                                       ~
               at (01,02),                                               ~
                  "Complaint Tracking - View and Reporting-(EWDCOMPT)",  ~
               at (01,66), "Today:",                                     ~
               ...
               at (04,25), fac(hex(8c)), comp_so$               , ch(08),~
                                                                         ~
               at (05,02), "  S.O. Line Item No.:",                      ~
               at (05,25), fac(hex(8c)), comp_line$             , ch(02),~
                                                                         ~
               at (06,02), "  Customer PO Number:",                      ~
               at (06,25), fac(hex(8c)), comp_po$               , ch(16),~
                                                                         ~
               at (07,02), "  APC RGA Number    :",                      ~
               at (07,25), fac(hex(8c)), comp_rga$              , ch(08),~
                                                                         ~
               at (08,02), "  Customer Code     :",                      ~
               at (08,25), fac(hex(8c)), comp_cuscode$          , ch(09),~
               at (08,40), fac(hex(8c)),   cust_name$           , ch(30),~
               at (08,75), fac(hex(8c)),   comp_slmn$           , ch(04),~
                                                                         ~
               at (09,02), "  Part Number       :",                      ~
               at (09,25), fac(hex(8c)), comp_part$             , ch(25),~
/*PAR000*/     at (09,52), fac(hex(8c)), comp_subp$             , ch(20),~
/*PAR000*/     at (10,52), fac(hex(8c)),str(comp_part_d$,1%,28%), ch(28),~
                                                                         ~
               at (10,02), "  Quantity-No.Items :",                      ~
               at (10,25), fac(hex(8c)), comp_quan$             , ch(04),~
                                                                         ~
               at (11,02), "  Complaint Code    :",                      ~
               at (11,25), fac(hex(8c)), comp_code$             , ch(03),~
               at (11,40), fac(hex(8c)),   comp_code_desc$      , ch(32),~
                                                                         ~
               at (12,02), "  Initiator Code    :",                      ~
               at (12,25), fac(hex(8c)), comp_init$             , ch(02),~
               at (12,40), fac(hex(8c)),   comp_init_desc$      , ch(32),~
                                                                         ~
               at (13,02), "  Initiator Text    :",                      ~
               at (13,25), fac(hex(8c)), comp_init_txt1$        , ch(03),~
               at (13,40), "Date:",                                      ~
               at (13,46), fac(hex(8c)), comp_init_dte$         , ch(08),~
                                                                         ~
               at (14,02), "  CSC/PSC Error Code:",                      ~
               at (14,25), fac(hex(8c)), comp_svcs$             , ch(02),~
               at (14,40), fac(hex(84)),   comp_svcs_desc$      , ch(32),~
                                                                         ~
               at (15,02), "  Editor Initials   :",                      ~
               at (15,25), fac(hex(8c)), comp_ed_id$            , ch(03),~
                                                                         ~
               at (15,40), "Date of Err:",                               ~
               at (15,53), fac(hex(8c)), comp_err_dte$          , ch(08),~
                                                                         ~
               at (16,02), "  Entry Initials    :",                      ~
               at (16,25), fac(hex(8c)), comp_en_id$            , ch(03),~
                                                                         ~
               at (17,02), "  Proofer Initials  :",                      ~
               at (17,25), fac(hex(8c)), comp_pr_id$            , ch(03),~
                                                                         ~               
               at (18,02), "  Window Wizard Code: ",                     ~
               at (18,25), fac(hex(8c)), comp_wizd$             , ch(02),~
               at (18,40), fac(hex(84)),   comp_wizd_desc$      , ch(32),~
                                                                         ~
               at (19,02), "  Credit  Code      :",                      ~
               at (19,25), fac(hex(8c)), comp_crd$              , ch(02),~
               at (19,40), fac(hex(84)),   comp_crd_desc$       , ch(32),~
                                                                         ~
               at (20,02), "  Cost of Complaint :",                      ~
               at (20,25), fac(hex(8c)), comp_cost$             , ch(10),~
                                                                         ~
               ...
               if keyhit% <> 8 then goto L41180
                  gosub view_init_text
                  goto L40350                  
                  
L41180:        if keyhit% <> 15 then goto L41190
                  call "PRNTSCRN"
                  goto L40350
...
REM           comp_number% = 0%
REM           convert comp_number$ to comp_number%, data goto L50230

REM           convert comp_number% to comp_number$, pic(00000)

           gosub pack_comp_number           /*  (EWD004)  */
           read #1,key = comp_number$, eod goto L50230
           gosub dataload
...
L50705: return
        ...
        lookup_part
             err% = 0%
             if len(comp_part$) <> 3 then goto L50795
                readkey$ = all(hex(00))
...
L50795:     call "APCLDSUB" (comp_part$, comp_part_d$, #9, err%)
        ...
        lookup_sourcecode
            readkey$ = all(hex(00))
            readkey$ = "SRCE CODE" & comp_svcs$
            call "DESCRIBE" (#4, readkey$, comp_svcs_desc$, 0%, f1%(4))
            if f1%(4) = 0 then comp_svcs_desc$ = " "
        return
        ...
        lookup_userid
            ...
            if f1%(17) = 0 then comp_en_id$ = "000"
            readkey$ = all(hex(00))
            readkey$ = comp_ed_id$
            call "DESCRIBE" (#17, readkey$, " ", 0%, f1%(17))
            if f1%(17) = 0 then comp_ed_id$ = "000"
            readkey$ = all(hex(00))
            readkey$ = comp_pr_id$
            call "DESCRIBE" (#17, readkey$, " ", 0%, f1%(17))
            if f1%(17) = 0 then comp_pr_id$ = "000"
        return
...
L61340:     str(comp_key$,1%,4%) = str(comp_number$,1%,4%)  /*  (EWD004)  */
            str(comp_key$,5%,1%) = " "
            read #1,key = comp_number$, eod goto generate_next
            dataload% = 1%
            gosub dataload
            dataload% = 0%
...
L61400:     gosub pt_line
        ...
        view_init_text
            gosub'099(comp_init_txt$)
            str(header$,,4%) = "View Initiator TEXT for " & comp_init_desc$ 
