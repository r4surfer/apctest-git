...
/*EWD002*/  * -- <PF28> Overrides Restrictions of EWD002 for Admins --  *~
            *-----------------------------------------------------------*~
            ...
            * 04/02/98 ! (EWD) Mods for insuring that no more than! RHH *~
            *          !     99 Receipts for a given Line Item on !     *~
            *          !     can be entered.                      !     *~   
            * 10/29/98 ! (EWD001) Mod to 4/2/98 mod to prevent    ! BWS *~ 
            *          !     locked rcvrs & to allow unreceiving. !     *~
            * 03/17/99 ! (EWD002) Mods to prevent edit of invoiced! BWS *~
            *          !     lines; restrict to 1 Vendor per rcvr.!     *~
        ...
        dim                                                              ~
            ...
            readkey$50, sav_key$25, rhh$3,  /* Generic Key   (EWD)     */~
            hdr$40, msg$(3%)79,          /* Warning Messages (EWD)     */~
...
/*EWD002*/  admin% = 0%
/*EWD002*/  call "CMSMACHK" ("VBK", lfac$(1%), lfac$(2%))
/*EWD002*/  if /*lfac$(1%) = "Y" or*/ lfac$(2%) = "Y" then admin% = 1%
...
/*L12431:*/     askhit% = 2%            /*EWD002*/
                call "ASKUSER" (askhit%, "ITEM INVOICED",                ~
                     "This Receiver Item has already been Invoiced.",    ~
/*EWD002*/           "---","Press RETURN to Abort Modification") /* -or-", */
* EWD002             "Press PF16 to Continue with Edit.")           /* Hidden */
/*EWD002*/      if admin% <> 0% and askhit% = 28% then goto L12440  /*Override*/
/*EWD002*/      /* if askhit% = 0% then */ goto edtpg3
* EWD002        if askhit% <> 16% then L12431
...
/*EWD002*/    if vencode$(v%) = vencode$(1%) then L51510
/* Begin*/       errormsg$ = "Only One Vendor Per Receiver Allowed"
                 if admin% = 0% then return else k% = 2%
                 call "ASKUSER" (k%,"ADMIN OVERRIDE??"," ",errormsg$," ")
/*EWD002*/       if k% = 28% then errormsg$ = " "   /* Hidden Override */
/* End  */       return
...
L52400: REM Validation for rec this shpmnt
            gosub check_lines                 /* (EWD) - Modification */
...
check_lines                                   /* (EWD) - Begin        */
            temp = 0                                          /*EWD001*/
            convert qtyrecd$(c%,m%) to temp, data goto L52475 /*EWD001*/
L52475:     if temp = 0 then return                           /*EWD001*/
            init(" ") readkey$, sav_key$ : rhh% = 0%
            str(readkey$,1%,9%)   = vencode$(v%)
            str(readkey$,10%,16%) = ponumber$(v%)
            sav_key$              = str(readkey$,1%,25%)
...
check_done
            ...
            qtyrecd(c%) = 0.0
*EWD001     return clear all
*           goto inputmode                          /* (EWD) - End        */ 
/*EWD001*/  errormsg$ = "Can't Receive This Line Item -- Too Many Receipts"
/*EWD001*/  return
