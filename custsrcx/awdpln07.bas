        REM *************************************************************~
            ...
            *                                                           *~
            *                                             (AWD004)      *~
            *  aes_prt_flg$   - '1' = 3844Z(Small) '2' = 220XI(Big)     *~
            *                                                           *~
            *                                                           *~  
            *-----------------------------------------------------------*~
            ...
            *06/27/2007! (AWD008) switch preinter to 220          ! DES *~
            *************************************************************
        ...
        dim aes_po$16,                   /* Purchase Order Number      */~
            ...
            aes_serial$8, sc_serial$8,   /* Serial Number Assigned (8) */~
        ...
        REM *************************************************************~
            ...
            *-----+----------+------------------------------------------*~
            * #1  ! AESPRDLB ! AES Barcode Label Database               *~
...
*        Define the Input Message for the Screen/Field Indicated
            ...
            return
                                                     /* (AWD004)    */  
        scrn1_msg  :  data                                                       ~
         "Enter a Valid P.O. Number?                                           ",~
         "Enter a Valid Line Item Number for Selected P.O. and Printer(1 or 2)?",~
...
*        Define the Input Message for the Screen/Field Indicated
        ...
        initialize_variables
            init(" ") errormsg$, inpmessage$, sc_raw_no$, sc_rack_qty$,  ~
                      ...
                      current_avail$, spec_errormsg$, aes_prt_flg$,      ~
                      aes_prt_flgd$

            aes_prt_default$ = "2"                        /* (AWD008)   */ 
                                                          /* (AWD004)   */
        return
        ...
        dataput   
           ...
           str(aes_key$,20%,3%)  = aes_pan_no$
           str(aes_key$,23%,8%)  = aes_serial$   /* This should not occurr */
...
L30010:   
                                         /* (AESPRDLB)                 */
        put #1, using L35040,                                            ~
            aes_po$,                     /* Purchase Order Number      */~
            aes_item$,                   /* Purchase Order Item Number */~
...
L35040:     FMT CH(16),                  /* Purchase Order Number      */~
                CH(03),                  /* Purchase Order Item Number */~
                CH(03),                  /* Pan Id Assoc. with PO Item */~
                CH(08),                  /* Serial Number Assigned (8) */~
        ...
        deffn'101(fieldnr%, edit%)
              ...
              on fieldnr% gosub L40160,     /* Purchase Order Number   */~
                                L40160,     /* P.O. Line Item Number   */~
...
L40190:     accept                                                       ~
               ...
               at (02,02), fac(hex(94)), errormsg$              , ch(79),~
                                                                         ~
               at (03,02), "Purchase Order Number:",                     ~
               at (03,25), fac(lfac$(1%)), sc_po$               , ch(16),~
                                                                         ~
               at (04,02), "P.O. Line Item Number:",                     ~
               at (04,25), fac(lfac$(2%)), sc_item$             , ch(03),~
                                                                         ~  
               at (04,50), "Printer:",                                   ~
               at (04,62), fac(lfac$(2%)), aes_prt_flg$         , ch(01),~
               at (04,65), fac(hex(84)), aes_prt_flgd$          , ch(05),~
                                                                         ~
               at (05,02), "Rack Quantity        :",                     ~
               at (05,25), fac(lfac$(3%)), sc_rack_qty$         , ch(10),~
               at (05,50), "Available:",                                 ~
               at (05,62), fac(hex(84)), current_avail$         , ch(10),~ 
                                                                         ~
...
L43700:                              /*  Edit Mode - Enabled    */
        ...
        deffn'151(fieldnr%)
            ...
            on fieldnr% gosub L50010,        /* Purchase Order Number */~
                              L50050,        /* P.O. Line Item Number */~
...
L50050: Rem P.O. Lime Item Number               sc_item$
            ...
            if lookup_raw_material% = 0% then goto L50060
                                                   /* (AWD004)        */
            if aes_prt_flg$ = " " then aes_prt_flg$ = aes_prt_default$
            if aes_prt_flg$ <> "1" and aes_prt_flg$ <> "2" then         ~
                                      aes_prt_flg$ = aes_prt_default$
            if aes_prt_flg$ = "1" then aes_prt_flgd$ = "3844Z"          ~
                                  else aes_prt_flgd$ = "220XI"
                                                   /* (AWD004)        */
        return 
...
L52020:            
            ...
            read #1,key 1% = sc_serial$, using L52030, aes_rec$,           ~
                                                      eod goto L52050
L52030:        FMT CH(256)
...
L52230:     errormsg$ = "(Error) Invalid Raw Material Number?"
        ...
        delete_serial_no
            ...
            if delete% = 0% then goto L53010
               ...
               read #1,hold,key 1% = aes_serial$, using L52030, aes_rec$,           ~
                                                           eod goto L53000
                  delete #1
...
PO_5:
        ...
        lookup_raw_material                  /* (HNYMASTR)         */
            ...
            read #4, key 0% = aes_part$, using PO_6, part_cat$,  ~
                              part_class$, aes_loc$, part_type$, ~
                              aes_pan, eod goto PO_7

PO_6:        FMT POS(90), CH(04), POS(133), CH(04), POS(155%),   ~
...
create_rack_label_1:
                                                    /* (AWD004)                 */ 
                call "AWDPLA07" (pan%, been_here%, aes_rec$, aes_prt_flg$, err%)
...
create_rack_label_2
        ...
        search_raw_nos_nxt
            read #3, key > vbline_key$, using PO_3, vbline_rec$(),   ~
                                          eod goto search_raw_nos_done

            if str(vbline_rec$(),1%,7%) <> "VE32678" then goto search_raw_nos_done
               vbline_key$ = str(vbline_rec$(),1%,28%)
            ...
            search_raw
               ...
               str(vdt_due$,1%,6%) = str(vbline_rec$(),142%,6%)
                                              /* Special Date Test   */
               if str(vdt_due$,1%,6%) < pg_dte1$ then goto search_raw_nos_nxt
                                              /* Less than 30 Days   */

                                              /* Check Raw Material Number */
               if str(vbline_rec$(),32%,14%) <> sc_raw$ then          ~
         ...
         REM      vq_out = vq_ordr - (vq_rec + rhh)
                                           /* See if Qty is Avail   */
               if vq_out < 10.0 then goto search_raw_nos_nxt
        ...
        search_raw_nos_done
               ...
               if k_max% = 0% then goto search_raw_exit
                                                  /* P.O. Number    */
               sc_po$ = str(val$(1%),13%,16%)
                                                  /* Line Item No.  */
               sc_item$ = str(val$(1%),30%,3%) 

         return clear all
         goto inputmode_1
                                                  /* No P.O. Found */
         search_raw_exit
         errormsg$ = "No P.O. found for Raw Material ("& sc_raw$ &")"
         gosub error_prompt
         return clear all
         goto inputmode 
...
DISP_8:        close ws
        ...
        set_disp
            ...
            d_title$ = "**Open Purchase Order Display**"
            str(hh$,1%,4%)   = "Seq!"
            str(ss$,1%,4%)   = "---!"
            str(hh$,5%,8%)   = "Vendor !"
            str(ss$,5%,8%)   = "-------!"
            str(hh$,13%,17%) = "P.O. Number     !"
            str(ss$,13%,17%) = "----------------!"
            str(hh$,30%,4%)  = "Itm!"
            str(ss$,30%,4%)  = "---!"
            str(hh$,34%,15%) = "Raw Mat'l No. !"
            str(ss$,34%,15%) = "--------------!"
            str(hh$,49%,11%) = "Due  Date !"
            str(ss$,49%,11%) = "----------!"
            str(hh$,60%,10%) = "Ord  Qty !"
            str(ss$,60%,10%) = "---------!"
            str(hh$,70%,10%) = "Qty Avail!"
            str(ss$,70%,10%) = "---------!"
