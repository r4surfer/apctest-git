        REM THISPROGRAMWASGENERATEDUSINGTHEGENPGMPROGRAMWHICHISAPROPRIETR~
            *                                                           *~
            *  FFFFF   AAA    AAA   DDDD   DDDD   PPPP   RRRR   TTTTT   *~
            *  F      A   A  A   A  D   D  D   D  P   P  R   R    T     *~
            *  FFFF   AAAAA  AAAAA  D   D  D   D  PPPP   RRRR     T     *~
            *  F      A   A  A   A  D   D  D   D  P      R   R    T     *~
            *  F      A   A  A   A  DDDD   DDDD   P      R   R    T     *~
            *                                                           *~
            *-----------------------------------------------------------*~
            * FAADDPRT - THIS PROGRAM PRINTS THE DEPRECIATION ADDITIONS *~
            *            FOR A RANGE OF ASSET NUMBERS , DEPRECIATION    *~
            ...
            * 09/08/88 ! File changes to FAMASTER, Standardized   ! RJM *~
            *          !   Added Report ID "F/A001" & Print new   !     *~
            *          !   data fields.  Fixed report Alignment.  !     *~
            * 02/19/93 ! PRR 11374 - Corrected page numbering when! MLJ *~
            *          !   running multiple reports.              !     *~
            *          ! MISC - Added 'END OF REPOR' and time     !     *~
            *          !   stamp, fixed numerous implied integers.!     *~
        ...
        dim                                                              ~
            ...
            first_date$10,               /* FIRST DATE IN RANGE        */~
            hi_asset$10,                 /* End of Asset Range         */~
            hi_date$10,                  /* End of Date Range          */~
            ...
            last_date$10,                /* LAST DATE IN RANGE         */~
            ldate$13,                    /* LAST DATE IN RANGE         */~
            life$(3)5,                   /* ASSET LIFE                 */~
            line2$79,                    /* Screen Header              */~
            lo_asset$10,                 /* Start of Asset Range       */~
            lo_date$10,                  /* Start of Date Range        */~
            method$56,                   /* METHOD                     */~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            str(line2$,62%) = "FAADDPRT: " & str(cms2v$,,8%)
        ...
        REM READ "SYSFILE2" FOR FISCAL YEAR END DATE
            ...
            get #02, using L09290, begdate$, enddate$
L09290:          FMT XX(20), CH(8), CH(8)
        ...
        ~TO STRAIGHT LINE"
            ...
            first_date$ = fiscal_begin$
            last_date$ = fiscal_end$
...
L10280:     gosub L40000
           ...
           call "SHOSTAT" ("Printing Fixed Assets Additions Schedule")
            select printer(134)
            call "SETPRNT" ("F/A001", " ", 0%, 0%)
            gosub L11000
            goto  inputmode
...
L11340: FMT                      /* FILE: FAMASTER                     */~
            ...
            if type_code$ <> "3" then L11750
            if servdate$(cat%) < lo_date$ or                             ~
               servdate$(cat%) > hi_date$ then L11000
L11750:     gosub L20000
            goto L11000
...
L23200:     method$, life$ = " "
         ...
         if meth% > 0% and meth% < 9% then method$ = method_descr$(meth%)
            disposal_desc$ = " "
            if dispdate$ < today$ and dispdate$ <> " " and ~
               dispdate$ <> blankdate$ then disposal_desc$ = "*** RETIRED ***"
...
L28000: REM *************************************************************~
            ...
            *************************************************************
            ...
            print using L30140, fdate$, ldate$
            print
            print using L30160
            print using L30190
            print using L30230
...
L40230:     accept                                                       ~
               at (01,02),                                               ~
                  "FIXED ASSETS ADDITIONS SCHEDULE",                     ~
               at (01,66), "Today:",                                     ~
               ...
               at (08,30), fac(hex(81)), first_date$            , ch(10),~
               at (09,02),                                               ~
                  "ENDING SERVICE DATE",                                 ~
               at (09,30), fac(hex(81)), last_date$             , ch(10),~
               at (10,02),                                               ~
                  "DEPRECIATION CATEGORY",                               ~
               at (10,30), fac(hex(81)), depr_category$         , ch(01),~
                                                                         ~
...
L50000: REM *************************************************************~
        ...
        REM Test Date Range Here
            if first_date$ <> "ALL" then L50130
                fdate$ = "(BEGINNING OF FILE)"
                ldate$ = "(END OF FILE)"
                goto L50360

L50130:     if first_date$ = " " or first_date$ = blankdate$ ~
                                 or first_date$ = "ALL" then L50160
                call "DATEOKC" (first_date$, d%, errormsg$)
                if errormsg$ <> " " then return
L50160:     if last_date$ = " " or last_date$ = blankdate$ ~
                                or last_date$ = "LAST" then L50230
                call "DATEOKC" (last_date$, d%, errormsg$)
                if errormsg$ <> " " then return

            call "DATUFMTC" (last_date$)
            call "DATUFMTC" (first_date$)

L50230:     call "TESTRNGE" (first_date$, last_date$, lo_date$,          ~
                             hi_date$, errormsg$)
            call "DATFMTC" (first_date$)
            call "DATFMTC" (last_date$)
                if errormsg$ = " " then L50310
                    errormsg$ = "For Date - " & errormsg$
                    return

L50310:     if lo_date$ = hex(00) then fdate$ = "(BEGINNING OF FILE)"    ~
                                  else fdate$ = first_date$
            if hi_date$ = hex(ff) then ldate$ = "(END OF FILE)"          ~
                                  else ldate$ = last_date$

        REM Test Asset Code Range Here
L50360:     call "TESTRNGE" (first_asset$, last_asset$, lo_asset$,       ~
                             hi_asset$, errormsg$)
            if errormsg$ <> " " then return

            if first_asset$ <> "ALL" then L50450
                fasset$ = "(BEGINNING OF FILE)"
                lasset$ = "(END OF FILE)"
                goto L50500

L50450:     if lo_asset$ = hex(00) then fasset$ = "(BEGINNING OF FILE)"  ~
                                   else fasset$ = first_asset$
            if hi_asset$ = hex(ff) then lasset$ = "(END OF FILE)"        ~
                                   else lasset$ = last_asset$

L50500:     if pos("BFS"=depr_category$) = 0% then L50600

            category_descr$ = "BOOK DEPRECIATION"
            if depr_category$ = "F" then category_descr$ = "FEDERAL TAX D~
        ~EPRECIATION"
            if depr_category$ = "S" then category_descr$ = "STATE/LOCAL T~
        ~AX DEPRECIATION"
            cat% = pos("BFS"=depr_category$)
            return

L50600:     errormsg$="VALID DEPRECIATION CATEGORIES: 'B', 'F', 'S'"
            return
