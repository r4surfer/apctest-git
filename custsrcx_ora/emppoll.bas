        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            select #3,  "APCEMPDT",                                      ~
                        varc,     indexed,  recsize =  128,              ~
                        keypos = 1,    keylen =  18,                     ~
                        alt key 1, keypos = 104, keylen =  6, dup
...
REM CALL "SHOSTAT" ("PURGE MT ")  STOP
         ...
         err%, pass% = 0%    /*More than 14 Days Old;Reset pass for ADPEMPMT*/         
         gosub purgeMTADPRecords         
         purgeMT% = 1%
...
REM       call "DATE" addr("G+",dte$,-14%,purgeDte$,err%)
       ...
       gosub getMinDate
       mindte$ = field$
       call "DATUFMTC" (mindte$)
       purgeDte$ = str(mindte$,1%,6%)       
...
REM ========================
    getMinDate
        init(" ") stmt1$, stmt2$
REM                  ....+....1....+....2....+....3....+....4....+....5....+.
          stmt1$  = "select min(field7) as mindate from mssql.oe_adpbuf ~
...
REM ========================
    ...
    purgeMTADPRecords
      ...
      init(" ") logMessage$
      logMessage$ = "RECORD MTADPPURGE" & date$ & " " & file$ & " " & trans$ ~
         ~ & " " & jobid$ & " " & state$ & " " & mfgDept$ & " " & fields$(01%) 
                    
      purgekey$ = all(hex(00))
      str(purgekey$,1%,6%) = purgeDte$
      read #chno%, hold, key 3% >= purgekey$, using PURGEADPMT_FMT, rec$,  ~
                                                    eod goto purgeADPMTDone
PURGEADPMT_FMT:      FMT CH(128)
           goto purgeADPMTFirst
    purgeADPMTNxt:
      read #chno%, hold, using PURGEMT_FMT, rec$, eod goto purgeADPMTDone

    purgeADPMTFirst:
      purgekey$ = str(rec$,90%,6%)    /*Don't purge if not applied Date*/
      if purgekey$ = " " then goto purgeADPMTNxt
      if blankdte$ = purgekey$ then goto purgeADPMTNxt
      if purgeDte$ > purgekey$ then goto purgeADPMTNxt
    ...
    purgeADPMTDone
      if pass% >= 1% then return
      pass% = pass% + 1%
      goto purgeMTADPRecords
      if debug% = 1% then call "SHOSTAT" ("MT ADP PURGE DONE")
    return
