        REM *************************************************************~
            ...
            *          !   VIQS. Note - Can Fax                   !     *~    
            * 10/31/05 ! (AWD003) CR347 Mod for Sub Part          ! CMG *~
            * 01/15/06 ! (PAR000) CR347 Mod to put in New Part No.! RHH *~
            *          !   Explosion.                             !     *~
            * 01/31/06 ! (PAR001) CR347 Mod to display new part   ! RHH *~
            *          !   on the initial display screen.         !     *~ 
            * 05/11/06 ! remove lock (in BCKBUFFR) and add TXTCUST! DES *~
            * 06/02/06 ! (PAR002) Mod to add new Feild to the Part! RHH *~
            *          !   number Explosion.                      !     *~  
            *07/17/2013! (AWD004) mod for dim1es, dim2es, dim3es  ! CMG *~
        ...
        dim                                                              ~
            ...
            comm%(3%), comm$(35),        /* Commission Split %s        */~
            ...
            model$(100%)3,               /* Model                      */~
            part_sub$(100%)20,           /* New Sub Part Number(PAR001)*/~
            partno1$(100%)20,            /* (AWD003) Sub Part Number   */~
            ...
            refdesc$(1005)30,            /* Reference Part Description */~
        ...
        dim size$1, bck$10,              /* (O)pening, or (E)xact      */~
            ...
            code$2, cod$10               /* Sales Order Number         */
                                         ...
                                         /* (PAR000)                   */
        dim hs_ln$3,                     /* Sales Order Line Item      */~
            hs_so$8,                     /* Customer Sales Order       */~
            hs_part$25,                  /* MFG Part Number            */~
            hs_sub_part$20,              /* Sub Part Number            */~
            hs_info_part$9               /* Info Part Number           */
                                         /* (PAR000)                   */
        ...
        REM *************************************************************~
            ...
            *************************************************************
            dim cms2v$50
            cms2v$ = "R7.00.00 01/31/06 New Part Number Display "
        REM *************************************************************
        ...
        REM *************************************************************~
            ...
            * #22 ! TXTFILE  ! System Text File (old)                   *~
            ...
            * #53 ! TXTCUST  ! System Text File (new)                   *~
            * xxx !          ! (EWD) - Begin Files                      *~  
...
/*AWD004*/              varc,     indexed,  recsize =  768,              ~
        ...
        call "SHOSTAT" ("Opening Files, One Moment Please")
            ...
            call "OPENCHCK" (#61, 0%, f2%(61%), 100%, " ")
            call "OPENCHCK" (#63, 0%, f2%(63%), 100%, " ")
                                                     /* (EWD) - End    */
...
*        Set some flags and misc variables
            ...
            str(line1$,53%)= date$ & " BCKFASTR: " & str(cms2v$,,8%)
                                                   /* (PAR001)         */ 
        REM    smryhdr$ = "Seq Part Number                Order Qty"     &  ~
        REM               "   Open Qty   Open Ext Rqd Ship Status"
                      
            smryhdr$ = "Seq Part Number               New Part   "     &  ~
                       "Ord Qty Opn Qty Opn Ext  Rqd Ship Stat"
...
*        the fields related to pricing of the line item.
        ...
        clear_line   /* Clear variables for line C%                    */
            ...
            parttype$(c%), reftype$(c%), part_sub$(c%), model$(c%) = " "
                                                           /* (PAR001)  */
/*AWD004*/  partno1$(c%) = " "
...
*        length.
            ...
            for s% = 1%  to  wndw%
              ...
              if seq$(s1%) = " " then L17740
                str(smry$(s%),1%,3%) = seq$  (s1%)
                                                          /* (PAR001)   */
                if cms% = 1% then                                        ~
                    str(smry$(s%),5%,25%) = part$(s1%) else              ~
                    str(smry$(s%),5%,25%) = refpart$(s1%)
         
                gosub lookup_sub_part

                str(smry$(s%),31%,10%) = str(part_sub$(s1%),1%,10%)

                convert order(s1%) to str(smry$(s%),42%,7%), pic(###.##-)

                convert openqty(s1%) to str(smry$(s%),50%,7%), pic(###.##-)
...
L17690:         temp1 = round(temp * linedisc(s1%) * .01  , 2)
                ...
                convert temp to str(smry$(s%),58%,8%), pic(####.##-)

                str(smry$(s%),67%,8%) = shipdate$(s1%)

                str(smry$(s%),76%,4%) = str(lsts$(s1%),1%,4%)
                                                          /* (PAR001)   */
L17740:     next s%
            return
...
L26950:     refflag% = 1%
        ...
        init_for_input
            init(" ") errormsg$, inpmessage$, cuscode$, so$, po$,        ~
                      ...
                      keepcus$, cus$, custype$, cod$, bck$, sp$, keepso$,~
                      model$()
                                                    /* (EWD) - Mods   */
            ...
            mat alloc    = zer
            mat mlquote_cut% = zer
            mat dim1es   = zer            /* (AWD004) */
            mat dim2es   = zer            /* (AWD004) */
            mat dim3es   = zer            /* (AWD004) */
            smryh$ = smryhdr$
...
REM         write #09 using L30540, readkey$, " ", " ", " ", " ",         ~
REM                               eod goto L30250
REM L30540:         FMT CH(45), 3*CH(250), CH(225)
...
L31770:     if copy% = 1% then                                           ~
            ...
            print at(09,02,79), textidl$(c%) /* remove */           
...
L31910:     if lc% = 47% then gosub mlq_lines_date_check
        ...
        save_sales_order
            u3% = 2%
            call "ASKUSER" (u3%, "*** ATTENTION ***",                    ~
                           "This Order Has Not Been Saved!", "Note - "  &~
                           "Nothing in this S.O. Has Changed??"          ~
                           , "'STARTOVER' without saving data?")
        return clear all
        goto startover2
...
L32120: REM IF CRHOLD$ = "H" THEN CHECK% = 1%       /* Can't Save S.O. */
            ...
            if origuser$ <> "EDI" then check% = 1%  /* (EWD002) Fix    */
...
L38410:     errormsg$ = " "
        ...
        deffn'101(fieldnr%, edit%)                    /* (EWD002)      */
            str(line1$,20%,15%) = "Display Only"      /* (EWD) - Begin */
...
L43592:     if keyhit% <> 26% then goto L43595
                gosub call_customer_credit
                goto L42420
                                                        /* (PAR000)     */
L43595:     if keyhit% <> 9% then goto L43600
               gosub load_part_explosion
               goto L42420 
                                                        /* (PAR000)     */
...
L44090:  if fieldnr% > 0% then L44310     /* Edit Mode- Select Option   */
                                                      /* (PAR000)       */
           pf$(1) = "(1)Start Over  (8/24)ATC/QTY   (9)New Part Display"&~
                    "            (27)ATC Tgl (13)I"
           ...
           if parttype$(c%)  = "000" then mfgcode$ = "(29)Options"
                                                      /* (PAR000)        */
                                                      /* Set PF(9)       */
           pfkey$ = hex(01ffffffff060708090aff0c0dff0f1018191d1a121b00ff)
                                                      /* (PAR000)        */
                                                      /* (EWD)- 10/22/96 */
...
L60190:   err% = 0%  : size$ = "Y" : mat pc = zer
          upd$ = "Y"                     /* TEMP SET FOR UPDATE        */
            ...
            call "APCPR0SB" ( model$(c%),  /* Model Number           */  ~
                              part$(c%),  /* Part Number to Build    */  ~
                              partno1$(c%), /* (AWD003) Sub part No  */  ~
...
L61998:     if tqty = 0 and topen(4) = 0 then L62180 /* Nothing there */
            REM if totalar2 > crlimit        then L62080 /* Over Cr Limit */
...
L62800: return
        ...
        build_category
            init(" ") cat_key$, cat$(c%), acct$(), brand$
/* CR1837 */
            get #01 using L63110, brand$  
L63110:         FMT XX(960), CH(2)
            if len(part$(c%)) < 19 then cat_key$ = "PART"
/* CR1837  CR2501 */
            if len(part$(c%)) < 19 and (brand$ = "19" or brand$ = "18") ~
                      then cat_key$ = "PGPA" 
...
L63180: return
        ...
        load_part_explosion
            ...
            dim1es        = dim1es(c%)           /* (AWD004)          */
            dim2es        = dim2es(c%)           /* (AWD004)          */
...
sub_part2:
        ...
        call "AWDBKSUB"   (flag$,        /* Flag 0=SalesOrder 1=Invoice*/~
            ...
            if err1% <> 0% then str(bcksubpt_rec$,48%,20%) = "ERROR                    "

            part_sub$(s1%) = str(bcksubpt_rec$,48%,10%)          

            err1% = 0%
/* (AWD004) */          
            get bcksubpt_rec$ using dimFmt, dim1es(s1%), dim2es(s1%)
