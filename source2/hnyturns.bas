...
L11120:     if cursor%(1) < 11% then fieldnr% = cursor%(1%) - 6%
            if cursor%(1) = 13% then fieldnr% = cursor%(1%) - 8%
            if cursor%(1) > 15% then fieldnr% = cursor%(1%) - 10%
...
*        Define the Input Message for the Screen/Field Indicated
        ...
        initialize_variables
            ...
            init(" ") errormsg$, inpmessage$, ecat$, edate$, epart$,     ~
                      ...
                      store$, storedescr$, pdescr$, sdescr$
...
L30090:     call "PLOWNEXT" (#2, plowprime$, 0%, f1%(2))
                if f1%(2) = 0% then L30190
                     get #2 using L30120, mpart$, mpdesc$, mcat$
L30120:                  FMT CH(25), CH(32), POS(90), CH(4)
...
L31105:         FMT CH(42), CH(25), CH(32), CH(4), 2*PD(14,4), CH(13)
...
L40155:     accept                                                       ~
               ...
               at (09,51), fac(lfac$(03)), ecat$                , ch(04),~
                                                                         ~
               at (10,04), "Store Code",                                 ~
               at (10,23), fac(lfac$(04)), store$               , ch(03),~
               at (10,28), fac(hex(8c))  , storedescr$          , ch(30),~
                                                                         ~
               at (12,02), "Calculation Options:",                       ~
                                                                         ~
               at (13,04), "Weighting Factor",                           ~
               at (13,23), fac(lfac$(05)), factor$              , ch(04),~
                                                                         ~
               at (15,02), "Sort Options:",                              ~
                                                                         ~
               at (16,04), "Primary Sort",                               ~
               at (16,23), fac(lfac$(06)), psort$               , ch(01),~
               at (16,28), fac(hex(8c)),   pdescr$              , ch(30),~
                                                                         ~
               at (17,04), "Secondary Sort",                             ~
               at (17,23), fac(lfac$(07)), ssort$               , ch(01),~
               at (17,28), fac(hex(8c)),   sdescr$              , ch(30),~
                                                                         ~
...
L51300: REM Test for Primary Sort                 PSORT$
            if psort$ < "1" or psort$ > "3" then L51340
                if psort$ <> "1" then L51322
                ssort$     = " "
                if edit%  <> 2% then fieldnr% = 9%
L51322:     if psort$ = ssort$ then L51510        /* Error Msg */
            goto set_sort_descrs
L51340:     errormsg$ = "The Primary Sort Options are 1, 2, or 3 ONLY."
            return
...
L51400: REM Test for Secondary Sort               SSORT$
            if ssort$ = " " then goto set_sort_descrs
                if psort$ <> "1" then L51460
                     ...
                     return
L51460:         if ssort$ >= "1" and ssort$ <= "3" then L51500
...
L55145:             FMT POS(43), CH(6), CH(2), POS(51), 2*PD(14,4)
                if trancode$ = "IW" or trancode$ = "JK" or trancode$ =   ~
                           "RT" or trancode$ = "IQ" then L55150 else L55120
...
L56140:         str(workkey$,,29) = str(mcat$,,4) & str(mpart$,,25)
                goto L56210
L56170:     if ssort$ = "1" or ssort$ = " " then L56200
                workkey$=str(turns$,,13) & str(mcat$,,4) & str(mpart$,,25)
                goto L56210
L56200:     str(workkey$,,38) = str(turns$,,13) & str(mpart$,,25)
L56210:     write #8 using L56230, workkey$, mpart$, mpdesc$, mcat$,      ~
                                              tranvalue, invvalue, turns$
L56230:           FMT CH(42), CH(25), CH(32), CH(4), 2*PD(14,4), CH(13)
L56240:     return
