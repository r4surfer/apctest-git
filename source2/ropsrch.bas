        REM THISPROGRAMWASGENERATEDUSINGTHEGENPGMPROGRAMWHICHISAPROPRIETR~
            ...
            *          !  - Change to get ROP Class from ROPHNY   !     *~
            *          !  - Added Alt Key definition to ROPHNY    !     *~
            * 09/12/89 ! Changed number of records created-ROPHOLD! JDH *~
            * 11/25/91 ! PRR 12156.  Changed call to HNYTOTSB so  ! JDH *~
            *          !  that alpha stores are excluded.         !     *~
            * 03/08/94 ! Changed record length for BOMSPEC        ! WPH *~
            * 07/09/96 ! Corrected short arg list in ASKUSER.     ! JDH *~
        ...
        REM *************************************************************~
           ...
           select #25, "ROPHOLD", varc, indexed,                         ~
            ...
            call "OPENCHCK" (#23, fs%(20), f2%(20), 0%, rslt$(20))
...
L12400:     close #25
        ...
        find_rop_intrusions
               ...
               if str(plowkey$,1,25) <= lastpart$ then L12480
                  f1%(2) = 0  :  return
L12480:        get #2 using L12490, rop, eoq, pclass$
L12490:        FMT POS(26), 2*PD(14,4), POS(104), CH(4)
               if eoq = 0 then find_rop_intrusions
               if pclass$ > lastclass$  then find_rop_intrusions
               if pclass$ < firstclass$ then find_rop_intrusions
...
L12580:        get #1 using L12600, partdescr$, cat$, leadtime$,          ~
                                   type$, priority$
L12600:        FMT POS(26), CH(30), POS(90), CH(4), POS(170), CH(03),    ~
                                    POS(180), CH(3), POS(334), CH(1)
               if type$ > lasttype$ then find_rop_intrusions
               if type$ < firsttype$ then find_rop_intrusions
               if cat$ > lastcat$ then find_rop_intrusions
               if cat$ < firstcat$ then find_rop_intrusions
               convert leadtime$ to leadtime, data gosub L12774
               call "CONVERT" (leadtime, 0.0, leadtime$)
               convert leadtime$ to leadtime%
               store$ = " "
               lot$ = " "
               sw% = 1%     /* Use numeric stores only */
...
L12774:     leadtime = 0  :  return

        plow_pipin
               call "PLOWALTS" (#4, plowkey2$, 1%, 25%, f1%(4))
               if f1%(4) <> 1 then check_rop
               get #4 using L13040, pipdate%, pipqty
L13040:        FMT POS(26), BI(4), POS(49), PD(14,4)
               if today% + leadtime% < pipdate% then plow_pipin
...
L17660:        FMT CH(1), CH(1), POS(7), CH(25), CH(6), 3*PD(14,4),      ~
                ...
                if str(pf7$,9,1) = "D" then part_dsply$(i%) = part$(i%)  ~
                   else part_dsply$(i%) = part_descr$(i%)
            next i%
...
L20310: REM Def/Enable Exposure Class              CLASS$
            ...
            inpmessage$ = "Enter ROP Class Range."
            return
...
L50290: REM Test for ROP Class                   CLASS$
