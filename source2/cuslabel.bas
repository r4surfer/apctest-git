        REM CAELUSASSOCIATESSPOKANEWASHINGTONALLRIGHTSRESERVEDCAELUSASSOC~
            ...
            * 06/11/90 ! Modified to print standard label size    ! MJB *~
            *          !  15/16 x 3-1/2 inch labels.              !     *~
            * 03/26/92 ! Corrected RECORD$ size                   ! MJB *~
            * 12/15/93 ! PRR 13067.  Smarter # of records for WF. ! JDH *~
        ...
        com                                                              ~
            ...
            record$(12)250,              /* 3 RECORDS * 1200 CHARS EA. */~
        ...
        dim                                                              ~
            ...
            cusbilladdress$(3,5)30,      /* CUSTOMER SHIP-TO ADDRESS   */~
            cusname$(3)30,               /* CUSTOMER NAME              */~
            cusprt$(3)30,                /* CUSTOMER TYPE - CODE - RGN */~
            date$8,                      /* TODAY'S DATE               */~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            get rslt$(3%) using L02770, rec%
L02770:         FMT POS(17), BI(4)
            rec% = rec% / 2
        ...
        REM *************************************************************~
            ...
            * SORT_REGISTER                                  12000      *~
            *                                                           *~
            * PRINT_LINEUP_TEST                              18000      *~
            *                                                           *~
            * PRINT_LABELS                                   24000      *~
            *                                                           *~
            * CONTROL_PRINT_LABELS                           28000      *~
            *                                                           *~
...
L10130:        prompt$(5) = "   SORT BY (1 - 4)"   /* RESET SORT PROMPT */
...
L10240:        gosub test_or_print_screen
                     if keyhit%  =  1 then L10130
                     if keyhit%  =  5 then mode$ = "TEST"
                     if keyhit%  =  9 then mode$ = "PRINT"
                     if keyhit%  = 16 then exit_program

               if mode$ <> "TEST"    then L10300
                  gosub print_lineup_test
                  goto L10240

L10300:        gosub select_records
               gosub sort_register
        ...
        select_records:
            ...
            REM OPEN THE WORK FILE FOR SORTING
                ...
                call "WORKOPEN" (#10, "OUTPT", rec%, f2%(10))
        ...
        REM Now Process Sort Option On A Selected Record
            ...
            on sort% gosub L11370, L11420, L11470, L11520
                ...
                if str(a$(6%),17,1) <> " " or str(a$(6%),16,1) <> " " or ~
                    ...
                    a$(6%) = b$
                    goto L11340
L11330:         call "SPCESMSH" (a$(6%), 2%)
L11340:         call "LINSMASH" (a$())
                write #10, using L11355, sortkey$, str(a$()), cusprt$(1%)
L11355:                    FMT CH(20), CH(180), CH(20)
                goto L11180

L11370: REM  Sort Customer Code By Customer Type
            str(sortkey$,  1,  9) = str(record$(),   1, 9)
            str(sortkey$, 10,  2) = str(record$(),1023, 2)
            return

L11420: REM  Sort Customer Type By Customer Code
            str(sortkey$,  1,  2) = str(record$(),1023, 2)
            str(sortkey$,  3,  9) = str(record$(),   1, 9)
            return

L11470: REM  Sort Zip Code By Customer Code
            str(sortkey$,  1, 10) = str(record$(), 424, 9)
            str(sortkey$, 11,  9) = str(record$(),   1, 9)
            return

L11520: REM  Sort Region Code By Customer Code
        ...
        sort_register:
            call "SHOSTAT"("Sorting CUSTOMER Records...One Moment Please")
            call "SLCTSORT"(#10, 20%)
            return
...
L24170:         gosub read_next_labels   /* LOAD LABEL OFF SORT FILE   */

                   REM PRINT LINE ......................................
                         gosub control_print_labels
*       ***             PRINT USING 28360, CUSPRT$(1%),                 ~
*       ***                   CUSPRT$(2%),                              ~
*       ***                   CUSPRT$(3%)
                         print using L28360, cusname$(1%),                ~
                               cusname$(2%),                             ~
                               cusname$(3%)
                         print using L28360, cusbilladdress$(1%, 1%),     ~
                               cusbilladdress$(2%, 1%),                  ~
                               cusbilladdress$(3%, 1%)
                         print using L28360, cusbilladdress$(1%, 2%),     ~
                               cusbilladdress$(2%, 2%),                  ~
                               cusbilladdress$(3%, 2%)
                         print using L28360, cusbilladdress$(1%, 3%),     ~
                               cusbilladdress$(2%, 3%),                  ~
                               cusbilladdress$(3%, 3%)
                         print using L28360, cusbilladdress$(1%, 4%),     ~
                               cusbilladdress$(2%, 4%),                  ~
                               cusbilladdress$(3%, 4%)
                         print skip(1)
                         if f1%(10) = 0 then exit_print_labels
                goto L24170

        exit_print_labels:
                if record% = 1 then L24420 /* ANY LABELS PRINTED?       */
                   close printer
                   call "SETPRNT" ("CUS004", " ", 0%, 1%)
                   return

L24420:         REM IF RECORD% = 0, THEN TELL USER "NO LABELS"
                   errormsg$ = "No Labels Were Selected Based On The Choi~
        ~ces You Made"
                return
        ...
        control_print_labels:
                labelcnt% = labelcnt% + 1
                if labelcnt% < 13% then return
                   gosub new_page
                   return

        new_page:
                   select printer (107)
                   if labelcnt% > 200% then L28200
                                         /* NO NEW PAGE NEEDED         */
                      labelcnt% = 1%
                      return
L28200:            print using L28360, "CUSTOMER LBLS - CUSTOMER CODE:",  ~
                                   prtfrom$(1),                          ~
                                   prtto$(1)
                   print using L28360, date$ & "        CUSTOMER TYPE:",  ~
                                   prtfrom$(2),                          ~
                                   prtto$(2)
                   print using L28360, "                ZIP CODE:",       ~
                                   prtfrom$(3),                          ~
                                   prtto$(3)
                   print using L28360, "                REGION CODE:",    ~
                                   prtfrom$(4),                          ~
                                   prtto$(4)
                   print skip(2)
                   labelcnt% = 2%
...
L28400: %  XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX      XXXXXXXXXXXXXXXXXXXXXXXXXX~
        ...
        read_next_labels:
             ...
             for l% = 1 to 3%
                record% = record% + 1%
                read #10, record = record%, using L30260,                 ~
                          cusname$(l%), cusbilladdress$(l%,1%),          ~
                          cusbilladdress$(l%,2%), cusbilladdress$(l%,3%),~
                          cusbilladdress$(l%,4%), cusbilladdress$(l%,5%),~
                          cusprt$(l%), eod goto L30240
                if cusbilladdress$(l%,5%) <> " " then                    ~
                   cusbilladdress$(l%,4%) = cusbilladdress$(l%,5%)
...
L30260:     FMT XX(20), CH(30), 5*CH(30), CH(17)
...
L34590:         next temp%
            ...
            inpmessage$ = "Select Customer Range and Sort Option, "  &   ~
                          "Then Press (RETURN)"
