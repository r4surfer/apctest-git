        REM THISPROGRAMWASGENERATEDUSINGTHEGENPGMPROGRAMWHICHISAPROPRIETR~
            *                                                           *~
            *  PPPP    OOO   RRRR   EEEEE  L      X   X  RRRR   FFFFF   *~
            *  P   P  O   O  R   R  E      L       X X   R   R  F       *~
            *  PPPP   O   O  RRRR   EEE    L        X    RRRR   FFFF    *~
            *  P      O   O  R   R  E      L       X X   R   R  F       *~
            *  P       OOO   R   R  EEEEE  LLLLL  X   X  R   R  F       *~
            *                                                           *~
            *-----------------------------------------------------------*~
            * PORELXRF - Manage list of Part Class Codes that the       *~
            *            indicated user is allowed to process. Ie.,     *~
            *            what parts is the user allowed to release to   *~
            *            purchasing entry people from planning.         *~
            *-----------------------------------------------------------*~
            ...
            * 11/05/92 ! PRR 12631 - Now auto deletes Buyer Codes ! MLJ *~
            *          !  from PORELUSR if not found in USERLCMS. !     *~
            * 11/20/92 ! Standardized STARTOVER procedure and     ! MLJ *~
            *          !  display of screen lines 1 & 2.          !     *~
            * 07/19/93 ! Modify per Purchase jobs Req#564 -       ! RAN *~
            *          !  add fields for Alternate Buyer flag     !     *~
            * 07/26/93 ! Minor cleanup & standards.               ! JDH *~
            PRODUCTOFCAELUSASSOCIATESSPOKANEWASHINGTONALLRIGHTSRESERVED**

        dim                                                              ~
            alt$(500)1,                  /* Alternate Buyer flags      */~
        ...
        REM *************************************************************~
            ...
            * #2  ! PORELUSR ! User Reqisition Release Cross Reference  *~
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            str(line2$,62) = "PORELXRF: " & str(cms2v$,,8)
        ...
        inputmode
            init (" ") append$, errormsg$, message$, user$, code$(),     ~
                       codedescr$(), descr$, alt$()
...
L10210:         next fieldnr%
            ...
            message$ = "Enter Buyer Part Classes That This User Is Allowe~
        ~d To Release. 'ALL' for all."
...
L10410:     code$(c%), codedescr$(c%), alt$(c%) = " "
...
L11330:     gosub'112(0%)
            errormsg$ = " "
                  ...
                  if keyhit% <> 11 then L11480
                     ...
                     c% = maxlines%
                     message$= "Enter '?' to List Classes.  Enter 'Y' if ~
        ~Alternate Buyer, 'Blank' if Primary."
                     goto enter_lines
L11480:           if keyhit%  = 16 then       datasave
            ...
            message$= "Enter '?' to List Classes.  Enter 'Y' if Alternate~
        ~ Buyer, 'Blank' if Primary."
...
L12180:         if code$(c%)=" " then L12260 /*Avoid Never Never Land*/
                t% = c%
                gosub test_for_dup
...
L15170
*              IF TAG% = 0% THEN PRINT USING 16260
                print using L16350
                close printer
                goto inputmode
...
L15290:     call "PLOWNEXT" (#2, readkey$, 3%, f1%(2%))
                ...
                gosub form_control
                if tag% <> 0% then pageline% = pageline% - 1%
                tag% = 1%
                goto L15150

L15370:     get #2, using L15380, code$, typ$
L15380:         FMT XX(3), CH(3), POS(40), CH(1)
            if typ$ = "Y" then typ$ = "Alternate" else typ$ = "Primary"

            call "READ100" (#3, "BYCLASSES" & code$, f1%(3%))
                if f1%(3%) = 1% then L15450
                codedescr$ = "*** NOT ON FILE ***"
                if code$ <> "ALL" then L15480
                codedescr$ = "(Buyer Can Release ANY Part)"
                goto L15480
L15450:     get #3, using L15460, codedescr$
L15460:     FMT XX(24), CH(30)

L15480:     gosub form_control
            print using L16320, startuser$, descr$, code$, codedescr$, typ$
            tag% = 0%
            startuser$, descr$ = " "
            goto L15290
...
*                 IF PAGENUMBER% >0% AND TAG% = 0% THEN PRINT USING 16260
                   print page
...
L16220: %PAGE ###   B U Y E R/P A R T   C R O S S   R E F E R E N C E   L~
        ~ I S T I N G   DATE: ########

L16260: %----- ------------------------------ ---------- ----------------~
        ~-------------- ---------

L16290: %Buyer Name                           Part Class Description     ~
        ~               Type

L16320: % ###  ##############################     ###    ################~
        ~############## #########

L16350: %   *** End of Report ***
...
L30110:     call "PLOWNEXT" (#2, readkey$, 3%, f1%(2))
            ...
            get #2, using L30160, code$(c%), alt$(c%) /* Get Line Info */
L30160:         FMT XX(3), CH(3), POS(40), CH(1)
            if code$(c%) <> "ALL" then L30200
                codedescr$(c%) = "(Buyer Can Release ANY Part)"
                goto L30110
L30200:     call "DESCRIBE"(#3, "BYCLASSES" & code$(c%), codedescr$(c%), ~
                                                              1%, f1%(3))
...
L31170:     for c% = 1 to maxlines%
            write #2,using L31190, user$, code$(c%), user$, descr$,       ~
                                  alt$(c%), " "
L31190:         FMT 3*CH(3), CH(30), CH(1), CH(30)
            next c%
            return
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            deffn'101(fieldnr%)
                  ...
                  pfdescr$(2) ="(3)Print Cross Reference               "&~
                               "(14)Find Current Buyer  (16)Exit Program"
                  pfkeys$ = hex(0001030d0f100e)
                  if fieldnr% = 1 then L40130
...
L40130:           init(hex(8c)) lfac$()
                  ...
                  str(line2$,1%,58%) = "Last User Managed:"
                  str(line2$,20%,3%) = lastuser$
                  on fieldnr% gosub L40500          /* USER ID          */
                     goto L40570
...
L40570:     accept                                                       ~
               at (01,02), "Manage Buyer/Part Cross Reference",          ~
               at (01,66), "Today:",                                     ~
               ...
               at (06,02),                                               ~
                  "Buyer Code (User Id.)",                               ~
               at (06,24), fac(lfac$( 1)), user$                , ch(03),~
                                                                         ~
               ...
               if keyhit% <> 13 then L40822
                  call "MANUAL" ("PORELXRF")
                  goto L40570

L40822:        if keyhit% <> 14 then L40830
...
L40870:        if fieldnr% <> 0 then return
               close ws
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            deffn'112(fieldnr%)
                  ...
                  pfdescr$(2) ="(2)First       (5)Next    (10)Copy Buye"&~
                               "r XXX   (12)Remove Code    (16)Save Data"
...
L41530:     REM Common Logic Here...
            ...
            str(line2$,1%,58%) = "Buyer Part Class Codes this buyer may"&~
...
L41640:     accept                                                       ~
               ...
               at (06,20), fac(lfac$( 1)),  alt$(line%+01)      , ch(01),~
               at (07,13), fac(lfac$( 2)), code$(line%+02)      , ch(03),~
               at (07,20), fac(lfac$( 2)),  alt$(line%+02)      , ch(01),~
               at (08,13), fac(lfac$( 3)), code$(line%+03)      , ch(03),~
               at (08,20), fac(lfac$( 3)),  alt$(line%+03)      , ch(01),~
               at (09,13), fac(lfac$( 4)), code$(line%+04)      , ch(03),~
               at (09,20), fac(lfac$( 4)),  alt$(line%+04)      , ch(01),~
               at (10,13), fac(lfac$( 5)), code$(line%+05)      , ch(03),~
               at (10,20), fac(lfac$( 5)),  alt$(line%+05)      , ch(01),~
               at (11,13), fac(lfac$( 6)), code$(line%+06)      , ch(03),~
               at (11,20), fac(lfac$( 6)),  alt$(line%+06)      , ch(01),~
               at (12,13), fac(lfac$( 7)), code$(line%+07)      , ch(03),~
               at (12,20), fac(lfac$( 7)),  alt$(line%+07)      , ch(01),~
               at (13,13), fac(lfac$( 8)), code$(line%+08)      , ch(03),~
               at (13,20), fac(lfac$( 8)),  alt$(line%+08)      , ch(01),~
               at (14,13), fac(lfac$( 9)), code$(line%+09)      , ch(03),~
               at (14,20), fac(lfac$( 9)),  alt$(line%+09)      , ch(01),~
               at (15,13), fac(lfac$(10)), code$(line%+10)      , ch(03),~
               at (15,20), fac(lfac$(10)),  alt$(line%+10)      , ch(01),~
               at (16,13), fac(lfac$(11)), code$(line%+11)      , ch(03),~
               at (16,20), fac(lfac$(11)),  alt$(line%+11)      , ch(01),~
               at (17,13), fac(lfac$(12)), code$(line%+12)      , ch(03),~
               at (17,20), fac(lfac$(12)),  alt$(line%+12)      , ch(01),~
               at (18,13), fac(lfac$(13)), code$(line%+13)      , ch(03),~
               at (18,20), fac(lfac$(13)),  alt$(line%+13)      , ch(01),~
               at (19,13), fac(lfac$(14)), code$(line%+14)      , ch(03),~
               at (19,20), fac(lfac$(14)),  alt$(line%+14)      , ch(01),~
               at (20,13), fac(lfac$(15)), code$(line%+15)      , ch(03),~
               at (20,20), fac(lfac$(15)),  alt$(line%+15)      , ch(01),~
                                                                         ~
               ...
               at (23,02), fac(hex(8c)), pfdescr$(1)            , ch(79),~
               at (24,02), fac(hex(8c)), pfdescr$(2)            , ch(79),~
               at (24,43), fac(afac$),   append$                , ch(03),~
               keys(pfkeys$),                                            ~
               key (keyhit%)
...
L50120:     REM TEST DATA FOR USER ID
        ...
        REM  Buyer not in USERLCMS, check PORELUSR and if found, delete...
            readkey$ = str(user$) & hex(000000)
            call "PLOWNXT1" (#2, readkey$, 3%, f1%(2%))
                if f1%(2%) = 1% then L50168
            errormsg$ = "Buyer " & user$ & " is not a valid CMS user"
            return

L50168:     call "DELETE" (#2, readkey$, 3%)
            errormsg$ = "Buyer " & user$ & " is not a valid CMS user.  "&~
                        "Now deleted from the Buyer file."
            return
...
L50180:     gosub L30000
        ...
        REM *************************************************************~
            ...
            *************************************************************
            ...
            if alt$(c%)=" " or alt$(c%)="Y" then L51090
                errormsg$ = "Leave Alt Field BLANK for Primary Buyer;" & ~
                            " Enter 'Y' if Alternate." : return

L51090:     REM TEST DATA FOR CODE
            if code$(t%) <> "ALL" then L51130
                codedescr$(t%) = "(Buyer Can Release ANY Part)"
                return
L51130:     readkey$ = "BYCLASSES" & code$(t%)
            codedescr$(t%) = hex(06) & "Select Buyer Part Class"
            f1%(3) = -(max(1,t%-line%) +5)
