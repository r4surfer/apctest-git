        REM THISPROGRAMWASGENERATEDUSINGTHEGENPGMPROGRAMWHICHISAPROPRIETR~
            ...
            *04/16/2019! (CR1993) mod for new sales tax source    ! RDB *~
...
*        GLFMT).  It may or may not be valid!!!!
        ...
        esc_account          /* Derive ESC ACCOUNT     (AWD001)  (#12) */
            gosub read_customer
            if acct$(acct%) <> " " then exit_routine
...
L10470:     if acct% <> 7% then L10490    /* Freight          */
L10480:         gosub read_system        /* Freight & Tax    */
                if acct$(acct%) <> " " then exit_routine  else L65000
                
L10490:     if acct% <> 12% then L65000   /* ESC (AWD001)    */
                gosub read_system       
                if acct$(acct%) <> " " then exit_routine  else L65000
         
        hd_account       
L10495:     if acct% <> 13% then L65000                
                gosub read_taxcode
                if acct$(acct%) <> " " then exit_routine  else L65000
...
/* (AWD001) */                
            call "BCKSWTCH" ("AR ", "ESCACCT ", acct$(12), v, r%)
                call "GLUNFMT" (acct$(12))                
/* CR1993 */
            call "BCKSWTCH" ("AR ", "HDACCT ", acct$(13), v, r%)
                call "GLUNFMT" (acct$(13))    
